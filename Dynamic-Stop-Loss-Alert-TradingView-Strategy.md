...
## Overview
This strategy shows how to use TradingView's dynamic variables to pass stop prices in alerts and execute trades on the MT4/5 platform via TradingConnector. The strategy uses the Stochastic indicator to determine the timing of entry, dynamically sets the latest support and resistance as stop-loss points, and can also partially take profits.
## Strategy Principle
The K-line and D-line of the Stochastic indicator have golden crosses for longs and dead crosses for shorts. Calculate the nearest support and resistance points as the stop loss price. After entering the market, the stop loss price is transmitted to the broker in real time through dynamic variables. Partial take profit is set as a certain proportion of the stop loss distance. Take profit prices can also be passed dynamically.
## Advantage Analysis
- Dynamic stop loss allows the stop loss price to be set accurately
- Partial profit taking improves capital utilization efficiency
- Transmit the stop loss price to the broker account in real time
- Backtest stop loss price is close to the actual price, realistic simulation
## Risk Analysis
- Stochastic indicator has hysteresis
- Some take-profits are too frequent and affect positions
- Dynamic variables have different effects in different time frames
- Need to optimize the proportion of partial take-profit
You can appropriately shorten the parameter period of Stochastic, adjust the proportion of partial take-profit, etc. to control risks.
## Optimization direction
- Test different Stochastic parameter combinations
- Optimize the proportion setting of partial take profit
- Try different stop loss methods, such as trailing stop loss
- Tested in multiple markets and varieties
## Summarize
This strategy shows how to take advantage of the new TradingView features to execute dynamic stop-loss trades on MT4/5. Can be used as a basic framework for further backtesting. Optimization and adjustment for specific varieties still need to be made.
||
