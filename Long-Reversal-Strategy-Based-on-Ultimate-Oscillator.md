...
[trans]
The name of this strategy is "Reversal Long Strategy Based on Ultimate Volatility Indicator". This strategy uses the ultimate volatility indicator to determine overbought and oversold conditions, and performs long operations against the market when the indicator reaches the oversold state.
The ultimate volatility indicator integrates price information from multiple cycles to determine the overbought and oversold levels of the market. When the indicator crosses its lows, it indicates that the market has entered an oversold state, indicating that prices may rebound.
The transaction logic is as follows:
1. When the ultimate volatility indicator falls below the low point (such as 45), it means that the market is oversold and consider going long.
2. Continue to hold the long position until the indicator crosses the midline (such as 70), then close the position and take profit.
3. Set a stop loss line. If the price falls below the stop loss line, stop the loss and exit. If the indicator shows bullish divergence, the stop loss line can be adjusted appropriately.
4. If the indicator falls below the low again, you can consider adding a long position.
The advantage of this strategy is to capture oversold rebound opportunities. However, the indicator parameters need to be optimized, and the indicator itself lags behind and needs to be combined with trend analysis. Stop loss and money management are also particularly important.
Generally speaking, using indicators to determine reversal timing is a common method. However, traders still need to maintain flexibility in judgment and cannot rely entirely on any single indicator.
