...
Progressive element-weighted DCA quantitative trading strategy Quantized-Gradual-Weighted-DCA-Trading-Strategy
> Author
...
![IMG](assets/images/0363f02f11c36da48a02a23bcfb70bdc8f30a7d69cb0bb9761c8804103ed2e44.png)
[trans]


## Overview
The progressive element-weighted DCA quantitative trading strategy is a quantitative trading strategy that combines the moving average indicator trigger signal and the progressive weighted dollar cost averaging mechanism. This strategy aims to obtain more stable returns in markets with strong trend direction through trend judgment and cost sharing.
## Principle
The strategy mainly consists of three parts:
1. Judgment of entry signals
Use the intersection of the fast moving average and the slow moving average as a signal to determine entry. According to user settings, SMA, EMA or HMA can be selected as the fast and slow moving average. When the fast moving average breaks through the slow moving average from below, a buy signal is generated; when the fast moving average falls below the slow moving average from above, a sell signal is generated.
2. Progressively weighted DCA
After the buy signal is triggered, the strategy will immediately open a position to establish a basic position. If the price continues to fall thereafter, the strategy will increase subsequent safe positions in a progressively weighted manner. The price of each new safe position will be reduced by a certain amount based on the price of the previous safe position. At the same time, the amount of funds for new safe positions will also increase in sequence.
In this way, by gradually increasing positions, costs can be shared evenly to a certain extent, and a better cost price can be obtained while ensuring that transaction risks are controllable.
3. Stop profit and stop loss
When the price rises and breaks through the take-profit line, the strategy will choose take-profit; when the price falls and breaks through the stop-loss line, the strategy will choose stop-loss.
The take-profit line is fixed as 1 + a fixed ratio of the average transaction price of the basic position.
The stop loss line fluctuates with the price of the last safe position. The stop loss signal is confirmed based on a certain percentage below the transaction price of the last safe position.
## Advantages
1. Combine trend judgment and cost allocation to make the strategy more stable
Trend judgment can avoid directionless market shocks, and cost sharing can obtain better costs in the trend.
2. Gradually increasing positions can control risks
Each equilibrium position size has a certain range, and subsequent positions have certain retracement requirements, which can control risks.
3. Real-time monitoring of funds occupied by strategies
A real-time monitoring label is added to the code so that users can clearly know the upper limit of funds occupied by the strategy and avoid positions being liquidated due to excessive use.
4. Flexible take-profit and stop-loss for each position
Basic positions and safety positions can respectively stop profits and losses, terminate profits and control risks.
## Risk and Optimization
1. Severe price fluctuations may result in multiple positions being added.
When prices fluctuate violently, multiple positions may be triggered, thereby increasing losses. The number of additional positions can be reduced by appropriately increasing the retracement requirements between subsequent safe positions.
2. The selection of moving average parameters needs to be optimized.
The moving average parameters directly affect the timing of entry, and different varieties need to be tested to determine the appropriate parameters.
3. The stop-loss and take-profit ratios need to be tested and optimized.
The stop-loss and take-profit ratio is related to the rate of return and retracement control, and needs to be optimized through backtesting data.
4. Forced closing conditions can be set based on retracement or time
You can test adding forced liquidation conditions for maximum drawdown or position holding time exceeding the threshold to further control risks.
## Summarize
The progressive element-weighted DCA quantitative trading strategy combines the advantages of trend judgment and cost sharing, and can obtain stable returns in strong trend markets. By optimizing parameter settings, adjusting position size and retracement requirements between positions, stable transactions with controllable risks can be achieved. This strategy can be applied to the design of hedge funds, CTA funds and some adversarial strategies.
|| 
