...
Indicator-Verification-Automated-Trading-Strategy
> Author
...
## Overview
888 BOT v4 is an automatic trading strategy that combines multiple indicators to determine trends and issue trading signals. It uses a combination of 8 indicators including moving average, range filter, ADX, Parabolic SAR, Volume RSI, MACD and Bollinger Bands to send out more reliable trading signals.
## Strategy Principle
### Indicators Overview
1. **Jurik Moving Average (JMA)**: A moving average designed by Mark Jurik for professionals to eliminate signal lag. Its "length" parameter can control denoising, and its "phase" and "power" parameters can balance lag and overshoot.
2. **Range Filter**: By calculating the average price fluctuation range within a period of time and amplifying the range, you can filter out market noise and better judge short-term trends.
3. **Average Directional Indicator (ADX)**: Created by Wilder, measures the strength and direction of a trend. ADX has a positive slope and exceeds the threshold for a period of time, indicating strong price movement.
4. **Parabolic Stop Loss (SAR)**: Also created by Wilder, it places points to determine the trend. When the price contacts the indicator, it will jump to the other side of the price, forming a parabolic shape.
5. **Volume RSI**: Adds trading volume parameters to the classic RSI to make it more accurately reflect market trends.
6. **MACD**: A fast and slow moving average difference indicator created by Appel. Histogram can predict moving average crossovers. MAC-Z normalizes the price and adds it to the VWAP calculation.
7. **Trading volume conditions**: Filter signals that the trading volume is lower than the average volume. Different magnitudes correspond to different leverages.
8. **Bollinger Bands**: John Bollinger's wave band indicator, which can be used as a re-entry opportunity.
### Strategy logic
1. Determine the long and short conditions respectively based on eight indicators.
2. After multiple indicators are confirmed, a trading signal is generated and a position is entered.
3. Set take-profit and stop-loss levels based on position quantity and risk parameters.
4. When the price reaches the take-profit and stop-loss levels, close the position and leave the market.
5. When the price reaches the Bollinger Band again, there is an opportunity to increase the position and improve the entry price.
6. After each position is closed, wait for the indicator confirmation again before entering a new position.
### Advantage Analysis
The biggest advantage of 888 BOT v4 is the combination of indicators. Different indicators can verify each other and reduce false signals, which is more reliable than a single indicator strategy. In addition, adding positions and improving entry prices are allowed to pursue greater profits.
Specifically, its advantages are:
1. JMA removes lag and interval filter removes noise to improve signal quality.
2. ADX determines the strength of the trend, and Parabolic SAR determines the direction, making entry more accurate.
3. Add more market information and multi-layer verification signals with RSI and MACD.
4. Trading volume conditions can filter out false signals, and different magnitudes correspond to leverage.
5. The stop loss method can choose risk stop loss, ATR stop loss or double stop loss to control the risk of decline.
6. It is allowed to increase positions through Bollinger Bands to improve the entry price and pursue greater profits.
7. You can choose whether to divide the take profit level to balance profit and profit probability.
8. You can choose time periods and trading pairs for backtesting to evaluate the effectiveness of the strategy.
### Risk Analysis
Although 888 BOT v4 reduces risks through indicator combination and parameter optimization, there are certain risks in trading any strategy, mainly including:
1. The probability of the indicator sending out an erroneous signal can be reduced by appropriately adjusting the parameters.
2. The risk of loss expansion after adding a position can be avoided by reducing the optimization range of the entry price.
3. When the Bollinger Bands are not triggered, losses will expand. You can use trend indicators to determine the timing of adding positions.
4. If the stop loss is too loose and the risk is covered, the stop loss range can be appropriately narrowed.
5. If the backtest time is insufficient, the backtest cycle can be expanded for verification.
6. Insufficient trading volume, adjust the position size.
7. When special market conditions fail, risk management needs to be done, Jenkinson.
### Optimization direction
888 BOT v4 still has some room for optimization:
1. Adjust indicator parameters and find the best combination.
2. Try to replace it with other indicators, such as KDJ, oscillators, etc.
3. Optimize the entry price optimization range.
4. Adjust the take profit and stop loss algorithm.
5. Set a profit stop and then protect the capital and stop the loss.
6. Optimize position size and leverage size.
7. Try methods such as machine learning for automatic optimization.
8. Add exit conditions to avoid specific market conditions.
9. Try arbitrage across markets.
10. Develop graphical interface to improve ease of use.
In summary, 888 BOT v4 is a typical representative of a multi-indicator strategy, and trading through indicator combinations can significantly increase the probability of profit. However, no strategy can conquer the world. It requires continuous testing and optimization, as well as good risk management, in order to continue to make profits.
||
