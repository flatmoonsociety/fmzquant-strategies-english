...
![IMG](assets/images/181f6ec956016459f74053c8b704001d579aca80ef767488ef9e09b557b6ab11.png)

[trans]

## Overview
This strategy is based on the breakthrough signal of the Camarilla pivot point, combined with the RSI reversal indicator as a bargain hunting opportunity, to form an advanced momentum reversal bargain hunting strategy. When the price breaks through the Camarilla pivot point, a trading signal is generated, and the RSI low level further confirms the accumulation opportunity, which is an advanced momentum reversal strategy.
## Strategy Principle
The core signal of the strategy comes from the Camarilla pivot. Camarilla pivot points are calculated based on yesterday's price range and are divided into S1 to S5 pivot points and R1 to R5 pivot points. A buy signal is generated when the price breaks upward from the S1 pivot point, and a sell signal is generated when the price breaks downward from the R1 pivot point. In addition, combining the RSI indicator to determine whether it is oversold can improve the success rate of entry.
Specifically, the strategy first calculates the Camarilla pivot point based on yesterday's highest price, lowest price, and closing price. Then determine whether the closing price breaks through the pivot point to generate a trading signal. At the same time, determine whether the RSI indicator is at a low level. If it is below 30, it is considered oversold. Only when the closing price breaks through the pivot point and the RSI is below 30 will a real trading signal be generated. The long signal is an upward breakthrough of the S1 pivot point, and the short signal is a downward breakthrough of the R1 pivot point.
For example, if yesterday's price fluctuated between 10-11, today's closing price breaks through 11.05 (S1 pivot point), and the RSI indicator shows 20, a buy signal will be generated. If today's closing price breaks through 10.95 (R1 pivot point) and the RSI shows 20, a sell signal will be generated. Therefore, this strategy combines the advantages of breakout signals and oversold signals.
## Advantage Analysis
The biggest advantage of this strategy is identifying oversold and reversal opportunities. The Camarilla Pivot itself will hold important support and resistance points for the price. Combined with the RSI indicator to determine the timing of reversal, you can accurately locate the bottom and avoid chasing the rise and killing the fall. This is a relatively advanced breakthrough strategy.
In addition, the pivot point is dynamically calculated and price changes will be followed in time. Unlike traditional technical indicators, parameters need to be set. The strategy inherits the advantages of fulcrum analysis and is more flexible. In addition, the reversal opportunities are relatively clear and there will be no frequent false signals.
## Risk Analysis
The biggest risk of this strategy is that the price may falsely break through. Although combined with the RSI indicator to confirm the oversold condition, there is still the possibility of a reversal after the price breaks through the pivot point. This will cause the stop loss to be hit.
Another risk is that the RSI indicator fails. Even though it is oversold, the RSI has not dropped below 30. At this time, no trading signal is formed, and the reversal opportunity will be missed. To address this risk, the parameter settings of RSI can be appropriately optimized.
## Optimization direction
This strategy can be optimized through the following aspects:
1. Optimize the parameters of RSI. You can test different oversold levels to see whether 30 is better or 20 is more appropriate.
...
3. Test different Camarilla pivots. You can only use S1 and R1 to reduce the probability of false breakthroughs.
4. Optimize stop loss strategy. You can set the stop loss position based on the ATR indicator, or track the breakthrough point as the stop loss position.
5. Test different varieties of contracts. Applicable to different varieties such as stock indices, foreign exchange, commodities, etc. Parameters need to be adjusted.
## Summary
This strategy is an advanced momentum reversal breakout strategy. Breakthrough signals are determined through the Camarilla pivot point, and oversold conditions are determined by the RSI indicator. The advantage of the strategy is to identify reversal opportunities, and the biggest risk is false price breakthroughs. Through parameter optimization and risk management, the stability and profitability of the strategy can be further improved.
||
