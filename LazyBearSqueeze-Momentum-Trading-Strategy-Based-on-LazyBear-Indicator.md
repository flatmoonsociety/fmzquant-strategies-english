...
Quantitative trading strategy Squeeze-Momentum-Trading-Strategy-Based-on-LazyBear-Indicator
> Author
...
![IMG](assets/images/b7c5f75869fe0e512a1dff13862fb7c5a9414c4ce9a14c1d7f119ad2248a241e.png)
[trans]

## Overview
This strategy is based on LazyBear's compressed momentum indicator, adds a momentum filter, changes the data source, and adds a risk management system with customizable backtesting time periods, aiming to capture price explosions after volatility compression.
## Strategy Principle
This strategy uses the Bollinger Bands indicator and the Keltner Channel indicator to calculate the price channel. When the price breaks through the channel, it is regarded as a signal of increased volatility. It incorporates LazyBear's Compressed Momentum indicator, which uses a linear regression method to determine the direction of price momentum.
The strategy adds a momentum filter, which will only issue a trading signal when the absolute value of momentum exceeds the threshold. When volatility compresses (tightening within the channel) and the momentum filter passes, the strategy determines the direction of the trend and goes long or short. At the same time, set stop loss, take profit, and trailing stop loss to control risks.
## Advantage Analysis
This strategy integrates a variety of indicator judgments and is relatively comprehensive; it adds a risk management mechanism to limit single losses; it can judge the price trend direction in a timely manner after volatility is compressed; the parameters are customizable and have strong adaptability.
## Risk Analysis
The main risks include: false breakthroughs leading to wrong judgments; improper parameter settings and failure to reverse in time; stop loss being exceeded, causing losses to expand. You can optimize parameters, adjust risk management parameters, and select appropriate products and trading periods to reduce these risks.
## Optimization direction
You can consider combining other indicators to filter signals, such as trading volume indicators; adjusting the momentum threshold to be more precise; setting retracement stops to further control risks; and testing the data effects of more varieties. These optimizations can make the strategy more stable and generalizable.
## Summarize
This strategy more comprehensively judges price trends and volatility, has a high degree of integration, and complete risk control measures. It can be further improved according to the optimization direction, and has strong adaptability to capture price explosions after volatility compression.
||
