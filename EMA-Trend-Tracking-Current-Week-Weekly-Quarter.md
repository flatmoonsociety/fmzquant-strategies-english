...
EMA Trend Tracking Current Week Weekly Quarter
> Author
...
This strategy is based on trend tracking of the EMA moving average. When the EMA golden cross is formed, a position is opened and when an EMA dead cross is formed, the position is closed. Earn profits from trends. This strategy supports two-way opening of long and short positions on OKEX contracts.
Additional explanation: EMA (Exponential Moving Average) is an exponential moving average. Also called the EXPMA indicator, it is also a trend indicator. The exponential moving average is an exponentially decreasing weighted moving average.
The EMA trend following method is developed based on the trading philosophy of cutting off losses and letting profits run. The trend following method believes that due to the greed and fear of human nature in the market and the impact of the economic cycle, the market trend will continue to rise or fall and form a trend. By capturing the main trend of the market, market average or profits exceeding the market average can be obtained.
Note: This strategy is only for learning and debugging, use it for real trading at your own risk.
