...
![IMG](assets/images/d6cac36809e35c8b3bc4cab74916cab13d01f1652279d7fa213e2ad76c6f792d.png)
[trans]


## Overview
This strategy is based on the dual moving averages of Bollinger Bands for trend following trading decisions. It uses the convergence and divergence of the upper and lower rails of the Bollinger Bands to judge changes in trends, buy near the lower rails of the Bollinger Bands, sell near the upper rails, buy low, sell high, and exit at a profit.
## Strategy Principle
This strategy applies both the Simple Bollinger Bands and the Enhanced Bollinger Bands versions.
Simple Bollinger Bands uses the SMA of the closing price to calculate the middle track, and the enhanced Bollinger Bands uses the EMA of the closing price to calculate the middle track.
Both the upper and lower rails are calculated by ±N times the standard deviation of the middle rail.
The strategy determines the trend based on the distance (spread) between the upper and lower rails of the Bollinger Bands. When the spread is less than the set threshold, it means that it is entering the trend range, and trend following transactions can be performed.
Specifically, when the price is close to the lower track, buy to go long, and when the price is close to the upper track, sell to close the position. The stop loss method is a fixed stop loss percentage, and you can choose to enable trailing stop loss.
The target profit depends on choosing to close the position near the middle track or the upper track.
This strategy can also choose to sell only when profits are guaranteed to prevent losses.
## Advantage Analysis
This strategy has the following advantages:
1. Combination of double Bollinger Bands to improve decision-making efficiency
By applying simple Bollinger Bands and enhanced Bollinger Bands, you can compare the effects of the two Bollinger Bands, choose a better version, and improve decision-making efficiency.
2. Determine the degree of trend based on the width of the Bollinger Bands channel
When the Bollinger Bands channel narrows, it indicates that the market has entered a trend, and the winning rate of trading following the trend is higher.
3. Flexible stop-profit and stop-loss methods
Use a fixed percentage stop loss to control single losses. At the same time, you can choose to take profit near the middle track or the upper track, and enable trailing stop loss to lock in more profits.
4. Protection mechanism against losses
Only sell when profits are guaranteed to prevent losses from expanding.
## Risk Analysis
This strategy also has the following risks:
1. Drawback risk
There is a certain risk of retracement in trend-following trading, which requires you to withstand the psychological pressure of continuous losses.
2. Risk of volatile market conditions
When the Bollinger Bands channel is wide, it means that the market may enter a shock. At this time, the trading effect of this strategy is not good, and trading needs to be suspended to wait for the trend to re-form.
3. Risk of stop loss being triggered
Fixed percentage stop loss may be too aggressive and needs to be adjusted to a more moderate stop loss method such as ATR stop loss.
## Optimization direction
This strategy can be optimized from the following aspects:
1. Optimize Bollinger Band parameters
You can test different moving average parameters and standard deviation multiples to find Bollinger Band parameter combinations that are more suitable for different markets.
2. Combine with other indicators to filter
On the basis of Bollinger Band signals, the filtering of indicators such as MACD and KD can be added to reduce market-shaking transactions.
3. Optimize take-profit and stop-loss strategies
You can test different swimming stop loss methods, or optimize stop loss points based on amplitude, ATR and other indicators.
4. Optimize fund management
Optimize position management for each trade and test different replenishment strategies.
## Summarize
This strategy integrates the advantages of the dual Bollinger Bands indicator, judges the degree of the trend based on the width of the Bollinger Bands channel, and conducts tracking transactions of buying low and selling high during the trend. At the same time, set up a scientific stop-loss mechanism to control risks. This strategy can further improve stability through parameter optimization and filtering combined with other indicators.
||
