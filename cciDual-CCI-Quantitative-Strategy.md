...
![IMG](assets/images/dfa9fd397f662bbf952f12262dbd56fbc36b863a8787c990752870539a65ffd4.png)
[trans]

## Overview
This strategy forms a trading signal by combining the classic technical indicator CCI with the self-developed VCI and MCI dual indices, which is a typical quantitative trading strategy. It identifies the changing trends of Volume and Price, determines the main trading direction and intensity of the current market, and forms trading signals. It can be widely used in financial instruments such as digital currencies, foreign exchange, and stocks.
## Strategy Principle
1. Calculate the ohlc4 moving average and determine the price based on the cci indicator;
2. Calculate the obv indicator to measure the flow of funds;
3. Calculate the VCI index, that is, measure the distribution of capital flow through the variance of the obv indicator;
4. Calculate the MCI index, which measures the price distribution through the variance of prices;
5. Compare the VCI and MCI index to determine the market buying and selling situation;
  - VCI > MCI, buyer’s willingness is strong;
  - VCI < MCI, seller’s willingness is strong;
6. Form long and short signals based on the comparison between VCI and MCI;
## Advantage Analysis
1. This strategy comprehensively considers multiple dimensions of price, transaction volume and capital flow to determine the market buying and selling situation, and the signals are relatively accurate;
2. VCI and MCI can adapt to real-time changes in the market through dynamic standard deviation calculations;
3. The strategy parameters have been optimized through extensive backtesting and have strong stability;
## Risk Analysis
1. The calculation of price and trading volume indicators lags behind, and emergencies cannot be captured in advance;
2. A single strategy cannot fully cover complex and changing market conditions;
3. It needs to be used in conjunction with other auxiliary indicators and cannot judge the market alone;
## Optimization direction
1. Combine with prediction models such as deep learning to improve the accuracy of signal judgment;
2. Add risk control modules such as stop loss to improve the stability of the strategy;
3. Different parameter combinations can be tried to test the applicability in specific markets;
## Summarize
This strategy forms trading signals through the comparison of double CCI indices, takes into account multiple factors such as price and trading volume, and evaluates market buying and selling intensity. It is a typical and practical quantitative trading strategy. However, it still needs to be used in conjunction with other auxiliary tools to maximize the effectiveness of the strategy. It is worthy of further optimization to improve applicable scenarios and reduce risks.
||
