...
![IMG](assets/images/820629f7669135bdc36bfa70407de1e994d280d3674ce03f48e1950426b295ea.png)
[trans]

## Overview
The name of this strategy is "Trend Strategy Based on Price Volume Changes". This strategy achieves the purpose of tracking trends by calculating the cumulative changes in price and trading volume, and constructing long and short warehouse receipts based on moving averages.
## Strategy Principle
The core indicator of this strategy is the cumulative price and volume change indicator (MPVT). This indicator reflects market sentiment and capital inflows and outflows through changes in price and trading volume. The specific calculation formula is as follows:
```
rV = 交易量 / 50000
xCumPVT = 昨日xCumPVT + (rV * (最新收盘价 - 昨日收盘价) / 昨日收盘价)
```

Then combine the parameters Level and Scale to construct the price and volume change Residence indicator:
```
nRes = Level + Scale * xCumPVT
```

The Residence indicator reflects combined changes in price and volume. When it crosses above its N-day simple moving average, go long; when it crosses below its N-day simple moving average, go short.
## Advantage Analysis
This strategy mainly has the following advantages:
1. Judging market sentiment and capital flow through price and volume indicators can capture the turning point of the trend in time.
2. Combined with parameter optimization, the parameters of the strategy can be flexibly adjusted to adapt to different market environments.
3. You can realize the short-selling strategy and expand the application scenarios of the strategy by inputting parameters in reverse.
## Risk Analysis
There are also some risks with this strategy:
1. Price and volume indicators are prone to produce false signals, and breakthroughs may not be established. Parameters can be adjusted appropriately or combined with other indicators for filtering.
2. The trend market has better applicability, while the consolidation market is prone to produce wrong signals. Consider combining with trend and volatility indicators.
3. The parameter optimization effect depends on the historical period, which may cause the risk of over-fitting. Parameters should be adjusted appropriately or a step optimization method should be used.
## Optimization direction
This strategy can be optimized from the following aspects:
1. You can test different moving averages, such as weighted moving averages, EMAs, etc., and combine them to see which one works better.
2. It can be combined with other indicators, such as RSI, KD, etc., to filter signals to reduce the probability of false signals.
3. Different parameter combinations can be tested to find the best parameter pair. A stepwise optimization method can also be used to update parameters in real time.
4. The stability of the strategy can be improved by combining it with trend indicators such as Bollinger Bands.
## Summarize
This strategy calculates the cumulative value of price and trading volume changes and designs the Residence indicator of price and volume changes, which can effectively reflect the inflow and outflow of market funds. It is a typical price-volume COMBO strategy. This strategy is simple and practical, suitable for trending markets, and has a lot of room for optimization through parameter optimization and indicator combination. It is a highly recommended trend strategy.
||
