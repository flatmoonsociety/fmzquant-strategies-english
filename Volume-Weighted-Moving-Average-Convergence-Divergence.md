...
Volume-Weighted-Moving-Average-Convergence-Divergence Volume-Weighted-Moving-Average-Convergence-Divergence
> Author
...
![IMG](assets/images/49d11dfb37447b950a2a22aab68facc2288c83a79833ab3323de4650ae15f220.png)
[trans]

## Overview
This strategy is an improved version of the classic MACD indicator, using 11 different types of moving averages to smooth the price curve to reduce misleading signals. The indicator consists of fast lines, slow lines and bars. The fast and slow lines use the fast moving average and slow moving average of price respectively. The bars represent the difference between the fast and slow lines. When the fast line breaks through the slow line from bottom to top, a buy signal is generated, and on the contrary, a sell signal is generated.
## Strategy Principle
1. Calculate the fast moving average MA12. Allows you to choose 11 different moving average calculation methods, the default is the rate of change line VAR.
2. Calculate the slow moving average MA26. Allows you to choose 11 different moving average calculation methods, the default is the rate of change line VAR.
3. Calculate the difference between fast and slow lines SRC2 = MA12 - MA26.
4. To calculate the trigger line MATR for SRC2, a moving average with a length of 9 is used. 11 calculation methods can be selected. The default is the rate of change line VAR.
5. Calculate MACD bar HIST = SRC2 - MATR. A buy signal is generated when the bar changes from negative to positive, and a sell signal is generated when the bar changes from positive to negative.
## Advantage Analysis
1. 11 different moving averages can be selected to calculate speed lines and trigger lines, which greatly reduces the lag of common moving averages and improves the accuracy of forecast signals.
2. The change rate line VAR can automatically adjust the weight of the moving average to better adapt to market changes.
3. The double moving average applying the buffer principle can effectively filter market noise.
4. The MACD bar line as a trigger signal can overcome the lag problem caused by the intersection of the traditional MACD fast and slow lines.
## Risk Analysis
1. The MACD indicator has a weak ability to judge the trend of volatile market conditions.
2. The moving average itself will produce a certain lag. The VAR rate line can be partially mitigated but not completely resolved.
3. Accumulation of errors will lead to false signals or missed valid signals.
## Optimization direction
1. Choose a moving average calculation method that matches the specific market conditions. Choose a more accurate combination based on the backtest results.
2. Optimize the length parameters of the speed line and trigger line, and find the best parameter combination to reduce error signals.
3. Add additional indicators to confirm the buying and selling signals, such as RSI, Bollinger Bands and other indicators.
## Summarize
This strategy is an optimized version of the MACD classic indicator. Using a variety of moving average models to calculate MACD's fast and slow lines and bars greatly enhances the practicality of the indicator. At the same time, there are certain limitations, which need to be continuously optimized according to the actual situation in order to achieve maximum effectiveness in transactions.
