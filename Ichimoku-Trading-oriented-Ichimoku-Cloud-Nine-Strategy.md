...
Trading-oriented-Ichimoku-Cloud-Nine-Strategy Trading-oriented-Ichimoku-Cloud-Nine-Strategy
> Author
...
## Strategy Principle
This strategy is mainly based on the following Ichimoku signals for entry:
1. Cloud breakout: A signal is generated when the price closes above the upper or lower edge of the cloud.
2. TK Cross: A signal is generated when the steering line (Tenkan) crosses the baseline (Kijun)
3. Cloud twist: Signal generated when Senkou Span A line and Senkou Span B line cross
4. Edge Crossover: A signal is generated when price moves from one side of the cloud into the other side of the cloud
In addition, this strategy will also close positions in the following situations:
1. Close the position when the price closes into the clouds
2. Close the position when TK reverse crosses
3. Partial closing of positions when Williams pattern is broken
This strategy integrates multiple trading signals of Ichimoku Cloud Chart, aiming to improve the reliability of trading signals, while using typing to set stop losses and control risks.
## Strategic Advantages
Compared with the strategy of a single signal, this strategy comprehensively utilizes multiple signals of the Ichimoku cloud chart, which can filter out some misaligned signals and improve the accuracy of the signal. At the same time, the strategy parameters can be flexibly configured and suitable for different varieties and parameter optimization.
In addition, the Williams breakout is introduced into the strategy to set stop losses, which can more proactively control risks, lock in profits, and avoid huge losses.
## Strategy Risk
This strategy mainly faces the following risks:
1. The cloud chart indicator has hysteresis and cannot reflect price changes in a timely manner.
2. Multiple signals may be too conservative and miss some opportunities.
3. Partial stop loss may be breached and cause losses.
To address the hysteresis problem, parameters can be adjusted appropriately or some filtered signals can be turned off. For the risk of stop loss by type, you can adjust the time period of the type or stop the loss only partially.
## Strategy optimization direction
This strategy can mainly be optimized from the following aspects:
1. Adjust Ichimoku parameters to adapt to different cycles and varieties
2. Adjust or close some filtered signals to retain the core signal
3. Adjust the parameters of the pattern, use a pattern with a larger time period, or only use a partial stop loss
4. Add other indicator filters, such as energy indicators, etc.
## Summarize
Ichimoku Cloud Nine strategy integrates multiple trading signals of Ichimoku Cloud to improve the accuracy and winning rate of signals while taking advantage of the cloud indicators. The strategy also uses typing as a stop loss method to control risk. This strategy can be optimized through parameters and signals, and is suitable for algorithmic trading of multiple varieties.
||
