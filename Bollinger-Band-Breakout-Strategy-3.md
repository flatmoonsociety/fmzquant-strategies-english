...
![IMG](assets/images/0bbe86332c11919385d3fd25ca2c22202eebdf14bcb7b622f6f4b8b5d170e484.png)

[trans]

## Overview
The Broadband Breakout Strategy is a trend following strategy. It uses volatility ranges to determine when to enter and exit trades. Specifically, it uses the upper and lower bands of Bollinger Bands to determine whether the price has broken through. Go long when the price breaks through the upper band, and close when the price falls below the lower band.
## Strategy Principle
This strategy is based on the Bollinger Bands indicator. Bollinger Bands contain three lines:
1. Centerline - n-day simple moving average
2. Upper rail - midline + k * n day standard deviation
3. Lower track - middle line - k * n day standard deviation
The k value here is generally 1.5 or 2. When the price breaks through the upper track, it means that the stock has entered a strong area, so go long; when the price falls below the lower track, it means that the stock has entered a weak area, so close the position.
This strategy uses the 20-day midline and 1.5x standard deviation to construct Bollinger Bands. Go long when the price breaks through the upper track, exited has two options:
1. Use lower stop loss
2. Use midline stop loss
If it is a highly volatile stock, it is better to use a lower stop loss.
## Advantage Analysis
This strategy mainly has the following advantages:
1. Ability to effectively track price trends and capture breakthrough signals in a timely manner
2. Use the volatility range to determine the entry point, which can effectively filter the noise
3. Prebuiltr has two stop loss methods, you can choose the optimal plan according to the characteristics of the stock.
## Risk Analysis
There are also some risks with this strategy:
1. The breakthrough signal may be a false breakthrough and cannot effectively track the trend.
2. Improper stop loss setting may cause excessive stop loss
3. Unable to effectively handle consolidation market
These risks can be reduced by optimizing parameters and combining with other indicators.
## Optimization direction
This strategy can be optimized from the following aspects:
1. Optimize the parameters of Bollinger Bands and find the best parameter combination
2. Combine with trading volume and other indicators to verify the reliability of the breakthrough signal
3. Use other indicators to build a filtering mechanism to avoid false breakthroughs
4. Dynamically adjust the stop loss position to reduce stop loss risk
## Summarize
Broadband breakout strategy is generally a more classic trend following strategy. It can be improved through parameter optimization and rule optimization to make it more adaptable to different market environments. This strategy is easy to understand and implement, and is a good introductory strategy choice for quantitative trading.
||
