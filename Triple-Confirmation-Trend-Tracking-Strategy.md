...
## Strategy Principle
### Moving average to determine the main trend
The strategy uses a 52-period moving average to determine the main trend direction. When the price crosses above the moving average, it is judged to be an upward trend; when the price crosses below the moving average, it is judged to be a downward trend.
### Identification of secondary reversal by memory line
The strategy also uses forget lines to identify short-term secondary reversals. The calculation method of forgetting line is similar to that of moving average, but the CLOSE price is replaced by the opening price, which can reflect price reversal information more quickly. When the price crosses the falling forgetfulness line, it indicates that Prices will stabilize and rebound in the short term; when the price crosses the rising forgetfulness line, it indicates that the price will fall back in the short term.
### Super trend judgment reversal point
The strategy also incorporates the Super Trend Indicator to identify key reversal points. The super trend indicator combines the window period and price datos of the ATR indicator to dynamically adjust the upper and lower rails of the channel to determine the timing of reversal.
### Triple confirmation signal filtering
The strategy will go long only when the three indicators of the moving average, the forgetful line, and the super trend send out buy signals at the same time; when the three indicators send out sell signals at the same time, the strategy will go short. Through triple indicator confirmation, false signals can be effectively filtered and the probability of entry is increased.
## Advantage Analysis
### Multi-dimensional judgment, high probability
The strategy combines the three indicators of moving average, forgetfulness line and super trend to judge trends and key points from different dimensions to ensure high probability of entry.
### Quick response, real-time tracking
The introduction of the forgetting line ensures that the strategy can quickly respond to short-term price reversals; the super trend indicator of the ATR adaptive channel can also track price changes in real time.
### Automatically stop profits and losses to effectively control risks
The strategy has built-in automatic take-profit and stop-loss logic, which can dynamically adjust the take-profit and stop-loss points based on ATR, effectively controlling single losses.
## Risks and Solutions
### Risk of too high trading frequency
Due to frequent strategic trading signals, it is easy to cause over-trading. The moving average cycle parameters can be appropriately adjusted to reduce the trading frequency.
### Reversal of uncertainty risk
The effect of forgetting lines and super trend indicators in judging reversal points is uncertain, and there may be a risk of misjudgment. Filter conditions for indicator parameters can be added to ensure a higher probability of reversal signals.
### Risk of loss in volatile market conditions
In volatile market conditions, due to repeated crossovers, strategies will frequently open positions and stop losses, resulting in loss risks. Can identify volatile market conditions and suspend strategic trading at this stage.
## Optimization direction
### Combined with volatility indicators
Consider combining volatility indicators, such as Bollinger Bands. When the price is close to the upper and lower rails of the Bollinger Bands, avoiding new positions can effectively avoid the risk of market shock.
### Add entry filter conditions
You can try to add other auxiliary judgment indicators, such as KDJ, MACD, etc., and only enter the market when they also send signals at the same time. This can further filter out false signals and reduce unnecessary transactions.
### Optimize take profit and stop loss strategy
Take-profit and stop-loss strategies can be optimized, such as moving take-profit, index moving take-profit, half-position interval take-profit, etc., to make profits more and more stable.
## Summarize
The triple confirmation trend tracking strategy makes full use of the advantages of the three major indicators of moving average, forgetfulness line, and super trend to achieve high-probability judgment and capture of trends. At the same time, an automatic stop-profit and stop-loss mechanism is set up to effectively control single losses. What deserves further optimization is that it can be combined with other auxiliary indicators to filter entries and improve the stop-profit and stop-loss strategy to make the strategy more practical.
||
