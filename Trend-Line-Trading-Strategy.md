...
![IMG](assets/images/591022751be2f7c1ee3ad86b0d49d9bec689a2483b72457d55869583b10d44e5.png)
[trans]


## Overview
This strategy is based on the idea of ​​breaking through key support and resistance by identifying key uptrend lines and downtrend lines in the price chart and trading when the price breaks out of the trend lines. The strategy is simple and reliable and suitable for market environments with clear trends.
## Strategy Principle
This strategy calculates the highs and lows of the left and right bar lines to identify key points where prices rise and fall, thereby obtaining support and pressure lines. Specifically:
1. Use the `pivothigh()` and `pivotlow()` functions to detect key highs and lows.
2. Draw equations for support and pressure lines based on high and low points.
3. When the price breaks through the pressure line, go long; when the price breaks through the support line, go short.
4. Choose to go long or short based on the trend direction.
5. You can choose whether to directly close the position and change direction when the breakthrough occurs.
6. You can choose to use stop loss, take profit, and trailing stop loss.
7. Swing point stop loss, ATR stop loss, and fixed stop loss can be selected.
This strategy uses simple trend line breakout trading, taking into account trend tracking and trend reversal, and is simple and practical.
## Advantage Analysis
- The strategy is relatively simple, easy to understand and implement.
- Using breakthrough theory, there is a certain probability advantage.
- Stop loss and profit can be set to control risks.
- Trend following or trend reversal can be achieved.
- Parameters can be optimized and suitable for different market environments.
## Risk Analysis
- Breakout signals may contain false positives.
- Improper stop loss setting may increase losses.
- There is a risk of being trapped in the reversal operation.
- Parameter tuning requires experience, and improper settings may fail.
- Pure trend breakthrough, not suitable for volatile market conditions.
Risks can be reduced by optimizing stop loss strategies, evaluating the quality of breakout signals, and evaluating reversal timing.
## Optimization direction
- Evaluate the reliability of breakthrough signals and improve accuracy.
- Combined with volume to strengthen breakthrough signals.
- Optimize stop loss strategy to adapt to market fluctuations.
- Evaluate the best time to reverse.
- parameter tuning.
- Evaluate multifactor models.
- Evaluate use in combination with other indicators.
## Summarize
This strategy is overall simple and practical, capturing price trends through simple trend breakthroughs with controllable risks. The strategy can be optimized in many aspects and is suitable for more market conditions. Overall, it is a very practical trend following strategy.
||
