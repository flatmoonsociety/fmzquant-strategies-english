...
Missed Signal Tracking Trading Strategy Based on Reversal RSI RSI-Signal-Tracking-Reversal-Trading-Strategy
> Author
...
## Overview
This strategy implements reversal trades by tracking overbought and oversold signals that the RSI indicator misses. When the RSI indicator falls back from the overbought area, it generates a tracking long signal, and when it rebounds from the oversold area, it generates a tracking short signal to capture reversal opportunities.
## Strategy Principle
### Signal determination
The RSI indicator is used to determine overbought and oversold. When the RSI goes above the overbought line, it is an overbought signal, and when it goes below the oversold line, it is an oversold signal.
```pine
overbought = rsi > uplimit 
oversold = rsi < dnlimit
```

If the previous K-line RSI indicator is in an overbought state, the current K-line RSI indicator exits the overbought state, generating a tracking long signal `up1`; if the previous K-line RSI indicator is in an oversold state, the current K-line RSI indicator exits the oversold state, generating a tracking short signal `dn1`.
```pine
...
### Exit judgment
When the position direction is consistent with the direction of the K-line entity, and the entity breaks through half of its 10-period average, an exit signal is generated.
```pine
...
1. Track the reversal signals missed by the RSI indicator to avoid the difficulty of catching overbought and oversold points in time.
2. Use the reversal attribute of the RSI indicator to capture reversal opportunities.
3. Make exit judgment based on the direction and size of the K-line entity to avoid continuing to track after rebound.
## Risks and Solutions
1. Risk of RSI indicator sending false signals
- Solution: Confirm in combination with other indicators to avoid misinformation.
2. When following the signal, the price may have already corrected to a certain extent, and the risk of loss is high.
- Solution: Reduce the entry position, or optimize the entry opportunity.
3. The risk of sending an exit signal if a partial rebound fails to make a profit.
- Solution: Optimize the exit determination logic and improve the profit opportunities of positions.
## Optimization ideas
1. Optimize parameter settings, such as overbought and oversold lines, lookback cycles, etc., and adjust them for different markets.
2. Adjust position management methods, such as reducing positions when tracking signals.
3. Optimize the timing of entry and add other conditional restrictions based on tracking signals.
4. Optimize exit methods and increase profit probability, such as introducing moving take-profit and other methods.
5. Optimize stop loss methods and reduce loss risks, such as introducing trailing stop loss, sector stop loss, etc.
## Summarize
This strategy implements tracking reversal trading based on the overbought and oversold signals of the RSI indicator. The strategy has the advantage of tracking reversal signals, but there is also a certain risk of false signals and losses. Through continuous optimization, the stability and profitability of the strategy can be further improved.
||
