...
Income can be put together with the strategy, but it is better to stand alone. Reasons: 1. When the trading strategy change is interrupted, the income will not be reset. 2. The income calculation itself will call API functions, which often causes strategy API network errors and affects transaction operation. Being independent can reduce this possibility. 3. You can customize the cycle interval.
