...
![IMG](assets/images/3400923fc9134238176f685a21fd8d6a74c0956b99cbd2206c7b16f1a194e4a8.png)
 [trans]

## Overview
The name of this strategy is “Bollinger Bands Based Price Action Strategy”. It integrates price action analysis and Bollinger Band indicators, and uses compound conditional judgment to generate trading signals.
## Strategy Principle
This strategy first calculates the upper and lower rails of the Bollinger Bands, and then determines whether the last K line breaks through the upper and lower rails of the Bollinger Bands. At the same time, it will also be judged whether the entity of the last K line is only half of the entity of the previous K line. When these two conditions are met, a trading signal is issued.
Specifically, the strategy uses the situation where the red K-line entity becomes smaller in the falling market to only half the size of the previous K-line entity, and cooperates with the closing price of the last K-line to break through the lower Bollinger Band as a long signal. On the contrary, use the situation where the green K-line entity becomes smaller in the rising market to only half of the previous K-line entity, and cooperate with the closing price of the last K-line to break through the upper Bollinger Band as a short-selling signal.
## Advantage Analysis
This strategy combines technical indicators and price action judgment to effectively filter out false breakthroughs. At the same time, it only sends signals at trend reversal points to avoid repeated trading in the trend. In addition, the strategy uses the characteristics of the K-line entity to become smaller and can lock the trend reversal point after slight adjustment. These advantages can improve the stability and profitability of the strategy.
## Risk Analysis
The main risk of this strategy is improper setting of Bollinger Band parameters and failure to break through. If the Bollinger Band parameter setting is too large or too small, it will lead to misjudgment. In addition, even if the price breaks through the upper and lower Bollinger Bands, it may be a false breakthrough and cannot form a real trend reversal. These risks may lead to strategic trading losses. In order to reduce these risks, Bollinger Band parameters can be appropriately adjusted, or other indicators can be added for combined verification.
## Optimization direction
This strategy can be optimized from the following aspects:
1. Optimize Bollinger Band parameters to make it more effective in capturing trends and fluctuations.
2. Add a trailing stop to lock in profits and manage risks.
3. Combine with other indicators such as MACD, RSI, etc. to verify and filter out false signals.
4. Add machine learning algorithms and use big data to train models to dynamically optimize strategy parameters and indicator weights.
## Summarize
This strategy successfully combines price action and Bollinger Band indicators to achieve high profitability with low risk. It only sends signals at key points to avoid interference from noises. By continuously optimizing parameters and filtering conditions, this strategy is expected to obtain more stable excess returns. It provides a reliable template for quantitative trading practice.
||
