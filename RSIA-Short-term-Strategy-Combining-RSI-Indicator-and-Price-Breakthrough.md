
> Name

A-Short-term-Strategy-Combining-RSI-Indicator-and-Price-Breakthrough
> Author

ChaoZhang

> Strategy Description

![IMG](assets/images/f7f58e841aa53b13a286f8df043e6aded0ded8e33bc6f01d7b113045cdd72caa.png)
[trans]

## Overview
This strategy combines the RSI indicator with price breakthroughs, looks for rotation opportunities within the consolidation range formed under a certain trend, and then conducts short-term transactions to pursue efficient short-term profits.
## Strategy Principle
1. RSI indicator judgment: When the RSI indicator is less than the oversold line of 30, a buy signal is generated as a potential reversal buying point; when the RSI indicator is greater than the overbought line of 60, a sell signal is generated to lock in profits;
2. Window restriction: It only takes effect within the specified backtest time window, thereby limiting the effectiveness of the strategy and preventing global arbitrage;
3. Breakthrough judgment: Combined with price trends, look for breakthrough opportunities, enhance the actual effect of the strategy, and prevent unnecessary idling.
Therefore, this strategy integrates the judgment logic of multiple dimensions, and uses the buying and selling signals generated by the RSI indicator to perform short-term profit-making rotation operations under certain trends and breakthrough opportunities. It can effectively seize the opportunity of short-term oversold rebound and overbought pullback in the market.
## Advantage Analysis
1. Combined with multiple logical judgments, it is more rigorous than the simple RSI strategy and can effectively avoid unnecessary losses caused by two-way idling;
2. Use the RSI indicator to determine local extreme value areas and look for reversal opportunities to make profits;
3. Set a backtest time window to verify and optimize specific market conditions to improve the actual usability of the strategy;
4. When pursuing short-term profits, there is no need to predict trend changes, which makes it easier to grasp and reduce risks.
## Risks and Solutions
1. It is impossible to directly judge the overall trend direction and requires manual analysis of the overall situation;
2. The RSI indicator lags in reflecting price changes and may miss the best buying and selling point;
3. It is necessary to fully understand the general market environment in which the strategy is applicable;
4. More technical indicators can be introduced to determine the general trend, optimize strategy parameters, and improve strategy flexibility.
## Optimization direction
1. Increase your judgment on the general trend and avoid long-term loss-making orders;
2. Adjust RSI parameters, optimize overbought and oversold lines, and improve the effect;
3. Add stop loss logic;
4. Optimize the backtest window range to make the strategy more in line with the actual market.
## Summary
This strategy uses the RSI indicator to determine short-term reversal opportunities for overbought and oversold conditions, and combines price breakthroughs with short-term profit-making rotation operations. It is characterized by the pursuit of short-term efficiency, simple operation, and limited risk. It is very suitable for short-term traders to use under specific market conditions. It is necessary to pay attention to judging the overall general trend and optimizing parameters to obtain better results.
||

## Overview  
This strategy combines the RSI indicator with price breakthroughs to find rotation opportunities within a certain trend and range-bound market, so as to make short-term trades and pursue highly efficient short-term profits.   

## Strategy Principle  
1. RSI Indicator Rule: Generate a buy signal when RSI falls below 30 as a potential reversal buy point. Generate a sell signal when RSI rises above 60 to lock in profits.  
2. Window Limit: Only take effect within the specified backtest time window, which limits the strategy's effectiveness and prevents overall arbitrage.  
3. Breakthrough Judgment: Identify breakthrough opportunities combined with price trends to enhance the actual effect of the strategy and avoid unnecessary idling.  

Therefore, this strategy integrates multiple dimensions of judgment logic to conduct short-term profitable rotation operations utilizing the buy and sell signals generated by RSI indicator, under certain trends and breakthrough chances. It can effectively seize the reversal bounce opportunity when the market is extremely oversold, as well as the retracement chance when extremely overbought in the short term.

## Advantage Analysis 
1. Integrating multiple logical judgments, which is more rigorous compared to simple RSI strategies, and can effectively avoid unnecessary losses caused by two-way idling.  
2. Identify local extremes with RSI indicator to spot reversal opportunities and make profits.   
3. The backtest time window setting enables verification and optimization targeting specific market conditions, improving the practical applicability of the strategy.  
4. Pursuing short-term profits without predicting trend reversals is easier to grasp and reduces risks.  

## Risks and Solutions  
1. Unable to directly determine the overall trend direction, manual analysis of the big picture needed.  
2. RSI indicator lags in responding to price changes, potentially missing out the best buy/sell timing.  
3. Requiring adequate understanding of the macro market environment where the strategy is applicable.  
4. Can incorporate more technical indicators for major trend judgment and optimize strategy parameters to improve flexibility.   

## Optimization Directions
1. Add judgment of major trends to avoid holding losing trades in prolonged drawdowns.  
2. Adjust RSI parameters and optimize overbought/oversold lines to improve efficiency.
3. Add stop-loss logic.   
4. Optimize the scope of the backtest window for better fitting actual market conditions.  

## Summary  
This strategy leverages RSI indicator to identify short-term reversal opportunities from extremely overbought/oversold scenarios, and conducts short-term profitable rotation operations combined with price breakthroughs. Its characteristics are pursuing short-term efficiency, easy operation, limited risks, and hence extremely suitable for short-term traders to use under certain market conditions. Attention should be paid to judging the overall major trend, parameter optimization etc., in order to obtain better performance.

[/trans]

> Strategy Arguments



|Argument|Default|Description|
|----|----|----|
|v_input_1|true|From Month|
|v_input_2|true|From Day|
|v_input_3|2020|From Year|
|v_input_4|true|Thru Month|
|v_input_5|true|Thru Day|
|v_input_6|2112|Thru Year|
|v_input_7|true|Show Date Range|
|v_input_8|30|oversold|
|v_input_9|60|overbought|


> Source (PineScript)

``` pinescript
/*backtest
start: 2024-01-01 00:00:00
end: 2024-01-31 23:59:59
period: 1h
basePeriod: 15m
exchanges: [{"eid":"Futures_Binance","currency":"BTC_USDT"}]
*/

// This source code is subject to the terms of the Mozilla Public License 2.0 at https://mozilla.org/MPL/2.0/
// © relevantLeader16058

//@version=4
strategy(shorttitle='RSI Classic Strategy',title='RSI Classic Strategy (by Coinrule)', overlay=true, initial_capital = 1000, process_orders_on_close=true, default_qty_type = strategy.percent_of_equity, default_qty_value = 30, commission_type=strategy.commission.percent, commission_value=0.1)

//Backtest dates
fromMonth = input(defval = 1,    title = "From Month",      type = input.integer, minval = 1, maxval = 12)
fromDay   = input(defval = 1,    title = "From Day",        type = input.integer, minval = 1, maxval = 31)
fromYear  = input(defval = 2020, title = "From Year",       type = input.integer, minval = 1970)
thruMonth = input(defval = 1,    title = "Thru Month",      type = input.integer, minval = 1, maxval = 12)
thruDay   = input(defval = 1,    title = "Thru Day",        type = input.integer, minval = 1, maxval = 31)
thruYear  = input(defval = 2112, title = "Thru Year",       type = input.integer, minval = 1970)

showDate  = input(defval = true, title = "Show Date Range", type = input.bool)

start     = timestamp(fromYear, fromMonth, fromDay, 00, 00)        // backtest start window
finish    = timestamp(thruYear, thruMonth, thruDay, 23, 59)        // backtest finish window
window()  => true


// RSI inputs and calculations
lengthRSI = 14
RSI = rsi(close, lengthRSI)
oversold= input(30)
overbought= input(60)

//Entry 
strategy.entry(id="long", long = true, when = RSI< oversold and window())

//Exit
//RSI
strategy.close("long", when = RSI > overbought and window())



```

> Detail

https://www.fmz.com/strategy/441161

> Last Modified

2024-02-06 12:01:14
