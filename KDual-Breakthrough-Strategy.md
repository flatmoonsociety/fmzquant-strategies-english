...
Dual-Breakthrough-Strategy based on K-line
> Author
...
![IMG](assets/images/c9c4c3ef123dd35630d4e8e54497892ef69c119384880676878dff33744c2fa3.png)
[trans]

## Overview
This is a two-way breakthrough trading strategy based on K-line. It will generate a trading signal when the closing price of the current K line breaks through the highest price and lowest price of the previous two K lines.
## Strategy Principle
The basic logic of this strategy is:
1. Define bull signal: `bull = close > open and close > math.max(close[2], open[2]) and low[1] < low[2] and high[1] < high[2]`. That is to say, the closing price of the current K line is greater than the opening price and greater than the highest price of the previous two K lines, while the lowest price of the current K line is lower than the lowest price of the previous K line.
2. Define bear signal: `bear = close < open and close < math.min(close[2], open[2]) and low[1] > low[2] and high[1] > high[2]`. That is to say, the closing price of the current K line is lower than the opening price and lower than the lowest price of the previous two K lines, while the highest price of the current K line is higher than the highest price of the previous K line.
3. When a bull signal is triggered, go long; when a bear signal is triggered, go short.
4. Stop loss and take profit levels can be set.
This strategy takes advantage of the characteristics of two-way breakthroughs to judge changes in trends by breaking through key price ranges, thereby generating trading signals.
## Advantage Analysis
This is a relatively simple and intuitive breakout strategy with the following advantages:
1. The logic is clear, easy to understand and implement, and the threshold is not high.
2. Breakthroughs are common trading signals and can easily form a trend.
3. Going long and short at the same time allows two-way transactions to increase profit opportunities.
4. Stop loss and profit can be flexibly set to control risks.
## Risk Analysis
There are also some risks with this strategy:
1. Two-way transactions are risky and require close monitoring.
2. Breakthroughs are easy to be trapped and may form false signals.
3. Improper parameter settings may lead to over-trading.
4. Improper stop-loss and stop-profit settings will also affect profit margins.
Parameters can be optimized and varieties appropriately screened to reduce risks.
## Optimization direction
This strategy can be optimized from the following aspects:
1. Optimize parameters, such as breakthrough cycle parameters, stop loss and take profit range, etc.
2. Add filtering conditions to avoid false signals such as arbitrage and market fluctuations.
3. Combine with trend indicators to avoid consolidation range.
4. Optimize fund management and improve position algorithm.
5. Different varieties have different parameters and can be tested and optimized separately.
## Summary
This is a simple strategy based on the idea of a two-way breakout. It has the advantages of clear logic and easy implementation, but also has certain monitoring risks. Through parameter and condition optimization, it is expected to obtain better strategic effects.
||
