...
![IMG](assets/images/5ef71dc0b81e5a6780528b6483b0c7e3776bbf91542bc4f78f2cccc1b66020e5.png)
[trans]

## Strategy Overview
This strategy comprehensively uses multiple technical indicators such as support and resistance levels, super trend indicators, and moving averages to comprehensively determine the trend direction on multiple time frames and implement a trend following trading system. The main idea of ​​this strategy is: first use the pivot point to determine the support and resistance position of the current price, then use the super trend indicator to determine the current trend direction, and finally use the moving average to filter the divergence market. At the same time, the strategy also supports risk control, such as setting trading time windows, limiting maximum positions, etc.
## Strategy Principle
1. First, calculate the pivot point based on the highest and lowest prices in a certain period in the past to obtain the support and resistance levels of the current price.
2. Use the Supertrend indicator to determine the trend. Supertrend is calculated from dynamic support and resistance and is a trend following indicator.
3. Use ATR to stop loss. This strategy adds ATR as a stop loss level based on the original super trend.
4. Use moving average as trend filter. Only go long when the trend is up and the price is above the moving average, and go short when the trend is down and the price is below the moving average.
5. Set the trading time window. Only open positions within a specific time range to avoid trading at important time points.
6. Long and short positions are managed separately. Long and short signals trigger their own logic for opening and closing positions respectively.
In summary, this strategy opens a position when the three conditions of pivot point support and resistance, super trend direction, and moving average direction resonate, and closes the position if any one of the conditions fails. This effectively captures the trend and controls risks at the same time.
## Advantage Analysis
1. The advantage of trading based on support and resistance levels is that it conforms to the laws of market supply and demand, and pivot points can dynamically reflect market balance.
2. Super trend can effectively capture the trend and stop losses in time. ATR stops further control risk.  
3. Moving average filtering avoids counter-trend trading. Entering the market when the trend and moving average resonate, the winning rate will be higher.
4. Customized trading time windows avoid trading at important time points, such as opening and closing.
5. Long and short signals operate independently, and long and short positions can be held at the same time, making full use of market opportunities.
## Risk Analysis
1. Risks of frequent trading. This strategy may frequently open and close positions in volatile markets, resulting in excessive transaction costs.
2. Counter-trend trades may still occur. Although this strategy uses moving average filtering, if the moving average itself goes against the general trend, counter-trend trading may still occur.
3. Parameter optimization problem. The strategy contains many parameters, such as the period and multiplier of the super trend, the moving average period, etc. Different parameters will produce different results, and how to choose the optimal parameter combination is a challenge.
4. It may fail under extreme market conditions. In extreme market conditions, such as sharp rises and falls, liquidity crises, etc., this strategy may not be able to stop losses in time.
## Optimization direction
1. Introduce more medium and long-term moving averages to improve the reliability of trend judgment and reduce frequent transactions.
2. Consider introducing volatility indicators, such as Bollinger Bands, to reduce trading in high-volatility markets.  
3. Optimize various parameters, find the best parameter combination, and improve the stability of the strategy.
4. Set hard stop loss under extreme market conditions to control risks. In addition, you can consider adding judgment on abnormal market fluctuations, such as price gaps, surges in trading volume, etc., and reduce or stop trading when abnormal.
## Summarize
This strategy combines a variety of technical analysis methods such as support and resistance, trend tracking, and momentum filtering, and can effectively profit in trending markets while controlling retracement risks. Its advantage is that the signal is clear and concise, the logic is clear, and it is suitable for use in medium and long periods. However, this strategy also has problems such as frequent trading, difficulty in parameter optimization, and insufficient risk control under extreme market conditions. In the future, further improvements can be made by introducing more technical indicators, optimizing parameters, setting hard stops, and judging abnormal market conditions. In general, this strategy is a relatively mature trend following strategy, and after proper optimization and improvement, it can become a robust trading system. The strategic ideas can be used for reference, but they still need to be combined with actual trading experience and market characteristics and used prudently. Quantitative trading requires both mathematical logic and human subjective judgment. Only the organic combination of the two can create ideal returns.
||
