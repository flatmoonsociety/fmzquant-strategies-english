...
![IMG](assets/images/080cafd8e8d94c0cb39bea789ffa63ed33cad55e2ca1f391316b55b3b69a0368.png)

[trans]

### Overview
This strategy is mainly used to automate long-term investment in BTC. Determine the trend direction through the intersection of double EMA and LSMA, and use the ATR indicator to calculate dynamic stop loss to effectively track the BTC bull trend.
### Strategy Principles
1. Use the 25-period EMA and the 100-period LSMA to form a double moving average, and their intersection is used to determine the market trend. EMA quickly responds to price changes, and LSMA filters false breakthroughs.
2. When the fast EMA crosses above the slow LSMA, it is judged that it is still in the bullish trend, and you can go long; on the contrary, when the fast EMA crosses below the slow LSMA, it is judged that you are entering a short position, and you can close the position at this time.
3. After entering the long position, the dynamic stop loss calculated using the ATR indicator is continuously adjusted to effectively track the rising trend of BTC. Specifically, the initial point of the stop loss line is the entry price, and each subsequent adjustment will slide upward by a fixed proportion of the ATR range.
4. The stop loss line can effectively lock in the floating profits caused by the rise of BTC, and at the same time prevent the stop loss point from being too close to the latest price, resulting in frequent stop losses. In addition, the strategy also sets two moving take-profits of different proportions to lock in more profits.
### Advantage Analysis
1. Using double moving averages to judge trends is more reliable and can effectively prevent false signals.
2. ATR dynamic tracking stop loss can not only lock in most profits, but also avoid frequent small stop losses.
3. No matter whether the bull market ends or not, as long as the moving average sends an exit signal, the loss will be stopped and the risk will be controlled in place.
4. High degree of automation, no manual intervention required, easy to run for a long time.
### Risk Analysis
1. It is still necessary to pay attention to sudden and major news to avoid huge slippage losses.
2. Although the combination of double moving averages can reduce false signals, it is difficult to completely avoid it in a volatile market.
3. Improper setting of ATR parameters will also affect the stop loss effect and needs to be adjusted according to different varieties.
4. Unreasonable moving average period or failure to update in time will also cause signal lag.
5. Ensure server stability and avoid automatic transaction interruption caused by abnormal downtime.
### Optimization direction
1. You can try to add more indicators to judge the trend, such as Bollinger Bands. Or use machine learning models to predict prices.
2. The calculation method of ATR dynamic stop loss can also be adjusted and optimized to make the stop loss smoother.
3. An alarm mechanism based on trading volume and intraday rotation FEATURE can be added to prevent the impact of major news.
4. Different currencies have different parameters, and more historical data can be used to train personalized parameters.
### Summarize
Overall, this strategy is a very practical BTC automatic investment program. It is very reliable to use double EMA to judge the general trend, coupled with ATR tracking stop loss, you can not only get good profits, but also have a long validity period. With the continuous optimization and adjustment of parameters, the effect of this strategy still has room for improvement, and it is very worthy of real-time verification.
||
