...
>Currently, only the Ouyi API interface and BTCUSDT perpetual contract trading are supported. Quantitative trading pursues stable returns, so please do not set high leverage. It is recommended that the leverage multiple be 5 times or less.
- The Windows version is the interface version. After downloading, first configure the exchange API through set_api, and then run the btcusdt program to start automatic trading;
- The Python source code interface version can run on any desktop operating system. Just install and configure the Python environment. It is recommended to use Python3.7.7 version. You need to install the requests library. The usage method is similar to the Windows version for the interface version;
- The Python source code command line version can run on any operating system. Just install and configure the Python environment. It is recommended to use Python3.7.7 version. You need to install the requests library and configure the exchange API by manually modifying the `okex_api.json` file. Under CentOS, the execution body can be automatically executed in the background through the following command:
```
nohup python3 start.py &
```
Terminate the process and stop trading with the following command:
```
...
### API Notes:
- If you are not running on a cloud host with a fixed IP, please do not set a bound IP, otherwise it will not work;
...
- The flag in `okex_api.json` is the trading order option, 0 is the real order, and 1 is the simulated order;
### FAQ
##### Stuck after starting transaction
- The API settings are incorrect. Please check whether the API is configured correctly. The APIs of real disks and simulated disks are different and need to be set separately;
- The domestic network cannot access the exchange. Please use an overseas cloud host to run the execution body. It is recommended to use a Hong Kong cloud host;
- Please do not use circumvention software to open the executor in China. Due to compatibility issues, there is a high probability that the software will not run if you use circumvention software;
