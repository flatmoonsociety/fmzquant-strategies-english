...
Trend-Following-Strategy-Based-on-MA-Candles-and-Supertrend Trend-Following-Strategy-Based-on-MA-Candles-and-Supertrend
> Author
...
[trans]
The name of this strategy is "Trend following strategy based on moving average K-line and super trend stop loss". This strategy uses moving averages to draw trend K lines, and combines the super trend stop loss mechanism to generate trading signals to track the trend.
Specifically, the trading logic of the strategy is as follows:
1. Use the moving average to calculate the opening high and low closing prices and draw the trend K line.
2. Apply the supertrend stop loss technique on the trend K line to obtain the stop loss levels for long and short positions.
3. When the price breaks through the long stop loss level, a buy signal is generated; when the price falls below the short stop loss level, a sell signal is generated.
4. Combine the judgment of the highest price and lowest price during the year in a higher time period to avoid generating too many invalid signals in a volatile market.
5. When the overtrend stop loss reverses, close the position and stop the loss.
The advantage of this strategy is that it integrates multiple technical indicators at the same time and improves the accuracy of judgment. However, the parameters of the moving average and supertrend stop loss need to be optimized. Stop loss strategies are also essential.
Generally speaking, the integrated application of indicators and models can make up for the shortcomings of a single indicator to a certain extent, but no strategy can be perfect. Traders still need to remain flexible enough to respond to market changes.
