...
Trading strategy based on KST indicator and EMA indicator KST-EMA-Momentum-Trend-Following-Strategy
> Author
...
![IMG](assets/images/f95999b0efcfbc56bace282d3f22f5226e229b9a15f703dba39adc8d75d07b2d.png)
[trans]

## Overview
The core idea of ​​this strategy is to combine the KST indicator and EMA moving average to judge and follow the trend. Buy when the KST indicator appears a golden cross and is lower than 0, and sell when a dead cross appears and is higher than 0. At the same time, combined with the EMA moving average as support and resistance, a trading signal will be issued only when the closing price breaks through the EMA moving average. This strategy is simple and practical, can automatically track trends, and is suitable for medium and long-term positions.
## Strategy Principle
1. Calculate the KST indicator: Calculate the ROC indicators on the 10th, 15th, 20th, and 30th respectively, then weight and sum them respectively, and finally obtain the KST indicator through a 9-day SMA smoothing.
2. Calculate the EMA moving average: Calculate the EMA moving average with a length of 50.
3. Generate a buy signal: When the fast line of the KST indicator crosses the slow line (gold cross) and is lower than 0, and the closing price is higher than the EMA moving average, a buy signal is generated.
4. Generate a sell signal: When the fast line of the KST indicator crosses the slow line (die cross) and is higher than 0, and the closing price is lower than the EMA moving average, a sell signal is generated.
5. Set trailing stop loss: Trailing stop loss is set to 1% of the account value to achieve automatic stop loss.
## Strategic Advantages
1. The KST indicator can identify trend changes, and the EMA moving average can confirm the trend direction. The combination of the two can accurately determine the timing of ENTRY.
2. Use fast and slow crosses combined with the 0-axis to determine the direction of the KST indicator to avoid unnecessary transactions.
3. The EMA moving average serves as support and resistance to further filter out false signals and only enter the market when the EMA is exceeded.
4. Automatically track stop loss to control risk and let profits run.
5. The strategy has fewer parameters and is easy to implement and optimize.
## Strategy Risk
1. The KST indicator lags behind in judging trend changes and may miss some opportunities. The calculation cycle can be shortened or the weighting method can be optimized.
2. The EMA moving average has hysteresis and may fail at the turning point of the trend. You can experiment with other indicators or multiple moving average combinations.
3. If the stop loss setting is too loose, the loss will expand; if the stop loss is too tight, the loss will be stopped due to large fluctuations overnight. Careful testing is required to find the balance.
4. Strategy signals are frequent and transaction costs may be high. Entry conditions can be appropriately relaxed and the number of transactions can be reduced.
## Strategy optimization direction
1. Optimize the calculation period parameters of the KST indicator and find a parameter combination that is more sensitive to specific varieties.
2. Test different moving average indicators or combinations, such as MA, WMA, etc., to see which one works better with KST.
3. Try to dynamically adjust your stop loss based on volatility or ATR.
4. Add filtering conditions, such as sudden increase in transaction volume, to avoid being trapped.
5. Consider combining with other indicators, such as RSI, MACD, etc., to make the strategy more comprehensive.
6. Test the parameter effects of different varieties and formulate optimization plans suitable for different varieties.
## Summarize
The overall idea of ​​this strategy is clear, reliable, and easy to implement. It uses the KST indicator to determine trend turning points, EMA moving averages to further filter, stop loss to control risks, and can automatically track medium and long-term trends. The parameter selection is reasonable and the optimization space is large. Users can adjust the parameters according to needs. It is suitable for different varieties and has good universality. This strategy is suitable for novices to learn and can also provide direction for professional traders. Through continued optimization and testing, this strategy is expected to become a stable and reliable trend following strategy.
||
