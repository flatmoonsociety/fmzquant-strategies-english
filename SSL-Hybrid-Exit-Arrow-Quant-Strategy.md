...
Daily Exit Arrow Quant Strategy SSL-Hybrid-Exit-Arrow-Quant-Strategy
> Author
...
![IMG](assets/images/9cc65cf686a6274975350f49586f38079d0e501b19a29728c35e11b1cced1ebb.png)
[trans]

## Overview
The daily winding distraction line quantitative strategy is a short-term quantitative trading strategy based on moving averages and maximum and minimum price indicators. It uses the EXIT arrow of the SSL hybrid indicator to determine buying and selling points, cooperates with the QQE indicator for filtering, and uses the ATR indicator to calculate stop loss levels and increase positions in batches. This strategy is suitable for investors who are sensitive to market fluctuations and have strict risk control.
## Strategy Principle
This strategy uses the EXIT arrow of the SSL hybrid indicator to determine buy and sell entry points. The top of the EXIT arrow is the EXIT high point, and the bottom is the EXIT low point. A sell signal is generated when the closing price crosses downward from the EXIT high, and a buy signal is generated when the closing price crosses upward from the EXIT low.
In order to improve the reliability of the signal, this strategy introduces the QQE indicator as an auxiliary filtering condition. The signal generated by the EXIT arrow will only be executed when the QQE indicator is in the same direction.
In order to control risks, this strategy uses multiple ATR indicators to calculate stop loss levels and increase positions in batches. The short stop loss is the closing price + ATR × 1.8, and the long stop loss is the closing price - ATR × 1.8. Add positions in three batches. The amount of each batch is 10% of the initial amount. The positions added are closing price - ATR × 0.1, closing price - ATR × 0.3 and closing price - ATR × 0.7.
A separate stop loss is set for each batch of positions added. 20% of the first batch of positions will be stopped when it reaches the stop loss level, and the remaining positions will continue to be held.
## Strategic Advantages
1. Make profits through EXIT arrows, stop losses in time, and effectively control risks
2. QQE indicator filtering to improve signal accuracy
3. Use the ATR indicator to calculate stop loss and increase positions based on market fluctuations, making risk control more accurate.
4. Add positions in batches and fully grasp the trend to make profits
## Strategy Risk
1. Reaching a partial stop loss on a profitable position may expose the remaining positions to the risk of continued stop loss. You can consider taking profit as a whole or taking profit based on the fundamentals of the stock itself.
2. The EXIT arrow and the QQE indicator have different sensitivities to market fluctuations and may produce conflicting signals. Parameters should be adjusted to reduce signal conflicts.
3. Adding positions too aggressively may lead to chasing highs and selling lows. We should assess the situation and reduce the leverage level.
## Optimization direction
1. Take profits based on the fundamental indicators of the stock itself, such as setting reasonable take-profit levels for book value ratio, price-to-earnings ratio, dividend rate, etc.
2. Adjust the parameters of the QQE indicator to make it consistent with the signal generated by the EXIT arrow.
3. Reduce the proportion of positions added according to the market heat, and reduce the position added during volatile market conditions.
4. Test the best parameter combination based on indicators such as maximum retracement and profit-loss ratio.
## Summary
This strategy uses the EXIT arrow of the SSL hybrid indicator as the core of the signal, and uses the QQE indicator and the ATR indicator for filtering and stop loss. Amplify profits by adding positions in batches. It is a short-term quantitative strategy suitable for tracking short-term market trends. This strategy has retracement control and risk control capabilities, but it also needs to pay attention to preventing signal conflicts, chasing highs and selling lows, and other risks. If the profit-taking method can be combined with stock fundamentals and be more prudent when judging market fluctuations and adjusting the proportion of positions, the profit margin of this strategy will be greater.
|| 
