...
![IMG](assets/images/f7d564d2103b081d1ef6d348c41bcd291c8195ebfd9f57d5a763910c1324dad8.png)
[trans]

## Overview
This strategy uses a combination of the Movement Index (DMI) and the Hull Moving Average (HMA). It uses the DMI to determine the market direction and the HMA to confirm the strength of the trend to achieve risk-free managed transactions.
## Strategy Principle
1. Calculate the true range (True Range), bull trend indicator (DIPlus), short trend indicator (DIMinus) and average directional index (ADX).
2. Calculate the fast Hull average (fasthull) and the slow Hull average (slowhull).
3. Trigger long conditions: DIPlus crosses DIMinus and fasthull crosses slowhull.
4. Short selling conditions are triggered: DIMinus crosses DIPlus and fasthull crosses slowhull.
5. Send long and short signals respectively after meeting the long and short conditions.
## Advantage Analysis
This strategy combines the double confirmation of the trend judgment indicator DMI and Hull moving average, which can effectively identify the market trend direction and avoid the recurrence of long and short markets. Risk-free management reduces the frequency of transactions and the overall profitability is good in the long run.
## Risk Analysis
The biggest risk of this strategy is that there is no stop loss setting, and the loss cannot be effectively controlled when the market fluctuates violently. In addition, the parameter optimization space is limited and the lack of pertinence is also a major shortcoming.
Risks can be reduced by adding moving stops and optimizing parameter combinations.
## Optimization direction
1. Add ATR stop loss and use real amplitude trailing stop loss.
2. Optimize Hull cycle parameters and find the best combination.
3. Dynamically adjust the parameter thresholds for long and short positions.
4. Add filters such as volume and energy indicators to ensure that the trend continues.
## Summarize
The combined strategy of DMI and HMA is accurate in judgment, simple and effective, and suitable for medium and long-term operations. After adding appropriate stop loss and parameter optimization, it can become an excellent trend following system.
||
