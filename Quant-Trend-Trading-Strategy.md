...
[trans]
This strategy builds long-term and short-term trading signals by integrating multiple indicators including moving averages, range filters, ADX, parabolic stop loss, volume price rancesar, MACD and trading volume. Specifically, the strategy will simultaneously monitor the fast line JMA, midline MACD, slow line ADX, volume price indicator RSI, stop loss indicator SAR and interval filter. Only when the trend indicators on multiple fast, medium and slow time axes all send consistent signals will a trading decision be made. This can filter out some false signals and avoid unnecessary wrong transactions.
Entry signal conditions include:
1. The fast JMA rises and the midline MACD is greater than 0
2. The +DI line on the slow line ADX exceeds the -DI line, indicating that it is currently in a trend state.
3. SAR crosses the price below, giving a bullish signal
4. RSI is above the mid-range, indicating that it is currently overbought.
5. The price of interval filters breaks through the upper track
6. Trading volume increased significantly
As long as the above conditions are met at the same time, it means that the general trend has entered a bullish state. At this time, the strategy will choose the appropriate time to establish multiple orders to follow the trend.
The take profit condition will also monitor multiple indicators at the same time, including moving take profit, shrinking take profit, time take profit, etc. This ensures the smooth progress of the trailing take profit and avoids leaving the market too early or taking profit too late.
Overall, this quantitative strategy integrating multiple indicators can fully explore trend opportunities in the market and greatly improve the efficiency of capital allocation. At the same time, the space for optimization of strategy parameters is also very large. Users can adjust the parameter combination according to their own needs to obtain a more reliable trading system.
Of course, there are certain risks with this type of trend following strategy. Specifically, they mainly include:
1. Retracement risk: A certain degree of retracement may occur during the tracking process, which requires mental endurance
2. Shock risk: In a volatile market, multiple small stop losses may occur, reducing profitability.
3. Breakthrough failure risk: The trend breakthrough signal being followed may be a false breakthrough, and it is necessary to stop the loss in time to control the loss.
4. Parameter risk: Improper parameter setting may also lead to unsatisfactory strategy effects.
5. Over-optimization risk: Over-optimization based on historical data may result in parameters that are not universal.
6. Transaction cost risk: When the number of transactions is large, handling fees will also have a certain impact on profits.

Generally speaking, this quantitative trend following strategy can achieve a stable and efficient trading system through parameter adjustment, and is worth recommending. However, users should also pay attention to controlling transaction risks and make appropriate optimizations during use to obtain long-term stable returns.
||
