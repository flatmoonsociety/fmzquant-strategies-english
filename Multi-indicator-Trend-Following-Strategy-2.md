...
![IMG](assets/images/fb7ef401b27d9cb0aa9ba62b5252a8f1dbcb4a7d4497c9182f4a986796fcb765.png)
[trans]

## Overview
This strategy realizes multi-timeline trend judgment by combining 3 open source indicators, and sets stop loss and profit to lock in profits. Specifically, the strategy uses the AK MACD BB indicator to determine the short-term trend direction, the SSL indicator to filter out some false signals, and finally combines the trading volume indicator VSF to determine the true strength of buying and selling orders, thereby determining the timing of entry. At the same time, the strategy presets stop-loss and stop-profit points to lock in profits, which can significantly reduce the risk of loss in a single transaction.
## Strategy Principle
1. AK MACD BB indicator
This indicator applies Bollinger Bands to the MACD indicator. A buy signal is generated when the MACD indicator line breaks through the Bollinger Band upper track, and a sell signal is generated when the MACD indicator line breaks through the lower track.
2. SSL indicators
The SSL indicator determines whether the price breaks through the moving average and detects retest signals. When the price crosses above the moving average and the SSL indicator is blue, it is an upward trend. When the price crosses below the moving average and the SSL indicator is red, it is a downward trend and a trading signal is issued.
3. VSF indicator
The VSF indicator determines the strength of buyers and sellers. The strategy only sends a signal when the buyer's power or the seller's power is greater than 50% to avoid invalid breakthroughs.
4. Stop loss and take profit
The strategy contains 4 levels of progressive take profit, with profit intervals set from 1.5 times to 3 times. At the same time, set a 2% fixed stop loss to effectively control the maximum loss of a single transaction.
## Advantage Analysis
1. Multi-indicator combination, accurate judgment
Judging multi-time axis trends through different indicators can filter out false signals and make judgments more accurate.
2. Automatically stop profits and losses, and the risks are controllable
The strategy has built-in stop-profit and stop-loss settings, which can control the loss of a single transaction to about 2% and avoid huge losses.
3. Excellent backtest data
According to the publisher's backtest, among 100 transactions, 74% of the profitable transactions were profitable, and 427% of the total profits were made.
## Risk and Countermeasure Analysis
1. Risk of severe market fluctuations
When oscillating in a large range, multiple small losses may occur. At this time, you can adjust the fixed stop loss range or suspend trading.
2. Restricted risks for long and short positions
The current strategy can be long or short. If the trade is restricted to only long or short positions, the chance of not being able to make a profit will be reduced by half.
3. Trading session risks
The strategy uses 5 minutes of data for judgment. If there are only a few hours of data in a trading day, the sample size is insufficient and the signal may be unreliable.
## Strategy optimization direction
1. Optimize stop loss and take profit parameters
You can test different stop loss and take profit levels to find the optimal parameters. If the stop loss is too small, the risk cannot be effectively controlled; if the stop loss is too large, you may miss greater profits.
2. Add automatic position adjustment
Trailing stop or trailing stop can be set to lock in profits. Or add positions based on specific conditions to make more profits.
3. Combined with other indicators
You can test combinations of different indicators to determine which indicator combinations work best. You can also add more indicators for cross-validation.
4. Parameter optimization
Backtesting can be conducted through different parameters to find the direction of parameter optimization. In this strategy, changing the Bollinger Band parameters or moving average parameters may produce better results.

## Summarize
This strategy integrates multiple indicators to determine the trend direction and sets automatic stop-profit and stop-loss, which can make profits in a strong trend and control the loss of a single transaction to a very small range. Judging from the publisher's backtest data, its profitability and profitability are very ideal. Through certain optimization, it is expected to further improve the stability and profitability of the strategy.
||
