...
![IMG](assets/images/33a12ce58e42f3fc2d2c2f8b1292636a42a7f0265316d5cd049466a096efed9d.png)
[trans]

## Overview
This strategy calculates the highest and lowest prices within a certain period of time recently and combines them with the current price to form a dynamic central axis. A red descending channel and a green ascending channel are then generated based on recent volatility. These three channel lines form a tradable range. When the price is close to the channel boundary, perform reverse operations with the goal of returning to the central axis to make a profit. At the same time, the strategy contains a trend calculation, which is used to filter out transactions that do not follow the trend and prevent them from being destroyed by the general trend.
## Strategy Principle
1. Calculate the highest price and lowest price in the recent N periods, and combine it with the current closing price to form a dynamic central axis
2. Generate dynamic channel bands based on ATR and multipliers, and the bandwidth changes with market volatility
3. Go long when the price rebounds from the lower channel line, and go short when it rebounds from the upper channel line.
4. There is take-profit and stop-loss logic, and the target returns to the central axis to stop profit.
5. Calculate the trend index at the same time to filter out transactions that do not follow the trend.
## Advantage Analysis
1. The position of the channel line changes dynamically, which can capture market volatility in real time.
2. The probability of trading with the trend is higher, which is conducive to grasping the trend.
3. There is stop loss logic to control single loss
## Risk Analysis
1. Improper parameter optimization may lead to over-trading
2. Counter-trend transactions cannot be completely filtered out under the general trend
3. Unilateral breakthrough channel line may continue to run
## Optimization direction
1. Adjust the parameters of the channel line to make it more consistent with the characteristics of different varieties
2. Adjust the trend index parameters to increase the probability of following the trend.
3. Add machine learning elements to dynamically optimize parameters
## Summarize
This strategy mainly relies on the volatile characteristics of the market to make profits. By capturing price reversal points through dynamic channels and combining with trend filtering, you can effectively use reversal trading to make profits while controlling risks. The key is to adjust the parameters so that the channel line can track the price in real time without being too sensitive. At the same time, the trend index also needs to select an appropriate period to play a filtering role. This strategy theoretically follows the trend and has a stop loss. In practical application, good returns can be obtained through parameter optimization.
||
