...
[trans]
The name of this strategy is "Configurable Two-way Super Trend Strategy". This strategy uses the super trend stop loss method to identify price trends, and can configure the parameters for long and short respectively to achieve accurate trend following transactions.
The calculation method of super trend stop loss is: multiply the ATR value by a coefficients to construct a price channel. The upper edge of the channel is the long stop loss line, and the lower edge of the channel is the short stop loss line. Trading signals are generated when price breaks out of the channel.
The innovation of this strategy is that the parameters for long and short can be configured independently:
1. Super trend parameters such as ATR period and ATR coefficients can be set separately;
2. The maximum holding period can also be configured separately to adjust the profit target;
3. The stop loss method (fixed percentage stop loss or ATR trailing stop loss) can also be set differently.
This allows the strategy to be long-only, short-only or two-way trading, better suited to the needs of specific market conditions.
The advantage of this strategy is that the judgment of super trend stop loss is simple and intuitive, and the configurable parameter combinations are rich. However, the supertrend itself is easy to be broken through and requires auxiliary judgment. Parameter optimization is also critical.
In short, the configurable two-way super trend strategy improves the accuracy of trend trading, but the core idea is still a simple parameter combination, which is easy to apply in real markets.
