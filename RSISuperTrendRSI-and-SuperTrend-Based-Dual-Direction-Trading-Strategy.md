...
RSI-and-SuperTrend-Based-Dual-Direction-Trading-Strategy based on RSI and SuperTrend
> Author
...
![IMG](assets/images/e8604221a1763c7c169c2f0fbb1e2573fbe7e8cefd1d74e14870b4a60fa6b77e.png)

[trans]

## Overview
This is a two-way trading strategy that comprehensively uses the RSI indicator and the SuperTrend indicator. This strategy aims to identify the strengths and weaknesses of the market and timely swap positions when the trend direction changes to obtain higher returns.
## Strategy Principle
This strategy is mainly based on the following principles:
1. Use the RSI indicator to determine the strength or weakness of the current market. When the RSI is above 50, it is a strong market, and when it is below 50, it is a weak market.
2. Use the SuperTrend indicator as a trend filter. A trading signal will only be issued when the price breaks through SuperTrend.
3. When the RSI indicator sends a strong signal, if the price breaks through the upper track, go long; if the price falls below the lower track, close the position.
4. When the RSI indicator sends a weak signal, if the price breaks through the lower track, go short; if the price breaks through the upper track, close the position.
5. Use the long-short conversion of the RSI indicator to capture trend change points and perform position swap operations in a timely manner.
### Specific implementation
1. Calculate the RSI indicator value, the length is 14, and use 50 as the limit to judge the strength and weakness.
2. Calculate the SuperTrend indicator, the length is 10, and the multiplier is 2.
3. When the RSI is above 50 and the price breaks through the SuperTrend upper rail, go long. When the RSI is below 50 and the price falls below the lower SuperTrend band, go short.
4. When you are long, if the RSI weakens and the price falls below the SuperTrend upper track, close the position. When shorted, if the RSI becomes stronger and the price breaks through the SuperTrend lower track, the position will be closed.
5. Configurable to only do long or only short.
## Strategic Advantages
This strategy combines trend following and overbought and oversold judgment, and has the following advantages:
1. It can capture trend changes in time and reduce unnecessary opening of positions.
2. The RSI indicator can effectively determine overbought and oversold areas to avoid chasing highs and selling lows before the market turns around.
3. SuperTrend can better filter out the noise in the market and track the medium and long-term trends.
4. Combining the two indicators RSI and SuperTrend can improve the stability of the strategy.
5. The parameter optimization space of this strategy is large and can be adjusted according to different varieties and cycles.
6. Supports long-only or short-only mode and can flexibly respond to different market types.
## Strategy Risk
There are also some risks with this strategy:
1. The RSI indicator is prone to generate false signals and needs to be verified in conjunction with the price entity.
2. Improper setting of SuperTrend indicator parameters may lead to missing points or chasing highs and killing lows.
3. There is a risk of divergence in the dual-index combination, and parameters need to be adjusted to achieve the best match.
4. When the market changes drastically, the stop loss may be taken out instantly, so the stop loss position needs to be set appropriately.
5. It is necessary to avoid opening reverse positions near important support and resistance areas.
## Strategy optimization
This strategy can also be optimized from the following aspects:
1. Adjust RSI parameters and find the optimal length to filter out false signals.
2. Optimize SuperTrend parameters to improve the trend tracking effect.
3. Test parameter combinations of different varieties and different periods to find optimal parameters.
4. Add other indicator filters, such as MACD, KDJ, etc., to improve signal accuracy.
5. Add important support and resistance levels, Bollinger Bands, moving averages and other judgments to provide qualitative strategy signals.
6. Optimize the stop loss strategy and try to reduce the probability of the stop loss being taken out immediately while ensuring the effectiveness of the stop loss.
## Summarize
This strategy integrates the advantages of the two indicators RSI and SuperTrend, which can effectively identify changes in the mid-term market trend and perform switching operations between bear and bull markets. Through parameter optimization, it can adapt to a wider range of market conditions. But you also need to pay attention to some common problems, such as false signals, ParameterSet, etc. Overall, the strategy is clear, easy to implement, and highly practical.
||
