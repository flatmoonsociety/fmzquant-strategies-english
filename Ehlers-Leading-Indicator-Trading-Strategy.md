...
![IMG](assets/images/4f532bf03a6f95827ce122583e364cb40960812e03d2bd0802dd9a39dd19c89d.png)

[trans]


## Overview
This strategy is based on the ideas of technical analysis master John Ehlers and uses the Ehlers leading indicator to judge the historical cycle of prices and issue buy and sell signals. The strategy combines the detrending synthetic price and the Ehlers leading indicator, and uses the indicator line to cross the detrending synthetic price to generate trading signals.
## Strategy Principle
The strategy first calculates the Detrended Synthetic Price (DSP). DSP subtracts the values ​​of a third-order Butterworth filter and a second-order Butterworth filter to obtain a function that is synchronized with the real price dominant cycle.
Then calculate the Ehlers Leading Indicator (ELI). The ELI is obtained by subtracting the simple moving average of the detrended synthetic price and the detrended synthetic price. It can signal the turning point of the cycle in advance.
Finally, buy and sell signals are generated when Ehlers leads the indicator line across the detrended synthetic price. If the ELI crosses the DSP above, a buy signal is generated; if the ELI crosses below the DSP, a sell signal is generated.
## Advantage Analysis
The biggest advantage of this strategy is to use the Ehlers leading indicator to determine the turning point of the price trend in advance, so that you can open a position before the price begins to reverse, thereby obtaining higher profit margins.
In addition, this strategy combines detrended prices to judge trading signals, which can filter out irrelevant low-frequency information in prices, allowing the strategy to focus more on the cyclical laws of prices and not be disturbed by short-term market noise.
## Risk and Optimization
The main risk of this strategy is that the Ehlers leading indicator may misidentify signals, resulting in losses from early opening of positions. The sensitivity of the indicator can be optimized by adjusting the indicator parameters.
In addition, traders need to pay attention to that this strategy is only applicable to varieties with obvious cyclical patterns, and the effect will be reduced for varieties with chaotic price trends. It is recommended to examine the cyclical regularity of the variety before deciding whether to use this strategy.
Risks can be controlled by combining other indicators for confirmation or adjusting position management strategies. For example, setting a stop loss line, or reducing the size of a single transaction, etc.
## Summarize
This strategy uses the Ehlers leading indicator to determine the cyclicality of prices and establish a position before the price starts a new cycle. It is a typical trend following strategy. This strategy works well for cyclical varieties, but there is also a certain risk of false signals. Through parameter tuning and risk management, the strategy can be made more stable and reliable.
||
