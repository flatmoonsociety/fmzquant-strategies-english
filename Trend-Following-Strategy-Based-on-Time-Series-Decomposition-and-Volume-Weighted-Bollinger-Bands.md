...
## Strategy Principle
1. Use time series decomposition to remove noise and periodicity in prices and obtain more accurate trend judgments;
2. Based on the trend line, calculate the new price after volume weighting;
3. Calculate the Bollinger Band percentage width BB%B indicator of the closing price to determine overbought and oversold;
4. Calculate the Bollinger Bands percentage width of the change amount Delta (OBV-PVT) of OBV-PVT as the criterion for judging the deviation between volume and price;
5. Generate trading signals based on the long-short crossover of the volume and price indicators and the overshoot retracement of the Bollinger Bands indicator.
## Advantage Analysis
1. It combines multiple judgments of price, trading volume and statistical characteristics, making the strategy robust;
2. The combination of BB%B and Delta (OBV-PVT) can better judge short-term overbought and oversold phenomena;
3. The volume-price cross signal filters out some noisy trading points.
## Risk Analysis
1. The parameter settings are too complex and difficult to adjust;
2. Short-term range fluctuations may increase losses;
3. The divergence between volume and price cannot completely filter out misleading signals.
The strategy can be optimized by adjusting the moving average period, Bollinger Band range and risk-to-profit-loss ratio, reducing the frequency of transactions while increasing the profit-to-loss ratio for a single transaction.
## Summarize
This strategy comprehensively uses a variety of analysis tools such as time series decomposition, Bollinger Band indicators, OBV indicators, etc., and realizes the identification of short-term aftershocks through the organic combination of volume-price relationships, statistical characteristics, and trend judgment, and can effectively capture the main trends of the market. At the same time, there are certain risks, and parameters need to be adjusted to achieve the optimal state.
|| 
