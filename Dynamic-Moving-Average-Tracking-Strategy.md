...
![IMG](assets/images/40a7d95c9ac58ad0900135c1c2225ccc404fb0b6e79c2f35871e6a15e4cc2b81.png)
[trans]

### Overview
This strategy follows the strategy explained by Larry Williams in his book Long-Term Secrets of Short-Term Trading, which uses two 3-period moving averages, one representing the highs and the other the lows. When the price is below the 3-period low moving average, we have a long position signal. This trade is closed when the price closes above the 3 period high moving average.
### Strategy Principles
The core logic of this strategy is to calculate a 3-period moving average of high and low points. Specifically, it uses the ta.ema function to calculate an exponential moving average of the highs and lows of the last 3 bars to generate dynamic support and resistance levels. When the price falls below the low moving average, it indicates that it is currently in a downward trend, so we can go long; when the price rises back above the high moving average, it means that the upward trend has ended, and we have to close the position. In this way, the strategy can dynamically track price changes and enable buying low and selling high.
### Advantage Analysis
The biggest advantage of this strategy is its simplicity and dynamic nature. Compared with the traditional moving average of high and low points in a certain period, this strategy uses a continuously calculated short-term moving average, which can capture price changes more sensitively and timely. This allows it to quickly identify buying and selling points to enter and exit the market. In addition, the small amount of calculation is another advantage, which helps reduce transaction delays.
### Risks and Solutions
The main risk with this strategy is its slow response to unexpected events such as major news events. Because its moving average period is very short, it takes a certain amount of time to adjust the position of the moving average when there are severe price fluctuations. This may result in losses or missed opportunities. In addition, being too sensitive may also result in wrong transactions. To reduce these risks, we can appropriately extend the number of periods of the moving average, or add filtering conditions to avoid false signals.
### Optimization direction
This strategy still has a lot of room for optimization. First, we can combine filtering with other indicators such as oscillators to make the signal more reliable. Secondly, stop loss logic can also be added to control risks. Furthermore, we can also dynamically adjust the moving average parameters according to market conditions, extending the cycle in trending markets and shortening the cycle in volatile markets. In addition, methods such as multi-time frame analysis and machine learning to identify patterns can be used to improve strategy performance.
### Summarize
Overall, this strategy is very simple and practical. It uses short-term moving averages of high and low points to judge the trend. Its advantages are strong dynamics, small amount of calculation, high real-time performance, and suitable for frequent transactions. However, there are also problems such as insensitivity to emergencies and high signal error rate. These problems all have directions for improvement and optimization. Through parameter adjustment, filtering conditions, pattern recognition and other means, the effect of this strategy can be further improved.
||
