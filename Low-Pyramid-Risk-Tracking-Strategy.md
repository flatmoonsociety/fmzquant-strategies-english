...
Low-Pyramid-Risk-Tracking-Strategy Low-Pyramid-Risk-Tracking-Strategy
> Author
...
## Strategy Overview
This strategy first uses the difference between RSI and EMA RSI to identify potential lows in price. To filter out false signals, the strategy also incorporates moving averages and multi-timeframe stochastics for confirmation. Once the low signal is confirmed, long orders will be gradually established slightly lower than the point. This is the idea of ​​tracking the pyramid. The strategy allows up to 12 tracking orders to be opened, and the quantity of each order will increase sequentially, which can effectively spread risks. All orders will be exited following an overall stop loss level, and it also allows you to set a separate take profit for each order. To further control risk, this strategy also sets an overall stop loss based on the percentage of account equity.
## Strategy Principle
The strategy mainly consists of three parts: low point identification module, pyramid tracking module and risk control module.
**Low Point Identification Module** uses the difference between the RSI indicator and its EMA to identify potential lows in price. To improve accuracy, moving average indicators and multi-time frame stochastic indicators are also introduced for signal filtering. Only when the price is below the moving average and the stochastic K-line is below 30, the validity of the low signal will be confirmed.
The **Pyramid Tracking Module** is the core of this strategy. Once the low signal is confirmed, the strategy will open the first order 0.1% below the low. After that, as long as the price continues to fall and is lower than the average entry price by a certain percentage, more orders will continue to be added. The quantity of new orders will increase sequentially. For example, the quantity of the third order is 3 times that of the first order. This pyramid tracking method averages risk. This strategy allows up to 12 tracking orders to be opened.
**Risk Control Module** mainly includes three aspects. The first is the overall stop loss, which is the stop loss level calculated based on the highest price in the most recent period. All orders will follow this stop loss level and stop loss at the same time. The second is the independent take-profit setting for each order, which allows the take-profit to be taken according to a certain proportion of the entry price. The third is the overall stop loss based on the account equity ratio, which is the strongest risk control method.
## Strategic Advantages
- Use pyramid tracking to reduce risk on individual orders while diversifying overall risk
- Multi-indicator combination improves the accuracy of low point identification
- The overall stop loss, take profit and trailing stop functions can effectively control risks
- Equity proportional stop-loss mechanism protects accounts from significant losses
- You can find the balance point between risk and return by adjusting parameters
## Strategy Risk
- The accuracy of low point identification still has certain limitations, and you may miss the best entry point or enter a false signal
- When placing additional orders, you may face adverse market conditions and increase losses.
- A longer running cycle is required to reflect the advantages of the strategy
- Improper parameter settings may lead to insufficient risk control
In order to reduce the above risks, the following aspects can be optimized:
1. Replace or add indicators to improve the accuracy of low point identification
2. Optimize order quantity, interval, take-profit range and other parameters to reduce the risk of a single order
3. Appropriately shorten the stop loss range to protect profits
4. Test different varieties and choose those with good liquidity and greater volatility.
## Strategy optimization direction
There is still room for further optimization of this strategy:
1. Try to introduce more advanced technologies such as machine learning to identify low points
2. Dynamically adjust order quantity, stop loss range and other parameters according to market conditions
3. Add a stop-loss strategy within the box to avoid expanding losses
4. Add re-entry mechanism
5. Optimize strategy parameters for stocks and digital currencies
## Summarize
This strategy effectively reduces the risk of a single order through the idea of ​​pyramid tracking, and functions such as overall stop loss, take profit, and trailing stop loss also play a very good role in risk control. However, there is still room for optimization in aspects such as low point identification. If more advanced technology can be introduced, dynamic parameter adjustment functions can be added, and coupled with parameter optimization, the benefit-risk ratio of this strategy will be greatly improved.
||
