...
## Strategy Principle
- The Hull indicator is used to determine the trend direction of value. When the middle rail line (MHULL) is above the lower rail line (LHULL), it indicates an upward trend; otherwise, it indicates a downward trend.
- The LSMA indicator is used to identify trend reversal points. When the LSMA indicator crosses above MHULL, it indicates the formation or acceleration of an upward trend; when the LSMA indicator crosses below MHULL, it indicates the formation or acceleration of a downward trend.
- Combining the two, when the Hull indicator shows an upward trend (MHULL > LHULL) and LSMA crosses above MHULL, go long; when the Hull indicator shows a downward trend (MHULL < LHULL) and LSMA crosses below MHULL, go short.
- Stop loss is set to the latest swing point. The long stop loss is the latest lowest point, and the short stop loss is the latest highest point.
## Advantage Analysis
This strategy has the following advantages:
1. The Hull indicator responds quickly and can catch trend transitions in time; the LSMA has strong smoothness and can identify reversal signals accurately and reliably. The combination of the two works well.
2. Use LSMA crossing to filter out false signals judged by the Hull indicator, reducing the probability of wrong transactions.
3. Use the fluctuation point as the stop loss level to protect the safety of funds to the greatest extent.
4. Suitable for medium and low-frequency trading, can be used in time frames of 1 minute or even lower, and has wide applicability.
## Risk Analysis
There are also some risks with this strategy:
1. In a volatile market, the Hull indicator and LSMA may cross multiple times, resulting in too frequent trading. Parameters should be adjusted appropriately to reduce transaction frequency.
2. Setting the stop loss to a fluctuation point may be triggered by short-term price adjustments, so the stop loss distance should be appropriately widened.
3. Due to the lagging nature of the LSMA indicator, there may be a slight risk of misjudgment. It should be confirmed in combination with other indicators such as K-line patterns.

## Optimization direction
This strategy can be optimized from the following aspects:
1. Optimize the parameters of Hull indicator and LSMA to make their combination more suitable for different varieties and time periods.
2. Add filtering conditions based on volatility, trading volume, etc. to avoid erroneous transactions in volatile market conditions.
3. Add machine learning algorithms to assist in judging trends.
4. Combine with deep learning and other technologies to determine key support and resistance areas to make stop loss more reasonable.
## Summarize
This strategy uses the combined application of Hull indicator and LSMA to judge changes in trend direction and implement trend following transactions. The advantages are simple operation, quick response, and can be widely used in medium and low-frequency quantitative transactions. By further optimizing filtering conditions, auxiliary judgments and stop-loss algorithms, it is expected to achieve better strategic effects.
||
