...
C futures high frequency arbitrage strategy OKEX-Websocket version
> Author
...
## C++ futures high frequency arbitrage strategy OKEX Websocket version
Strategy principles
The principle of the strategy is very simple. OKEX contracts are intertemporally hedged. In terms of position control design, the design is spread grid hedging.
  The strategy defines two contracts, Contract A and Contract B. Different contract codes can be set in the contract for hedging.
  For example, set A contract as a quarterly contract and B contract as a current week contract (you can also set A as a near-term contract and B as a forward contract, and other definitions are the opposite).
  The hedging operation is divided into short contract A (quarterly) and long contract B (similar to short forward contracts in intertemporal arbitrage in commodity futures, long recent contracts, and arbitrage)
  Go long on contract A and short on contract B (similar to shorting the near term in commodity futures, going long on the forward, and doing the opposite)
- ### Design Features
- code language
    The strategy writing code uses C++ language, which has the advantage of fast performance.
- Market driven:
    The market driver uses the OKEX websocket interface to receive market push from the exchange. The latest market data can be obtained in a timely manner. The market data uses real-time tick data with a small amount of data.
    The speed of market response has been greatly improved. For tick data, the strategy specially constructs a K-line generator to synthesize the K-line by calculating the contract price difference based on the acquired tick data.
    The opening and closing of strategic hedging operations are driven by the data generated by the K-line generator class object.
- Position control
    Position control uses a hedging position ratio similar to the "Porfinacci" sequence for control.
    The larger the price difference is, the greater the number of arbitrage hedging will be, and the positions will be diversified, so as to capture small price fluctuations and small positions, and increase the position of large price fluctuations appropriately.
- Close position: stop loss and take profit
    Fixed take profit spread, stop loss spread.
    When the position difference reaches the take-profit position and stop-loss position, take-profit and stop-loss will be executed.
- Market entry and exit cycle design
    The period controlled by the parameter NPeriod provides certain dynamic control over the opening and closing of the strategy.
- Position balancing system, order detection system
    The strategy has a dedicated regular testing and balancing system.
    Order detection system.    
- Strategy expansion
    The strategy code design has a low degree of coupling and can be extended to commodity futures hedging, or further optimized and modified.
- Strategy charts
...
|----|----|----|
|InstrumentA|this_week|Recent Contracts|
|InstrumentB|next_week|Forward Contract|
|DPeriod|30|Difference period (seconds)|
|NPeriod|20|Period|
|LeavePeriod|5|Exit Period|
|AddMax|5|Number of positions added|
|StopLoss|10|Stop Loss Spread|
|StopWin|30|Stop Profit Spread|
|OpenAmount|10|Lot size|
|SlidePrice|true|Sliding price|
|MaxDelay|500|Maximum market delay (milliseconds)|
|IsSetProxy|false|(?Proxy) Whether to set proxy|
