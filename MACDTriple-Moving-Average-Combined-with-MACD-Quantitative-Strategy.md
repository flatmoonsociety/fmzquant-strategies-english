...
Triple-Moving-Average-Combined-with-MACD-Quantitative-Strategy Triple-Moving-Average-Combined-with-MACD-Quantitative-Strategy
> Author
...
## Strategy Principle
This strategy is mainly based on the combined use of triple moving averages and the MACD indicator.
First, the strategy uses triple ponential moving averages with lengths 3, 7, and 2. These three moving averages build a moving average system from fast to slow, which is used to determine the direction of future trends. When the short-term moving average crosses the longer-term moving average, it is a long signal; when the short-term moving average crosses below the longer-term moving average, it is a short signal.
Secondly, the strategy also uses both the MACD indicator with parameters 3 and 7. When the MACD main line crosses the signal line, it is a long signal, and when it crosses below, it is a short signal.
By using dual indicators in combination, multiple false signals caused by a single indicator can be avoided, thereby improving the stability of the strategy.
## Strategic Advantages
1. Use dual indicator filtering to improve signal quality
2. The parameters have been optimized through multiple tests and are stable and reliable.
3. The use of triple moving average system can effectively filter market noise and determine future trends.
4. The MACD indicator parameters are set quickly and can quickly capture short-term opportunities.
## Strategy Risk
1. There is a certain risk of retracement and continuous losses.
2. When there is no obvious trend in the market, this strategy will cause more wrong transactions
3. The MACD indicator is prone to produce false signals and needs to be used in combination with the moving average indicator.
Solution:
1. Adopt appropriate stop loss strategies to control the maximum drawdown
2. When the Market State is obviously trendless, reduce trading frequency
3. Optimize MACD parameters and use them in combination with other indicators
## Strategy optimization direction
1. Test and optimize the parameters of moving averages and MACD to find the best combination
2. Add auxiliary indicators such as KDJ and VRSI to avoid false signals
3. Add a machine learning model to determine the Market State and achieve dynamic adjustment
4. Combine with stop loss strategy and set the best stop loss point
## Summarize
This strategy achieves stable trend capture through the combination of moving averages and MACD. The advantage of its strategy lies in the combination of indicators, which can effectively reduce false signals and achieve better strategic effects. In the next step, the strategy will be further improved through parameter optimization, stop-loss strategy introduction, dynamic adjustment and other means, making it an effective tool for finding medium and long-term opportunities.
||
