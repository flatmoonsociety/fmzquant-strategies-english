...
Dual moving average trading strategy MACD-Dual-Moving-Average-Tracking-Strategy based on MACD indicator
> Author
...
![IMG](assets/images/31b19d4413bb03dd8575ea5f219a52fb0fd96ff789ddf1262f3bbdae65b374a5.png)
 [trans]

## Overview
This strategy is called **MACD Double Moving Average Tracking Strategy**. It uses the double moving average golden cross of the MACD indicator as a trading signal, and combines yesterday's lowest price as a stop loss point to track the short-term movement of the stock price.
## Strategy Principle
1. Calculate fast EMA(close,5), slow EMA(close,8) and signal line SMA(MACD,3)
2. Define the long signal: go long when the fast line crosses the slow line
3. Define short signal: go short when the fast line crosses the slow line or the closing price of the day is lower than yesterday’s lowest price
4. The position is the initial capital of 2,000 US dollars divided by the closing price
5. Use the short signal to close the long stop loss position
## Advantage Analysis
1. Use the MACD indicator to determine the overbought and oversold areas of the market, and use the double moving average to form trading signals to avoid false breakthroughs.
2. Track short-term trends and stop losses in time
3. Dynamic adjustment of positions to avoid excessive losses in a single transaction
## Risk Analysis
1. MACDIndicator lags and may miss short-term opportunities
2. Double moving average trading signals may produce false signals
3. The stop loss point is too aggressive and there is a possibility of excessive frequent stop loss.
## Optimization direction
1. Optimize MACD parameter combination and improve indicator sensitivity
2. Increase trend judgment and avoid false signals generated by volatile markets
3. Combine with Volatility Index to evaluate market volatility and adjust stop loss points
## Summarize
This strategy uses the classic MACD double moving average combination indicator to determine the overbought and oversold ranges and generate trading signals. It also introduces dynamic positions and the stop loss point design of the previous day's lowest price to capture the short-term fluctuation characteristics of the stock price. The overall strategy idea is clear and easy to understand, and it is worthy of further testing and optimization.
||
