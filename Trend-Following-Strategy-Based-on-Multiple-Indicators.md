...
![IMG](assets/images/a9ed5c84bd6a19ebc0f74b14c806cf04fd12319ad5cb56a779225dfbb8d1c914.png)
 [trans]

## Overview
This strategy identifies trends by combining multiple indicators and sets trend-following stops to lock in profits. Mainly use Bollinger Bands, RSI, ADX and other indicators to determine the timing of entry, as well as ATR and Bollinger Bands for stop loss.
## Strategy Principle
The main judgment indicators of the strategy are Bollinger Bands, RSI and ADX. When the price is close to the lower track of the Bollinger Bands and the RSI is lower than 30, it is judged to be oversold, and you go long; when the price is close to the upper track of the Bollinger Bands, and the RSI is higher than 70, it is judged to be overbought, and you go short. In addition, if ADX is higher than 25, it is judged that the trend has formed, and the long and short signals are more effective at this time.
After opening a position, the strategy uses the ATR indicator and the upper and lower Bollinger Bands to stop losses. Specifically, ATR is used for the maximum stop loss range, and the loss is stopped when the price reaches the maximum stop loss point; the upper and lower Bollinger Bands are used to set the trailing stop loss point, and the trailing stop loss price is updated in real time according to the price operation.
## Advantage Analysis
This strategy combines multi-indicator judgment to effectively identify trends; it also uses a stop-loss mechanism to lock in profits and reduce the risk of losses, making it a relatively robust strategy. The specific advantages are as follows:
1. Use Bollinger Bands to determine overbought and oversold situations and determine reversal opportunities.
2. Combining RSI indicators can increase the accuracy of judgment.
3. The ADX indicator determines the trend formation and ensures the correct trading direction.
4. ATR and Bollinger Bands trailing stop loss can lock in profits to the greatest extent
## Risk Analysis
There are also some risks with this strategy:
1. Multi-indicator judgment, parameter settings are easy to optimize
2. When the Bollinger Bands range is wide, the overbought and oversold signals are less effective.
3. Improper stop loss tracking can lead to expanded losses
To address these risks, we can take the following measures:
1. Multi-combination parameter optimization to prevent over-optimization
2. Adjust Bollinger Band parameters according to market fluctuations
3. Test the stop loss distance parameters to ensure that they can withstand normal fluctuations
## Optimization direction
This strategy can also be optimized from the following directions:
1. Increase position control and adjust position size based on stop loss multiplier
2. Add money management module to strictly control the single stop loss amount
3. Test other stop loss indicators, such as DMI, Envelop, etc.
4. Add machine learning models to judge trend probability and improve the effect
## Summarize
Overall, this strategy is a relatively robust trend following strategy. By determining the trend direction through multi-indicator judgment and using stop-loss measures to control risks, you can get a better return rate. We have also proposed several directions that can be optimized. If further optimization can achieve better results.
||
