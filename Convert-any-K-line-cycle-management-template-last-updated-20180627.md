...
Convert any K-line cycle management template last updated 20180627
> Author
...
Updated on 20171114
    a. Solve the problem that Open cannot be found, which is caused by the record array access out of bounds. The access out of bounds is caused by the problem of the previous K line.
Updated on 20171113
    a. Filter out K-line combinations with incorrect starting time
    b. Filter out K-line combinations with incorrect time intervals
Updated on 20170622
     a. RecordsManager adds Name parameter to facilitate distinguishing different K lines
     b. Fix the problem of incorrect Time calculation when the number of fixed K lines is less than the period of a new K line.
Updated on 20170531
    a. fix Volume calculation error
1. Modified from Xiaoxiaomeng's "Convert any K-line cycle" template
Principle:
  - Obtain the period of the fixed K-line, and then synthesize a new K-line period that is an integer multiple of any fixed K-line
Function:
  - Convert the basic K-line to any K-line period
  - Second level is not supported at the moment
Limitations:
   - The new K-line period must be an integer multiple of the fixed K-line period.
   - The fixed K-line period is 1min, 3min, 5min, 15min, 30min. The new K-line period must also be minutes and <60
   - The fixed K-line period is 1 hour, and the new K-line period must also be hours and <24
   - The fixed K-line period is 1 day, and the new K-line period must also be days
   - The number of fixed K-line periods obtained each time must be >= 2
Test version, if there are any bugs or questions, please leave a message.
Output function:
    $.RecordsManager(NewCycleMS, Name) generates a new cycle manager
        NewCycleMS is the new K-line cycle in milliseconds. Default (1000*60*60*2) 2hour.
        Name: Specify a name for this K-line management
        Return to K line manager
    $.AssembleRecords(records, BaseCycleMS)
        records: the original records obtained
        BaseCycleMS is a fixed K-line cycle in milliseconds. By default, fixed records are used for calculation.
        Returns the records of the new K-line cycle
    $.GetRecordsTable(n) Gets the latest N entries of the new K line. By default, all entries are output. The output is of table type, which is convenient for LogStatus output.
