...
Quantitative trading strategy based on dynamic mean breakout of Bollinger Bands-Dynamic-Bollinger-Bands-Breakout-Quantitative-Trading-Strategy
> Author
...
![IMG](assets/images/c7eafdac8bb3445111fe900cf0f4e85edfece50720d5a368f7fabd8d3a4783c1.png)
![IMG](assets/images/756fc7fcddc03f8283746749f1654b7cb63ab8755c8efa63283c499e3d8aecd9.png)
...
[trans]
#### Overview
This strategy is a quantitative trading system based on dynamic breakthroughs of Bollinger Bands. It combines a variety of moving average types (including SMA, EMA, SMMA, WMA, VWMA) to construct Bollinger Bands, and makes trading decisions based on the relationship between price and the upper and lower tracks of the Bollinger Bands. The core idea of ​​the strategy is to capture the upward trend when the price breaks through the Bollinger Band upper band, and to stop losses in time when the price falls below the lower band.
#### Strategy Principle
The operating principle of the strategy mainly includes the following key elements:
1. Calculate the middle track of Bollinger Bands through selectable moving average types (SMA, EMA, etc.).
2. Calculate the upper and lower orbits using the standard deviation multiplier (default is 2.0).
3. When the closing price breaks through the upper track, open a long position.
4. When the closing price falls below the lower track, close the position and end the transaction.
The strategy also includes risk management mechanisms such as date range filtering and slippage control to improve the stability and reliability of transactions.
#### Strategic Advantages
1. Strong adaptability: supports multiple moving average types, and can select the optimal moving average according to different market characteristics.
2. Improved risk control: Through the dynamic adjustment of Bollinger Bands, it can adapt to changes in market volatility.
3. Flexible parameters: Allows adjustment of parameters such as the period length of Bollinger Bands and standard deviation multiples to adapt to different market environments.
4. Transaction cost considerations: Built-in handling fee and slippage settings are more in line with actual transaction conditions.
5. Reasonable position management: Use the account net value percentage to control positions and effectively manage risks.
#### Strategy Risk
1. False breakthrough risk: Frequent false breakthrough signals may appear when the market is volatile.
Solution: Auxiliary indicators can be added to confirm the validity of the breakout.
2. Trend reversal risk: There may be a delayed reaction when a strong trend reverses.
Solution: Consider adding a trend confirmation indicator.
3. Excessive trading risk: Frequent trading signals may lead to excessive trading costs.
Solution: Add signal filtering mechanism and position limit.
...
- Realize dynamic adjustment of Bollinger Band parameters
- Adaptively adjust trading thresholds based on market volatility
#### Summary
This is a complete trading system based on Bollinger Bands, with good adaptability and scalability. Through the selection of multiple moving average types and flexible parameter settings, it can adapt to different market environments. The risk management mechanism of the strategy is relatively complete, but there is still room for optimization. It is recommended to focus on the enhancement of the signal confirmation mechanism and the optimization of risk management to improve the stability and profitability of the strategy. ||
#### Overview
