...
Trend following strategy based on ATR and CCICCI-and-ATR-Based-Trend-Following-Strategy
> Author
...
## Overview
This strategy identifies price trends by combining the Average True Volatility Index (ATR) and the Commodity Channel Indicator (CCI), with overbought and oversold areas as entry and exit signals. ATR is used to calculate the upper and lower rails of the channel, while CCI is used to determine the trend direction. Go short when CCI breaks below the overbought zone, and go long when it breaks above the oversold zone to achieve trend following.
## Strategy Principle
1. Calculate ATR, here we choose 2-period ATR
2. Calculate the CCI value, here 10 periods are selected
3. Determine the trend direction based on the CCI value of the current K line
    - CCI >= 0, Defines as Uptrend
    - CCI < 0, Defines as Downtrend
4. Calculate upper and lower rails
    - Upper track = Highest price + ATR * Multiplier
    - Lower track = lowest price - ATR * Multiplier
5. Save and update the upper and lower tracks in different trend directions
    - When CCI >= 0, if the upper rail < the upper rail of the previous K line, reassign the value; if CCI < 0, if the lower rail > the lower rail of the previous K line, reassign the value
    - This prevents the channel from moving in the opposite direction with the price
6. Enter the upper rail or lower rail according to the CCI value as entry signal
7. Cross the 0 axis according to the CCI value as the exit signal
8. Set Take Profit and Stop Loss Exit
## Strategic Advantages
This strategy combines trend judgment and channel breakthroughs, and can effectively track trends.
1. Use CCI to determine the price trend direction and quickly determine the long and short trends.
2. Set stop loss and take profit on the ATR channel to control risks
3. When the price reverses, the upper and lower rails can quickly adjust the direction to avoid being trapped in the original trend channel.
4. Use the 0-axis cross of CCI as an exit signal to follow the trend and avoid reversal traps.
## Strategy Risk and Optimization
1. CCI and ATR parameter settings need to be optimized. Under different cycles and parameters, the effects may be quite different.
2. Although channel adjustment can be used to avoid tightness, if there is a sharp reversal, there may still be insufficient profits or losses.
3. The stop loss setting needs to be optimized, the stop loss range should be appropriately relaxed, and the stop loss should be avoided too frequently.
4. Can be combined with other indicators to filter entry signals and optimize entry opportunities
5. Can avoid reverse trading by combining it with higher cycle trend indicators
## Summarize
Overall this strategy is a simple and practical trend following strategy. The advantage is that the ideas are clear, easy to implement, and you can quickly seize trend opportunities. However, the parameter settings, stop loss range, etc. need to be optimized and adjusted according to market conditions. Combination with other indicators can further enhance the effectiveness of the strategy. Overall, this strategy is a good entry-level strategy for trend tracking and is worth learning from.
||
