...
Double-VWAP-Oscillation-Breakthrough-Strategy Double-VWAP-Oscillation-Breakthrough-Strategy
> Author
...
![IMG](assets/images/f5179b0c7cbafb2fdce1914a7694999ffde163379464d8fa57b7afdd40febbb4.png)

[trans]

## Overview
The double VWAP moving average shock breakthrough strategy analyzes the market trend through double VWAP moving average and looks for breakthrough opportunities in the volatile market. It combines the ADX indicator to determine whether the market is volatile, and uses two VWAP moving averages with different standard deviations to find a breakthrough point to place orders for entry.
## Strategy Principle
This strategy mainly consists of the following parts:
1. VWAP settings: Calculate VWAP moving average and its bandwidth. The internal VWAP bandwidth is controlled by `stDevMultiplier`, the default is 1; the external VWAP bandwidth is controlled by `stDevMultiplier`, the default is 2.
2. ADX setting: Calculate the ADX value to determine whether the market is volatile. When ADX is lower than the threshold, it is judged as a volatile market. ADX parameters are configurable.
3. Entry setting: In a volatile market, enter the market when the price breaks through the external VWAP bandwidth. Configurable stop loss price and take profit price.
4. Restrict entry: Optional EMA moving average or time period filtering to avoid entry during non-ideal periods.
5. Profit method: close the position when the trailing stop loss or take profit price breaks. Optional price breakout VWAP exiting.
This strategy uses the ADX indicator to judge the volatile market and looks for entry opportunities when the price breaks through the VWAP bandwidth. Dual VWAP bands provide more filtering to ensure a strong entry. Trailing stops make profits more stable.
## Advantage Analysis
1. Dual VWAP bands provide additional entry filtering to ensure strong entry timing.
2. The ADX indicator determines the market shock and avoids mistakes in the trending market.
3. Use trailing stop loss to lock in profits and avoid getting stuck.
4. Rich configurable parameters and strong adaptability.
5. The ideas are clear and easy to understand, and easy to copy and modify.
## Risks and Solutions
1. Improper parameter settings may lead to overly aggressive entry or position closing. Optimizing parameter combinations ensures strategy stability.
2. Trailing stop loss can easily be too aggressive or conservative. Dynamically adjust the stop loss position based on volatility indicators.
3. Performance is sensitive to trading hours. Can be optimized through time filters to ensure efficient entry.
4. The VWAP indicator is sensitive to abnormal prices. Combine with other indicators to confirm price reasonableness.
## Optimization direction
1. Dynamically adjust the stop loss range. The stop loss position can be adjusted in real time based on indicators such as volatility.
2. Use multiple timeframes to confirm entry timing. Add higher timeframe trend and institutional indicators to avoid entering against the trend.
3. Consider position management. Position percentages are dynamically adjusted based on volatility and account funds.
4. Test the performance of different VWAP cycles. The VWAP cycle setting determines the position period of the strategy and can be optimized.
## Summarize
The dual VWAP moving average shock breakout strategy uses ADX to determine market shocks and utilizes the dual VWAP bands to provide additional entry filtering. The strategy is clear and easy to implement. The stability of the strategy can be significantly improved through parameter adjustment, stop loss optimization, position management and other means.
||
