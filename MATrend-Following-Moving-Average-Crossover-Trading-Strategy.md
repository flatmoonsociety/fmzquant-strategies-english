...
![IMG](assets/images/d976080d79c34a40155142ad539a38be9585a585b736ca33dcc85c4f13ff2ace.png)

[trans]


## Overview
This strategy is a trend following trading strategy based on moving averages. It uses three Hull moving averages with different parameter settings to determine the price trend direction, and combines it with a fast ATR filter to achieve early identification of potential trend reversals. When the fast, medium and slow moving averages cross upward or downward, a buy or sell signal is issued. This strategy has both trailing stop loss and trailing take profit functions, which can effectively control risks.
## Strategy Principle
This strategy uses three Hull moving averages to determine the price trend, including a faster Hull MA, a medium-speed Hull MA and a slower Hull MA. Determine the direction of the trend based on their intersection:
1. When the fast line crosses the middle line, it means that the price has entered an upward trend and a buy signal is issued.
2. When the fast line crosses the middle line, it means that the price has entered a downward trend and a sell signal is issued.
In order to improve the sensitivity of identifying trend reversals, the strategy introduces a fast ATR filter based on RSI. This filter measures price variability, and its value will change significantly when the price trend changes. Therefore, we can judge the reversal of the price trend in advance based on the upper and lower breakthroughs of the ATR filter.
Specifically, the filtr function implements the calculation logic of the fast ATR filter. It calculates the size of ATR based on the value of RSI. When the ATR value crosses above or below the RSI curve, it may indicate a change in the price trend.
In addition, the trailing stop loss and trailing take profit conditions are set in the strategy, and automatic risk management can be realized according to the set stop loss percentage and take profit percentage.
## Advantage Analysis
- Use three Hull MA moving averages to determine the trend direction, which can effectively filter market noise and identify medium and long-term trends.
- The application of fast ATR filter can improve the ability to judge trend reversal in advance
- Automatically seize trend reversal opportunities and adjust positions in a timely manner without missing out on buying or selling.
- Trailing Stop Loss and Take Profit sets a dynamic balance between risk and reward
- Customizable parameters, suitable for different markets and trading varieties
## Risk Analysis
- MA crossover strategy is prone to produce long false signals and short false signals, and requires ATR filter for auxiliary verification.
- In a volatile market, MA is prone to frequent crossovers, so you should pay close attention to the trend of the ATR curve.
- If the stop loss point is too small, the loss will be easily stopped; if the stop loss point is too large, it will be difficult to control the loss. Parameters need to be adjusted according to specific circumstances
- This strategy is more suitable for trending market conditions and is not suitable for use in volatile market conditions.
- Parameter optimization can be used to select the best MA and ATR period combination to reduce the false signal rate
## Optimization direction
- You can try changing the MA type to DEMA, TEMA and other EMA variants to see if more noise can be filtered out
- The ATR filter can be changed to use the Keltner channel MIDDLE line to test the improvement of trend reversal judgment.
- Can test different MA parameter combinations and find the best parameter pair
- ATR cycle parameters can be tested to find the best smoothing effect
- Volume and energy indicators can be added to help determine the possibility of true or false breakthroughs
- Can test whether to add other indicators such as MACD to improve the reliability of the signal
## Summarize
This strategy integrates the functions of moving average to determine the trend direction, ATR filter to detect reversal in advance and automatic stop loss and profit management to manage risk. It can automatically track trends, seize reversal opportunities in a timely manner, and can be applied to different varieties and cycles through parameter optimization. It is a very practical trend-following trading strategy. Its advantages are simple and clear strategy logic and efficient risk control methods. But we also need to pay attention to the issues of illusory signals and stop loss point setting. Through further optimization, better strategic effects can be expected.
