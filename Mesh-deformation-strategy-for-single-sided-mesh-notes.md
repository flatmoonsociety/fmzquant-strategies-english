...
Grids can be oriented in customizable directions
Buy first, sell later:
The grid will place buy orders downwards starting from the first price. The interval between each buy order is the "price interval" parameter. The number of pending orders is "single quantity". If there are enough "total quantity" buy orders, after any buy order is completed, the program will add the "spread (yuan)" parameter to the purchase price to place a sell order. After selling, the buy order will be placed again at the original price of this grid.
Sell first, buy later:
The operation is just the opposite
The biggest risk of the strategy is unilateral market conditions, and price fluctuations beyond the grid range.
Grid with automatic stop loss and move function

// Little Dream Dream Back
// https://www.fmz.com/bbs-topic/334 Summary - inventor quantification video teaching and graphic teaching
...
|----|----|----|
|OpType|0|Grid Direction: Buy first, sell later|Sell first, buy later|
|FirstPriceAuto|true|First Price Auto|
|FirstPrice|100|First price|
|AllNum|10|Total quantity|
|PriceGrid|true|Price interval|
|PriceDiff|2|Price difference (yuan)|
|AmountType|0|Order size: Same amount for buying and selling|Customized amount|
|AmountOnce|0.1|Single transaction quantity|
|BAmountOnce|0.1|Buy order size|
|SAmountOnce|0.1|Sell order size|
|AmountCoefficient|*1|Amount difference|
|AmountDot|3|The longest number of decimal places for amount|
|EnableProtectDiff|false|Enable spread protection|
|ProtectDiff|20|Entry Spread Protection|
|CancelAllWS|true|Cancel all pending orders when stopping|
|CheckInterval|2000|Polling interval|
|Interval|1300|Failure retry interval|
|RestoreProfit|false|Restore last profit|
|LastProfit|false|Last Profit|
|ProfitAsOrg|false|The last profit is included in the average price|
|EnableAccountCheck|true|Enable funds check|
|EnableStopLoss|false|Enable stop loss|
|StopLoss|100|Maximum floating loss (yuan)|
|StopLossMode|0|Operation after stop loss: Recycle and exit|Recycle and then cast the net|
|EnableStopWin|false|Enable stop profit|
|StopWin|120|Maximum floating profit (yuan)|
|StopWinMode|0|Operation after taking profit: Recycle and exit|Recycle and then cast the net|
|AutoMove|false|Auto move|
|MaxDistance|20|Maximum distance (yuan)|
|MaxIdle|7200|Maximum idle (seconds)|
|EnableDynamic|false|Enable dynamic pending orders|
|DynamicMax|30|Order failure distance (yuan)|
|ResetData|true|Clear all data on startup|
|Precision|5|Price decimal place length|
|XPrecision|5|Decimal digit length of order quantity|
...
|----|----|----|
|Close the net|__button__|Stop and balance to the initial state|
