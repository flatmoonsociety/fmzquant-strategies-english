...
Multiple-Timeframe-Filtered-Trend-Following-Strategy Multiple-Timeframe-Filtered-Trend-Following-Strategy
> Author
...
## Overview
This strategy achieves accurate judgment on trends by rationally using various technical indicators such as moving averages, relative strength index (RSI), and moving average direction. On the basis of the long and short judgment of the double moving average, the long and short filtering of the RSI indicator is added to avoid false breakthroughs. At the same time, through the joint observation of moving averages of different periods, the trend direction can be effectively identified. The strategy optimization space is large and can be applied to different varieties and cycles.
## Strategy Principle
This strategy mainly operates based on the following technical indicators:
1. Double moving averages: The golden cross of the fast moving average and the slow moving average is bullish, and the dead cross is bearish. This strategy uses the EMA indicator to calculate the moving average.
2. RSI indicator: When the RSI reaches a high level, it is a bearish opportunity, and when it reaches a low level, it is a bullish opportunity. This strategy uses the long-short logic of the RSI indicator for trend filtering.
3. Moving average direction: Comparing the direction of long-term and short-term trends can determine the trend. In this strategy, the 200-period EMA is used to determine the long-term direction.
The transaction logic is as follows:
1. If the fast EMA crosses above the slow EMA, go long; if the fast EMA crosses below the slow EMA, go short.
2. The RSI indicator falls back from high levels and adds short selling opportunities, while rising from low levels adds long selling opportunities.
3. Only enter the market when the direction of the long-term (200-day EMA) is consistent. If the long-term rises, only go long, and if the long-term falls, only go short.
4. Adopt a stop-profit and stop-loss exit strategy.
## Advantage Analysis
This strategy has the following advantages:
1. The combined use of multiple technical indicators can effectively confirm the trend direction and reduce false breakthrough operation opportunities.
2. Add the long and short filtering of the RSI indicator to avoid the whipsaw of trend reversal.
3. Using short, medium and long-term trend judgment can improve the timeliness and directionality of entries.
4. Stop loss has set up risk control measures to control single losses.
5. Adjustable parameters achieve multi-period adaptability and are suitable for different trading varieties.
## Risk Analysis
This strategy also has certain risks:
1. In the general trend market, short-term adjustments may trigger stop losses. The stop loss range can be appropriately relaxed, or a trailing stop loss can be used to track the stop loss.
2. In a volatile trend, false breakthroughs may lead to losses. The filtering interval of the RSI parameter can be appropriately increased, or indicators such as the Donchian channel can be used for assistance.
3. Improper parameter optimization may lead to too frequent transactions, and optimization parameters need to be carefully tested for different varieties.
4. This strategy is only based on technical indicator trading and needs to be combined with fundamental analysis to determine the direction of the general trend.
## Optimization direction
This strategy can be optimized from the following aspects:
1. Adjust the moving average cycle to adapt to different market cycles.
2. Optimize the parameters of RSI and improve the accuracy of long and short selections.
3. Test and add Bollinger Bands, Keltner Channels and other indicators to improve the success rate of breakthroughs.
4. Try changing the stop loss to a trailing stop or a атель stop to better follow the trend.
5. Study the use of range breakout operations when the trend is weak to reduce the probability of mistaken transactions.
6. Based on the characteristics of different varieties, set reasonable stop-profit and stop-loss values ​​to control risks.
7. Add transaction volume control to avoid excessive single investment.
## Summarize
The overall idea of ​​this strategy is clear and easy to implement. After parameter optimization, it can be applied to a variety of varieties and cycles, and has strong ability to track trends. However, you need to pay attention to controlling risks to prevent being trapped in volatile market conditions. Strategies can be customized and optimized based on market characteristics and personal preferences.
||
