...
![IMG](assets/images/dbc6be188c1d38e9b3301343df64c065644ee13ad5cd27d1d024c948e4ba270a.png)

[trans]

## Overview
This strategy comprehensively uses MACD, RSI, PSAR and other technical indicators as well as dynamic fund management principles to achieve trend tracking and reversal trading under multiple time frames. Strategies can be applied to short-term, medium-term and long-term trading.
## Principle
策略使用PSAR指标判断趋势方向。 EMA快慢线与BB中线交叉作为第一确认点。 The MACD histogram direction serves as a second confirmation point. The RSI overbought and oversold area serves as the third confirmation point. A trading signal is generated when the above conditions are met.
Set a stop-loss and stop-profit point after entering the market. The stop loss point is set at a certain multiple of the ATR value. The same is true for the profit stop point. At the same time, set a floating loss percentage stop loss. Stop loss and exit when the loss reaches a certain proportion of the total equity of the account.
Floating profit also has a percentage setting. When the profit reaches a certain proportion of the total equity of the account, the profit will be taken out.
Dynamic fund management calculates the position size based on the total equity of the account, ATR, and the set stop loss multiple.同时设置最小交易量。
## Advantages
1. Multi-factor confirmation to avoid false breakthroughs and improve entry accuracy.

3. The stop-loss and take-profit points are set by ATR, which can be adjusted according to the degree of market fluctuations.
4. The percentage floating loss and floating profit setting locks in profits and avoids giving up.
## Risk

2. Setting the percentage too high may cause losses to expand.
...
5. Optimize time period settings and test trading periods.
## Summarize
This strategy comprehensively uses a variety of technical indicators to judge trends, adds dynamic fund management to control risks, and achieves stable profits under multiple time frames. Factor weights, risk control parameters and fund management settings can be continued to be optimized based on the backtest results to achieve better results.
||
