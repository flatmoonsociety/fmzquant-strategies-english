...
RSI Discrete Divergence Strategy RSI-Divergence-Strategy
> Author
...
![IMG](assets/images/30e56de54515e7a1ca92bb9c707bc2c37a0c625456be9ab53bd1cf9f9eaa9404.png)
[trans]

### Overview
This strategy determines long and short positions by calculating the intersection of the RSI indicator and its EMA moving average, and combines the deviation between RSI and price to find potential buying and selling points. It is a trend following strategy.
### Strategy Principles
1. Calculate the RSI indicator with a length of 14. When RSI crosses above the 50 dividing line, it is a bullish signal, and when it crosses below, it is a bearish signal.
2. Calculate the 20-period EMA and the 14-period EMA of RSI. When the fast line crosses the slow line, it is a buy signal, and when it crosses below, it is a sell signal.
3. Detect the divergence between RSI and price:
- Bullish divergence: The price hits a new low but the RSI does not hit a new low, which is a buy signal.
- Hidden bull divergence: the price hits a new high but the RSI does not hit a new high, which is a buy signal
- Short divergence: The price reaches a new high but the RSI does not reach a new high, which is a sell signal.
- Hidden short divergence: the price makes a new low but the RSI does not make a new low, which is a sell signal
4. You can choose to enable stop loss strategies, including percentage stop loss and ATR stop loss.
### Advantage Analysis
1. The advantage of the RSI indicator is that it can detect overbought and oversold conditions. The advantage of the EMA moving average is that it can play a smoothing role and filter out some noise.
2. RSI and price divergence can give early signals before the trend reverses.
3. Integrating two indicator signals can verify each other and improve the stability of the strategy.
4. The stop-loss mechanism can control single losses.
### Risk Analysis
1. RSI is an index indicator that fluctuates with the price. When the price fluctuates violently, the effectiveness of the RSI indicator will be reduced.
2. The EMA moving average has a time lag problem and cannot accurately locate the turning point.
3. The divergence signal may be a false signal and the price continues to run in the original trend.
4. Unreasonable stop loss setting may cause unnecessary stop loss.
5. The drawdown may be large and sufficient financial support is required.
### Optimization direction
1. You can test the calculation of RSI and EMA with different parameters and find the best parameter combination.
2. You can consider replacing the EMA moving average with other indicators such as MACD for combination optimization.
3. A confirmation mechanism can be set up to avoid false deviations. If multiple consecutive divergence signal triggers are required.
4. Add a take-profit strategy to lock in profits.
5. You can enter the market based on short-term signals such as the candlestick pattern and cooperate with the trend judgment of this strategy.
### Summarize
This strategy integrates the overbought and oversold judgment of the RSI indicator, the trend judgment of the EMA and the prediction of divergence signals, forming a relatively complete trend tracking system. On the basis of parameter adjustment and combination optimization, better strategic effects can be obtained. However, we still need to pay attention to guard against the impact of the trend market and the interference of false signals. Through strict fund management, this strategy can achieve stable excess returns in the medium and long term.
||
