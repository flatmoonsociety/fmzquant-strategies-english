...
MACD-based-Dual-Trading-Strategy MACD-based-Dual-Trading-Strategy
> Author
...
![IMG](assets/images/3b35f1e797f67cddf2a52a4eceba58b2903c446f5d6f0475bb1f26739d0c5713.png)
[trans]

## Overview
This strategy implements a two-way trading strategy based on the MACD indicator. It can do long and short positions respectively when the MACD indicator is golden cross and dead cross, and can be combined with other indicator judgments to filter out some signals.
## Strategy Principle
This strategy mainly uses the MACD indicator to achieve two-way trading. Specifically, it calculates the fast moving average, slow moving average, and MACD signal line. When the fast moving average crosses the slow moving average, a golden cross signal is generated to go long; when the fast moving average crosses below the slow moving average, a dead cross signal is generated to go short.
In order to filter out some invalid signals, the strategy also sets a ±30 range as a filter. The trading signal will only be triggered when the MACD histogram exceeds this range. In addition, when closing a position, the direction of the MACD bar line will also be judged. The position will only be closed when the direction of two consecutive bars changes.
## Strategic Advantages
- Use the MACD indicator as the main trading signal, which is sensitive to both sides of the stock market.
- Added a filter to filter out some invalid signals
- The closing logic of judging the direction of two consecutive pillars is adopted, which can avoid false breakthroughs to a certain extent.
## Strategy Risk
- The MACD indicator is prone to generate frequent trading signals, which may lead to excessive trading frequency
- Single indicator strategy, a slight delay in the signal may lead to losses
- The closing logic for judging the direction of the columnar line is not rigorous enough, and there may be a risk of signal loss.
## Strategy optimization direction
- You can consider combining other indicators to confirm the signal, such as KDJ indicator, Bollinger Band indicator, etc.
- You can study other more advanced indicators to replace the MACD indicator, such as KD indicator
- You can optimize the closing logic and set stop loss and take profit to control single losses
## Summarize
In general, this strategy is a basically usable two-way trading strategy. It takes advantage of the MACD indicator and also adds certain filters to control the quality of the signal. However, the MACD indicator itself also has some problems, and further testing and optimization are still needed in real trading to make the strategy more reliable. Overall, this strategy lays the foundation for a two-way trading strategy, which can be continuously optimized in the future to make it a powerful quantitative trading strategy.
||
