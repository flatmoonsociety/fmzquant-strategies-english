...
WaveTrend-and-DER-Based-Swing-Trading-Strategy based on deviation rate indicator optimization
> Author
...
![IMG](assets/images/70cbf44399315cc5315f7803d617d4cd9191dcf8f05bc96a580bbdd6e4fe7aeb.png)
[trans]

## Overview
This strategy achieves precise tracking of market fluctuations by combining multiple technical indicators such as Relative Strength Index (RSI), Directional Movement System, and Directional Energy Ratio (DER) to capture opportunities in short- and medium-term market trends.
## Strategy Principle
The core logic of the code is to use the WaveTrend indicator to determine the direction of price fluctuations, the RSI indicator to determine overbought and oversold conditions, and the custom Directional Energy Ratio indicator to determine the strength of the price movement to determine the long and short direction.
Specifically, when the second average line wt2 of the WaveTrend indicator crosses the first average line wt1, it is a long signal. At this time, if DER>0 indicates that the current trend is upward, then it will enter the long position; when the second average line wt2 of the WaveTrend indicator crosses below the first average line wt1, it is a short signal. At this time, if DER<0 indicates that the current downward trend, then it will enter the short position.
In addition, the RSI indicator will be used to determine extreme overbought and oversold conditions. If the RSI indicator shows overbought (RSI is greater than 70), then you will consider exiting the long position; if the RSI indicator shows oversold (RSI is less than 30), then you will consider exiting the short position.
## Strategic Advantages
1. Using the WaveTrend indicator to judge price fluctuations and direction changes is more accurate than using a single moving average and other indicators.
2. Use DER’s custom indicators to determine the intensity and direction of price fluctuations to avoid being trapped in volatile market conditions.
3. The RSI indicator determines overbought and oversold conditions and helps to stop losses in time.
4. Overall, this strategy has fast response speed and strong ability to control short-term market conditions.
## Risk Analysis
1. Multiple parameters are used in the strategy, and the parameters need to be optimized. If the parameters are set improperly, the strategy returns will be affected.
2. The strategy mainly targets short-term fluctuations and may not perform well in long-term sustained trend markets.
3. The strategy is sensitive to transaction fees, so you need to choose an exchange with lower fees.
4. The strategy does not take into account market news events, making it easy to be impacted by major emergencies.
## Optimization direction
1. Consider introducing machine learning algorithms to automatically optimize indicator parameters.
2. Can be combined with more transaction sizes and position management techniques.
3. It can be combined with long-term trend judgment indicators, such as moving averages, etc., to determine the overall enter and exit at a larger level.
4. Consider combining the risk identification capabilities of major news events.
## Summarize
This strategy comprehensively uses a variety of technical indicators to judge short-term fluctuation trends, achieve low-risk market tracking and manipulation, and is more suitable for seizing short- and medium-term opportunities. By further optimizing parameter settings, position management, and combining more indicators, better retracement control and overall effects can be achieved.
||
...
1. Introduce machine learning algorithms to auto-optimize parameters.
