...
Speculation-Gulf-Trend-Follow-Strategy-Based-on-SAR-based trend-following quantitative strategy
> Author
...
![IMG](assets/images/3ace468a8a93799397eb8dc3be38eeda094ad88710b4b0d2e37c38cbefa84366.png)
[trans]

## Overview
The Speculative Gap Strategy is a quantitative trading strategy that tracks trends. It uses the SAR smooth curve as the main trading signal, supplemented by various filters such as EMA, squeeze momentum, and volatility oscillators. It identifies trend reversal points by configuring SAR parameters to achieve low-risk trend tracking. This is a strategy that is very suitable for medium and long-term investments.
## Strategy Principle
This strategy uses Parabolic SAR as the main trading signal indicator. SAR can effectively determine the reversal point of the price trend. When the SAR sign changes, it means that the trend has turned. This strategy generally generates a buy or sell signal when the SAR flips over.
Additionally, the strategy also provides a SAR breakout option. That is, a signal is generated when the price has exceeded the last SAR value before the SAR has completely reversed. This can further pursue the sensitivity of the strategy.
In order to filter out false signals, this strategy also introduces three auxiliary filters: EMA, squeeze momentum and volatility oscillator, which can be used alone or in combination to confirm the reliability of price trends and trading signals.
Finally, the strategy provides three stop-loss and stop-profit methods: fixed stop loss, fixed take-profit and risk-reward proportional stop-loss. This allows the strategy to be flexibly adapted to the characteristics of different types of trading instruments.
## Advantage Analysis
1. SAR can accurately determine price trend reversal and capture new price trends in a timely manner, which is suitable for medium and long-term trend tracking.
2. Multiple filter settings reduce the probability of false breakthroughs and improve signal reliability.
3. The configuration is simple and flexible, and parameters can be customized to adapt to different trading varieties.
4. Provide a variety of stop-profit and stop-loss methods to pursue the balance of risk and return.
5. Can be directly connected to the trading robot to realize automated trading.
## Risk Analysis
1. In non-trending markets, there may be an increase in false signals and invalid transactions.
2. Improper SAR parameter settings will also affect the accuracy of signal judgment.
3. As a trend following strategy, it is easy to reach the stop loss line in a sharply volatile market.
In response to the above risks, SAR parameters or filter parameters can be appropriately adjusted to reduce the probability of invalid transactions. Stop loss restrictions can also be appropriately relaxed to withstand greater market fluctuations.
## Optimization direction
1. SAR parameter optimization. The step size and incremental parameters of SAR can be optimized through historical backtest data to obtain a more stable and efficient trading strategy.
2. Introduce trend judgment indicators. Add MACD, DMI and other auxiliary judgment indicators to the strategy to improve the ability to judge trends.
3. Optimize the risk-reward ratio. Adjust the fixed take-profit and stop-loss percentages and risk-reward ratio parameters to appropriately take higher risks to obtain higher returns.
4. Add foreign exchange varieties. Currently, the strategy only supports digital currency transactions, and can be expanded to support foreign exchange, commodity and securities market varieties.
## Summarize
Speculative Divide is a very practical trend-tracking quantitative strategy. It is responsive, reliable in signal judgment, and can obtain long-term stable income through stop-loss and stop-profit management. Appropriate parameter and rule optimization can further improve the efficiency of the strategy. This is an efficient quantitative strategy worth using over the long term.
|| 
