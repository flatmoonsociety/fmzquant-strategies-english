...
Uniswap-V3-Trading libraryUniswap-V3-Trade-Template
> Author

inventor quantification
> Strategy Description

The inventor platform is officially connected to the contract interactive call on the web3 chain. This template is a trading library using uniswap v3 as an example. Those with hands-on skills can easily access other defi exchanges.
The summary function call is as follows. The detailed documentation will be merged into the platform API documentation after it is completed. Here are some examples first.

If the calling contract is a standard ERC20 method, there is no need to register. You can call it directly. The third parameter has strong compatibility and you can write the method name. The methodId is such as "0x571ac8b0". You can also write the complete standard method name.
The constant attribute method directly returns the function result, and the non-constant method such as approve directly returns the broadcast txid.
```
...
Calling methods other than the standard contract requires registering the ABI content. See this template code example for details. After registration, you can call other methods of the contract.
```
exchange.IO("abi", tokenAddress, abiContent)
```

The ABI content of the contract can be obtained through the following URL, only the result field is obtained.
```
https://api.etherscan.io/api?module=contract&action=getabi&address=0x68b3465833fb72A70ecDF485E0e4C7bD8665Fc45
```

If the calling method is payable, you need to add a transfer ETH value (the fourth value) after the method name. For example, the parameter value in the following function is 123. Of course, you can also pass a string number.
```
exchange.IO("api", ContractV3SwapRouterV2, "multicall(uint256,bytes[])", 123, (new Date().getTime() / 1000) + 3600, "")
```

You can also pass in extra parameters to specify gasLimit/gasPrice/nonce
```
exchange.IO("api", ContractV3SwapRouterV2, "multicall(uint256,bytes[])", ..., {gasPrice: 123456, gasLimit: 21000})
```

If the second parameter of IO is `eth`, all available RPC methods of the node server are directly called, such as
```
exchange.IO("api", "eth",  "eth_getBalance", self.walletAddress, "latest")
```

> In order to facilitate transfers, an additional send function is added to send ETH
```
exchange.IO("api", "eth",  "send", toAddress, toAmount)
```

> Provides encode/encodePacked and decode methods, which can return function call encoding to hex string format. You can read the multicall call to uniswap v3 in this template code
```
...
> You can switch private keys at any time and operate multiple addresses, such as
```
exchange.IO("key", "219103fb1e058720b8431580cdc61ed77a270bac1f87503145b9e9b5cd8a92ce")
```


> ABI internal variable conversion rules
address: string starting with 0x
bytes: hexadecimal encoded string, such as "008395"
uint256: can be a string or a number "0x12345" or 123
tuple: Pass the structure directly, such as {path: "", amountIn: 0x123}
Other types are passed directly


> The uniswap template calling example is as follows, add exchange Web3, fill in the node address and private key, the private key can be saved locally (the private key is written as a file path), see the API document for details
The node address can be configured using infura's free node application and replaced with your own key https://mainnet.infura.io/v3/{apikey}
```
