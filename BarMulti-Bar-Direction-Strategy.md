...
![IMG](assets/images/bb1d1fe01b9fc4ea1e061a768a6005e386f0587afb86272b3a271cc641c383d0.png)

[trans]

## Overview
The multi-bar same-direction strategy counts the trend probabilities of multiple bars, identifies trend signals, and conducts reverse trades when reversal signals appear. This strategy is mainly used in short- and medium-term transactions.
## Strategy Principle
This strategy first sets the starting statistical time and the ending statistical time, which are used to extract historical data. Then set the trading time to identify the K-line that meets the conditions. The strategy statistics calculate the probability of rising or falling in the same direction from 2 K lines to 7 K lines. If the rise or fall reaches a certain proportion threshold, a trading signal is generated.
For example, the strategy counts the probability of decline within 3 K lines. If the probability of decline is less than 50%, then the current 3 K lines meet the conditions and generate a bullish signal. The strategy allows you to set the statistical parameters of 2 to 7 K lines.
Specifically, the strategy logic is as follows:
1. Set the backtest time range, including start date, end date, and transaction time range.
2. Count the number of K lines rising or falling in the same direction within 2 to 7 K lines.
3. Calculate the probability that the number of adjacent K lines continues to rise or fall.
4. If the probability is lower than 50%, it is considered that the current K line conforms to the reversal signal form.
5. Generate bullish or bearish signals within the trading time frame.
6. Conduct backtesting to verify the effectiveness of the strategy.
## Strategic Advantages
- By counting the probabilities of multiple K lines, avoid erroneous signals caused by a single K line
- The number of K lines can be customized to identify reversal signals in different time periods
- Set a clear trading time range to avoid signals during non-trading hours
- Visually display the statistical results of the number of K lines in each segment to facilitate judgment of the effect
- There are many parameters that can be optimized, suitable for optimization for different markets
## Strategy Risk
- The statistical number of K lines cannot completely determine the trend reversal point, and there is a certain probability of misjudgment.
- A longer statistical time period is required, and short-term trading opportunities may be missed.
- Static thresholds are easily affected by market changes and need to be dynamically adjusted
- The choice of backtest time range will affect the results, and overfitting needs to be prevented.
Risks can be reduced by:
1. Optimize the parameters of K-line quantity and use different quantities for different periods.
2. Combined with other indicators validated_hvgggjhjj tjgtdfnjnjhggvft
3. Use dynamic thresholds to consider the impact of market fluctuations
4. Expand the backtest time range and conduct multiple backtest verifications
## Strategy optimization direction
This strategy can be optimized from the following aspects:
1. Optimize the number of K lines. You can test different parameters from 2 to 10 and choose the optimal parameters.
2. Optimize the inversion threshold. Can test 40% to 60% different parameters, taking into account market changes.
3. Add a stop loss strategy. You can set a stop loss point after forming a signal to control risks.
4. Combine with other indicators. For example, reversal signals can be verified by combining indicators such as RSI.
5. Add different varieties such as futures and foreign exchange. Test parameters for different trading varieties.
6. Perform step optimization. Adjust parameters step by step to find the optimal parameter combination.
7. Add machine learning model. Use algorithms to automatically find optimal parameters.
## Summarize
The multi-Bar co-directional strategy identifies potential reversal signals by statistically analyzing the probability of multiple K lines, achieving more accurate signal processing. However, the strategy effect is related to parameter selection and needs to be fully optimized. In addition, there is a certain possibility of misjudgment in the reversal signal itself, which needs to be verified in combination with other factors. Overall, this strategy is a simple and effective statistical strategy worthy of further research and optimization.
||
