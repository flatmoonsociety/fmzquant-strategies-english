...
## Overview
This strategy uses super trend indicators to determine price trends, and is equipped with a stop-loss mechanism for trend following transactions. It combines the difference between two moving averages to construct a super trend indicator. The indicator goes long when it crosses above and short when it crosses below. At the same time, set a stop loss line to avoid excessive losses.
## Strategy Principle
Calculate the DEMA of the fast line and the slow line, and determine the super trend direction based on the difference between the two lines. In an upward trend, the fast line crosses the slow line to go long; in a downward trend, the fast line crosses the slow line and goes short. After entering the market, set a stop loss line. Once the loss reaches the stop loss range, stop loss and exit.
## Advantage Analysis
- Super trend judgment of price trends is simple and effective
- The stop-loss mechanism effectively controls the risk of a single transaction
- Different parameter combinations can be used according to different stages
- Strong retracement control ability
## Risk Analysis
- There is a lag in judging the trend of the moving average
- Setting the stop loss point too loosely may amplify losses
- The transaction frequency is high and the handling fee burden is heavy
- Unable to accurately determine trend reversal points
You can appropriately shorten the average period, optimize the stop loss range, reduce the number of transactions, etc. to control risks.
## Optimization direction
- Test different combinations of fast and slow line parameters
- Optimize stop loss width parameters
- Consider combining with other indicators to determine trend reversal
- Test parameter robustness in different varieties
## Summarize
This strategy uses simple indicators to determine the trend direction and stop loss to control risks, and is suitable for intraday trading. The effect can be further improved through parameter tuning, etc.
||
