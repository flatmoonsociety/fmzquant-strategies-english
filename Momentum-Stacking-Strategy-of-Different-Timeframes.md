
> Name

Momentum-Stacking-Strategy-of-Different-Timeframes
> Author

ChaoZhang

> Strategy Description

![IMG](assets/images/5b136922482e5e702e39f922a684f8e965af5fae78f7c1c5fb715d0aecec59a9.png)

[trans]


### Overview
The Spring and Autumn Overlay Momentum Strategy is a strategy that mainly calculates the rate of change ROC of different periods and weights them proportionally to form a comprehensive momentum indicator to determine the direction of the market trend. This strategy superimposes short-term, medium-term and long-term momentum indicators to balance short-term and long-term trends and avoid false signals.
### Strategy Principles
This strategy first calculates the ROC indicators for different periods such as the 10th, 15th, and 20th, then smoothes the ROC, and performs weighting and superposition according to the ratio of 1-4. The calculation formula is as follows:
```
roc1 = (sma(roc(close,10),10)*1)  
roc2 = (sma(roc(close,15),10)*2)
...
osc = roc1+roc2+roc3+roc4+...
```

Among them, roc1-roc12 represent the calculation of ROC in different periods, corresponding to the 10-day, 15-day to 530-day periods respectively. Computes the Rate of Change (ROC) over the specified period.
Then perform a day (default 10 days) SMA smoothing on osc to obtain oscsmt.
Then compare the size relationship between osc and oscsmt. When osc crosses oscsmt above, it is a bullish signal and enters the long direction; when osc crosses below oscsmt, it is a bearish signal and enters the short direction.
Finally, there is the option to reverse the trade direction.
### Strategic Advantages
1. Superimposing short-term and long-term momentum indicators can capture short-term and long-term trends at the same time and avoid false signals.
2. By comparing osc and oscsmt through the price difference, unnecessary transactions in the flat area can be reduced.
3. You can customize parameters and adjust the period parameters for calculating ROC and the smoothing parameters of SMA.
4. You can choose to reverse the trading direction to meet different trading styles.
5. Visual indicators to intuitively judge buying and selling points.
### Strategy Risk and Optimization
1. The ROC indicator is very sensitive to sudden abnormal prices and may produce false signals. The SMA smoothing parameter a can be appropriately increased to reduce the sensitivity of the ROC indicator.
2. The default parameters may not be suitable for all varieties. It is necessary to optimize the parameters according to the characteristics of different varieties and find the best parameter combination.
3. Trading signals are generated based only on the price difference comparison between osc and oscsmt, which can be combined with other indicators to filter the signals to reduce the probability of wrong transactions.
4. This strategy is more suitable for medium and long-term trading, and the effect of short-term trading may not be good. The calculation cycle of ROC can be adjusted to optimize the usage scenarios of this strategy.
### Summarize
The Spring and Autumn Overlay Momentum Strategy calculates ROC indicators for multiple periods and superimposes them to obtain a comprehensive momentum indicator, which can take into account both short-term and long-term trends to avoid false signals. Compared with a single ROC indicator, this strategy can greatly improve signal quality and reliability. However, this strategy also has certain monitoring risks, and parameters need to be optimized and used in conjunction with other indicators to achieve maximum effectiveness.
||


## Overview

The Momentum Stacking Strategy mainly calculates the Rate of Change (ROC) over different periods, weights and stacks them to form a comprehensive momentum indicator for judging the trend direction. This strategy stacks short-term, intermediate-term and long-term momentum indicators to balance short-term and long-term trends and avoid false signals.

## Strategy Logic

The strategy first calculates the ROC indicators over 10-day, 15-day, 20-day periods, etc. Then smooths the ROC and stacks them in a 1-4 weighted ratio to get the formula:

```
roc1 = (sma(roc(close,10),10)*1)
roc2 = (sma(roc(close,15),10)*2)  
...
osc = roc1+roc2+roc3+roc4+...
```

Where roc1-roc12 represent ROC calculations over different periods from 10-day to 530-day. 

It then smoothes osc by a SMA of a (default 10) days to get oscsmt.

Compares osc with oscsmt, when osc crosses over oscsmt as the bullish signal and enters long. When osc crosses below oscsmt as the bearish signal and enters short.

Finally, it can choose to reverse the trading direction.

## Advantages

1. Stacking short-term and long-term momentum indicators can capture both short-term and long-term trends, avoiding false signals.

2. Comparing osc and oscsmt can reduce unnecessary trading in sideways markets. 

3. Customizable parameters to adjust ROC periods and SMA smoothness.

4. Reversible trading direction caters to different trading styles. 

5. Visual indicators make buying and selling points intuitive.

## Risks and Optimizations

1. ROC is very sensitive to sudden price abnormalities, which may generate wrong signals. Can increase the SMA smoothness to lower ROC sensitivity.

2. Default parameters may not suit all trading instruments. Need optimization to find the best parameter combination based on different characteristics.

3. Trades only based on osc and oscsmt crossover. Can add other indicators to filter signals and reduce errors.

4. More suitable for medium-long term trading. May need to adjust ROC periods to optimize usage scenario. 

## Conclusion

The Momentum Stacking Strategy calculates multiple ROC periods to get a comprehensive momentum indicator, capturing both short-term and long-term trends, avoiding false signals. Compared to a single ROC, it greatly improves signal quality and reliability. But it still carries some monitoring risks. Parameters need optimization and combining other indicators to maximize usefulness.

[/trans]

> Strategy Arguments



|Argument|Default|Description|
|----|----|----|
|v_input_1|10|Smooth|
|v_input_2_close|0|Source: close|high|low|open|hl2|hlc3|hlcc4|ohlc4|
|v_input_3|false|Trade reverse|


> Source (PineScript)

``` pinescript
/*backtest
start: 2023-09-25 00:00:00
end: 2023-10-25 00:00:00
period: 1h
basePeriod: 15m
exchanges: [{"eid":"Futures_Binance","currency":"BTC_USDT"}]
*/

//@version=2
////////////////////////////////////////////////////////////
//  Copyright by HPotter 08/08/2017
// Pring's Special K is a cyclical indicator created by Martin Pring. 
// His method combines short-term, intermediate and long-term velocity 
// into one complete series. Useful tool for Long Term Investors
// Modified for any source.
//
// You can change long to short in the Input Settings
// Please, use it only for learning or paper trading. Do not for real trading.
////////////////////////////////////////////////////////////
strategy(title="Martin Pring's Special K Backtest", shorttitle="UCS_Pring_sK")
a = input(10, title = "Smooth" )
sources = input(title="Source",  defval=close)
reverse = input(false, title="Trade reverse")
roc1 = (sma(roc(sources,10),10)*1)
roc2 = (sma(roc(sources,15),10)*2)
roc3 = (sma(roc(sources,20),10)*3)
roc4 = (sma(roc(sources,30),15)*4)
roc5 = (sma(roc(sources,40),50)*1)
roc6 = (sma(roc(sources,65),65)*2)
roc7 = (sma(roc(sources,75),75)*3)
roc8 = (sma(roc(sources,100),100)*4)
roc9 = (sma(roc(sources,195),130)*1)
roc10 = (sma(roc(sources,265),130)*2)
roc11 = (sma(roc(sources,390),130)*3)
roc12 = (sma(roc(sources,530),195)*4)
osc = roc1+roc2+roc3+roc4+roc5+roc6+roc7+roc8+roc9+roc10+roc11+roc12
oscsmt = sma(osc,a)
pos = iff(osc > oscsmt, 1,
	     iff(osc < oscsmt, -1, nz(pos[1], 0))) 
possig = iff(reverse and pos == 1, -1,
          iff(reverse and pos == -1, 1, pos))	   
if (possig == 1) 
    strategy.entry("Long", strategy.long)
if (possig == -1)
    strategy.entry("Short", strategy.short)	   	    
barcolor(possig == -1 ? red: possig == 1 ? green : blue ) 
plot(osc, color=blue, title="Martin Pring's Special K")
plot(oscsmt, color = red, title = "Smooth")
hline(0, title="Zero Line")
```

> Detail

https://www.fmz.com/strategy/430285

> Last Modified

2023-10-26 17:39:26
