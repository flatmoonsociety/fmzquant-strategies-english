...
![IMG](assets/images/37ae809701732cf2845669143df99a997846981635e12af1b42a6d14c3f6eaf4.png)
[trans]

## Overview
This strategy calculates multiple trend indicators and performs buy and sell operations when they reverse. The main trend indicators are TDI, TCF, TTF and TII. The strategy will choose which indicator to use in the configuration to generate trading signals.
## Strategy Principle
- ### TDI indicator
The TDI indicator is calculated based on price changes momentum. Built with summing and smoothing techniques. Buy when the TDI directional indicator crosses above the TDI curve, and clear when it crosses below.
- ### TCF indicator  
The TCF indicator calculates positive and negative price changes to determine the strength of bulls and bears. When the positive change force is greater than the negative change force, go long, otherwise clear the position.  
- ### TTF indicator
The TTF indicator determines trends by comparing the strength of highs and lows. The signal to go long is when the TTF indicator crosses 100, otherwise the position is cleared.
- ### TII indicator
The TII indicator combines moving averages and price ranges to identify trend reversals. It considers both short-term and long-term trends. The long signal is when the TII indicator crosses above 80, and the clearing signal is when the TII indicator crosses below 80.
Enter the logic of long and close positions to select appropriate trading signals based on the configured indicators.
## Strategic Advantages
This strategy incorporates a variety of commonly used trend trading indicators and can be flexibly adapted to the market environment. Specific advantages include:
1. Use trend reversal signals to capture trend change opportunities in a timely manner
2. Configure different indicators for targeted optimization
3. Rich indicator combinations that can be used in combination to confirm signals
## Strategy Risk
This strategy mainly faces the following risks:
1. Trading signals generated by trend indicators may cause false positives and lead to losses.
2. A single indicator cannot completely judge the trend and is easily affected by market noise.
3. Configuring incorrect indicator parameters and trading parameters may lead to misinterpretation of the market and incorrect transactions.
Risks can be reduced by:
1. Optimize indicator parameters and find the best parameter combination
2. Combine multiple indicator signals for trading to improve signal quality
3. Adjust position management strategies to control single losses
## Strategy optimization direction
This strategy can be optimized from the following aspects:
1. Test the optimal combination of indicators and parameters for different market cycles
2. Add or delete indicators to find the optimal indicator combination
3. Filter trading signals to remove false positive signals
4. Optimize position management strategies, such as variable positions, trailing stop loss, etc.
5. Add machine learning scoring indicators to assist in judging signal quality
## Summarize
This strategy combines the advantages of multiple trend reversal indicators and optimizes it by configuring indicators and parameters. It can adapt to different market environments and operate at trend reversal points. The key is to find the optimal combination of parameters and indicators while controlling risks. Through continuous optimization and validation, a strategy with stable alpha can be built.
||
