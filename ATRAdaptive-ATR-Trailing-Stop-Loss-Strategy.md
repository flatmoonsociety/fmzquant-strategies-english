...
[trans]
The name of this strategy is "Adaptive ATR Trailing Stop Strategy". This strategy uses the ATR indicator to set the stop loss point and changes the stop loss from tight to loose after entering the market. The purpose is to track the trend while controlling risks.
The specific logic of the strategy is as follows:
1. Calculate the highest price and lowest price range within a certain period as the entry signal. When the price breaks through this range, an entry signal is generated.
2. After entering the market, initially use a tighter ATR stop loss, and the stop loss point is fixed at 1.5 times the ATR value. This can limit losses after entry.
3. During the position holding phase, change the stop loss to a more relaxed ATR value of 4 times. The stop loss continues to track the price movement, but there is more room to allow the demand trend to continue to extend.
4. The final stop loss point always tracks the lowest price (long order) or the highest price (short order), and is adjusted with price fluctuations to achieve a trailing stop loss effect.
5. Stop loss and exit when the price falls below the stop loss point (long order) or exceeds the stop loss point (short order).
The advantage of this strategy is that it adopts an adaptive stop-loss mechanism, which not only ensures risk control but also avoids premature stop-loss. However, ATR parameters and multiples need to be optimized, and stop-loss strategies should be used in conjunction with trend judgment.
Generally speaking, dynamic trailing stop loss is an important means to improve the profit probability of the strategy. Flexible use of stop loss can better maintain trend profits and control risks.
||
