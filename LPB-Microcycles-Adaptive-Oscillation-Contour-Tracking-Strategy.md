...
Multi-time scale adaptive oscillation contour tracking strategy LPB-Microcycles-Adaptive-Oscillation-Contour-Tracking-Strategy
> Author
...
## Strategy Principle
1. Use HP filter to extract price trend lines. The HP filter extracts the long-term trend component of prices through optimization methods and filters out short-term fluctuation interference.
2. Calculate VWAP based on user-defined time range. VWAP can more accurately reflect the average price in different periods.
3. When the price is higher than the HP trend line, the long condition is met; when the price is lower than the HP trend line, the short condition is met. This captures bottom-up breakouts or top-down breakouts in price.
4. ATR stop loss combined with taking reasonable risks to avoid excessive losses.
## Advantage Analysis
1. Use HP filter to extract price trends, which is smoother than MA and other indicators to avoid being misled by short-term price fluctuations.
2. Customize the VWAP cycle to more flexibly adapt to market cycle changes.
3. Trade according to the trend direction, which is in line with the trend trading concept and has a high winning rate.
4. ATR stop loss controls a single loss to avoid excessive losses.
5. Various parameters can be adjusted with a large space and can be optimized for different markets.
## Risks and Countermeasures
1. Stop loss may be hit frequently during concussive trading. The stop loss range can be appropriately relaxed.
2. At the end of the trend, ClientRawResponse often appears to test for exploratory breakthroughs, which makes the strategy trapped. Other indicators should be used to judge the end of the trend and close positions in a timely manner.
3. Improper VWAP cycle setting may miss more effective trading opportunities. The VWAP cycle should be dynamically adjusted in conjunction with trend indicators.
## Optimization direction
1. The HP filter parameter λ can adjust the smoothing intensity. When the λ value is large, the trend line is smoother and more conducive to capturing long-term trends; when the λ value is small, it is more sensitive to price changes and is more suitable for capturing short- and medium-term opportunities.
2. The ATR multiple can adjust the stop loss range. It can be optimized with the λ parameter. When the λ value is large, the stop loss range can be appropriately expanded; when the λ value is small, the stop loss range can be narrowed to lock in more profits.
3. The risk-reward ratio (R:R) directly affects the profit-loss ratio. It can test retracement control and profitability under different multiple conditions.
## Summarize
The overall strategy is designed using trend tracking ideas. Through a variety of parameter settings, it can be optimized for different time periods of long, medium and short periods, with strong winning rate and profitability. Certain considerations have also been made in terms of risk control, which can effectively ensure that a single loss will not be too large. Generally speaking, this strategy uses a more scientific method to extract price trend characteristics, combined with the characteristics of a large parameter optimization space, and has good application prospects.
||
