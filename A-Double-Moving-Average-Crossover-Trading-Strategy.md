...
Based on double moving average crossover trading strategy-A-Double-Moving-Average-Crossover-Trading-Strategy
> Author
...
![IMG](assets/images/904bbbeb278fcd3de92a659ed1d0d869d3ee4214d395bb28ab45a3589579f7b4.png)
[trans]


## Overview
This strategy generates buy and sell signals based on the intersection of two moving averages with different parameter settings. When the shorter-period moving average breaks through the longer-period moving average from below, a buy signal is issued; when the shorter-period moving average breaks through the longer-period moving average from above, a sell signal is issued.
## Strategy Principle
The policy is written using pine scripting language. First, the type, length and price source of the two moving averages are defined through input, named p1 and p2 respectively. Among them, p1 represents the moving average with a shorter period, and p2 represents the moving average with a longer period.
Determine the intersection of two moving averages through the crossover and crossunder functions. When p1 breaks through p2 from below, a buy signal is issued; when p1 breaks through p2 from above, a sell signal is issued.
To implement a trade, the strategy opens a long or short position on a signal via the strategy.entry function. If the shortOnly parameter is enabled, only sell signals are traded.
## Advantage Analysis
This strategy has the following advantages:
1. The rules are clear and easy to understand and implement.
2. Moving average crossover is a classical and well-known trading signal
3. The type, length and price source of moving averages can be highly customized
4. You can only trade sell signals and reduce the trading frequency
## Risk Analysis
There are also some risks with this strategy:
1. In a volatile market, the moving average may have multiple invalid crossovers, leading to over-trading.
2. Parameters need to be optimized to adapt to different varieties and trading cycles
3. Unable to determine the direction of the trend, you may open a position against the trend
Invalid signals can be reduced by adjusting the length of the moving average and introducing filter conditions. It can also be combined with trend indicators to determine the market trend.
## Optimization direction
This strategy can be optimized from the following aspects:
1. Introduce Volume weighted average price or typical price as price source to improve the reliability of cross signals
2. Increase the Validation Period to avoid short-term error crossovers
3. Combined with ATR stop loss, set the maximum tolerable loss based on market fluctuations
4. Use curve fitting to optimize parameters and find the optimal parameter combination
5. Only consider issuing trading signals when the general cycle trend is consistent
## Summarize
This double moving average crossover strategy is easy to understand and implement as a whole. It forms a trading signal through the intersection of two moving averages and is highly customizable. However, it may also produce more invalid signals in a volatile market. Risks can be reduced through parameter optimization and rule optimization, and the optimization space is large and worthy of further research.
||
