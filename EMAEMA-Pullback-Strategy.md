...
EMA golden cross callback strategy EMA-Pullback-Strategy
> Author
...
![IMG](assets/images/69d58bc2158d1e97c65923715dfbc14c7150507185ff7f7b2f8b695a646bb9e1.png)
[trans]

## Overview
The EMA golden cross callback strategy is a quantitative trading strategy based on the EMA indicator. This strategy uses three EMA curves of different periods to construct trading signals, and combines the price callback mechanism to set stop loss and profit to achieve automated trading.
## Strategy Principle
This strategy uses three EMA curves, namely:
- EMA1: used to determine the support/resistance level of price correction, with a short period, the default is 33 periods.
- EMA2: used to filter out some reversal signals, the period is 5 times of EMA1, and the default is 165 periods.  
- EMA3: used to determine the overall trend direction, the period is 11 times of EMA1, and the default is 365 periods.
The generation of trading signals follows the following logic:
Bullish signal: The price pulled back after crossing EMA1constitutes, forming a higher low above EMA1, and the correction did not touch EMA2. After meeting the conditions, go long when it crosses EMA1 again.
Short signal: The price pulled back after crossing EMA1, forming a lower high below EMA1, and the correction did not touch EMA2. After meeting the conditions, go short when it crosses EMA1 again.
The stop loss method is the callback of the lowest price/highest price. Take profit is set to 2 times the stop loss.
## Strategic Advantages
This strategy has the following advantages:
1. Use the EMA indicator to construct trading signals, which has high reliability.
2. Combined with the price callback mechanism, it can effectively avoid being trapped.
3. The stop loss point is set at the previous high and low levels to effectively control risks.
4. Set the take-profit point according to the take-profit and stop-loss ratio to meet the profit-loss ratio requirements.
5. EMA parameters can be adjusted according to the market to adapt to different cycles.
## Strategy Risk
This strategy also has certain risks:
1. The EMA indicator lags behind and may miss the trend reversal point.
2. The callback range is too large and exceeds EMA2 may generate false signals.
3. Stop loss may be breached in trending markets.
4. Improper parameter settings may lead to too frequent transactions or missed opportunities.
Parameters can be optimized by adjusting the EMA period, callback limit range, etc. Signals can also be filtered in combination with other indicators.
## Strategy optimization direction
This strategy can also be optimized from the following aspects:
1. Add trend indicator judgment to avoid counter-trend trading. For example, add MACD.
2. Add trading volume indicators to avoid false breakthroughs. For example, add OBV.  
3. Optimize EMA cycle parameters, or use adaptive EMA.  
4. Combined with machine learning methods such as bag-of-words models to dynamically optimize parameters.
5. Add model prediction and set adaptive stop loss and take profit.
## Summarize
The EMA golden cross callback strategy realizes automated trading by constructing a three-EMA trading system and setting stop-profit and stop-loss combined with the price callback characteristics. This strategy effectively controls trading risks and can be optimized by adjusting parameters according to the market. Overall, the strategy is logically reasonable and can be applied practically. In the future, further optimization can be carried out in terms of trend judgment, parameter optimization, risk control, etc.
||
