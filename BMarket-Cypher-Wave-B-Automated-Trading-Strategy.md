...
## Strategy Principle
This strategy is mainly based on the Market Crypto Wave B indicator. This indicator divides the price trend into two levels: wave 1 and wave 2. The 1st wave response is sensitive and the 2nd wave response is smooth. Their crossover is a buy or sell signal. When wave 1 crosses above wave 2, it is a buying opportunity, and when it crosses below, it is a selling opportunity.
In the code, wave 1 and wave 2 are calculated through the Wavetrend function. Then combine overbought and oversold to distinguish the wave direction. For example, when Wave1 crosses Wave2 and Wave2 is in the overbought zone, it is a sell signal.
In addition, the code also introduces a variety of auxiliary indicators to improve decision-making accuracy, including:
- Extreme value area: combine RSI and capital flow indicators to determine the price fluctuation area.
- KD: Determine the long and short momentum
- Divergence: Find the buying and selling turning point
- Flag shape: determine the continuity of the trend
Finally, the final trading decision is judged based on the Wave and auxiliary indicators.
## Strategic Advantages
The biggest advantage of this strategy is its ability to respond to different market environments. Specifically, it is mainly reflected in the following aspects:
1. Multiple indicator combinations, high judgment accuracy
The strategy not only uses the crossover of the Wave itself as the judgment criterion, but also introduces multiple indicators such as RSI, KD, and divergence to improve the accuracy of decision-making through combination.
2. Judgment of flag shapes and good at catching trends
By introducing the flag judgment module, it can effectively judge the continuity of the trend and avoid being top-heavy, so that opportunities can be identified at the beginning of the trend. This makes the strategy particularly good at trend following.
3. There is a lot of room for manual parameter optimization
The strategy parameters are highly customizable. Users can choose different parameters for optimization based on their own judgment and understanding of the market, thereby adapting to more complex market environments. This makes the strategy more broadly applicable.
## Strategy Risk
This strategy also has some potential risks, mainly reflected in the following aspects:
1. Judgment conflicts between multiple auxiliary indicators
There are many auxiliary indicators, and under certain extreme market conditions, there may be conflicts in indicator judgments. This creates uncertainty in the final decision.
2. Parameter tuning is difficult
Since there are many adjustable parameters, achieving the optimal parameter combination requires a large amount of data backtesting and testing, which is difficult and prone to over-optimization.
3. The buying and selling frequency may be too high
In volatile markets, wave crossovers may occur frequently, resulting in too frequent transactions. This creates additional transaction costs.
Correspondingly, it can be optimized from the following aspects:
1. Introduce more filtering conditions to ensure the robustness of decisions
2. Prioritize adjustments to parameters that have a large impact on the strategy
3. Increase the stop loss range and reduce the frequency of buying and selling
## Summarize
In general, this strategy comprehensively uses the market crypto wave theory and a variety of technical indicators to achieve the ability to automatically identify buying and selling points and track trends. It has strong parameter adjustability, wide adaptability, and is good at capturing trend opportunities. At the same time, we also need to pay attention to certain potential risks, and we need to continuously optimize the judgment rules and adjust parameters to make the strategy more robust.
||
