...
![IMG](assets/images/0d0d9b229f9f9145bc22e824a5ed1ccca966bac2dff9d2a67398a7006d075013.png)
[trans]

## Overview
This strategy uses multi-period adaptive moving averages and broken line risk evaluation indicators, combined with parting points in market operation to predict trends and output trading signals.策略名称为“多周期自适应走势预测策略”。
## Strategy Principle
The core logic of the strategy is divided into two main lines. The first main line is to use the adaptive moving average nAMA with different parameters to build a multi-period filtering judgment system, and the second main line is to use the polyline risk evaluation system out with different parameters to build a multi-period risk evaluation system.最后将两条主线进行组合,当短周期线超过长周期线时产生买入信号。 A sell signal is generated when the short period line is below the long period line.

此外,策略中还设置了量价确认模块。仅在成交量大于6周期平均成交量时,交易信号才有效。 This can filter out false signals to a certain extent.最后,策略中绘制不同周期级别的分型点,作为参考信号。
## Strategic advantage analysis


## Strategy risk analysis
The biggest risk of this strategy is that it requires the judgment of multiple cycle lines in the same direction, so its ability to capture the short-term reversing market is poor. When short-period lines and long-period lines change at the same time, ambiguous signals are easily produced.可以通过缩短参数周期进行优化。
In addition, the main cycles of the adaptive moving average and broken line risk evaluation systems are shorter than the broader market, and there is a certain lag when driven by longer levels.可以适当延长主力周期降低滞后性。
## Strategy optimization direction
1. Shorten the main period parameters of the adaptive moving average and polyline risk evaluation line to 5 and 20 periods to improve the capture of short-term market reversing.
2. Increase the main cycle parameters to 20 and 50 cycles to reduce the lag driven by longer-level markets.
3. Optimize the trading channel parameters and change it to 0.5 times ATR channel to reduce the possibility of noise trading.
4. Add MACD and other result verification indicators to improve signal reliability.
## Summarize
This strategy comprehensively uses a variety of technical indicators such as adaptive moving averages, polyline risk assessment, and volume and price analysis to build a multi-period adaptive trading decision-making system. Through the optimization and adjustment of parameters, it can be applied to different types of markets and automatically identify trends and areas. The strategy logic is clear and the value can be mined. It is a quantitative method worth recommending.
||
