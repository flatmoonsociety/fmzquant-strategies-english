...
Moving-Average-Bollinger-Bands-Breakout-Strategy
> Author
...
![IMG](assets/images/9e674e244cc99b435763cf425e63da6da1da28f129c3dabe60b9f64b769dd5f4.png)
[trans]

## Overview
This strategy combines the moving average indicator, Bollinger Bands indicator and UT Bot Alerts indicator to implement a simple breakout operation strategy. When the price breaks through the upper Bollinger Band, go long; when the price breaks through the lower Bollinger Band, go short.
## Strategy Principle
1. Use the 200-period EMA as the central axis to determine the trend. Prices above the EMA are bullish, and prices below the EMA are bearish.
2. The UT Bot Alerts indicator is combined with ATR to generate buy and sell signals. A long signal is generated when the price crosses the upper Bollinger Band with the fast EMA; a short signal is generated when the price crosses the lower Bollinger Band with the fast EMA.
3. The ATR stop loss indicator is used to set the stop loss point. The stop loss distance is 1.5 times the ATR value.    
4. After entering the market, set the stop loss point, take profit point and stop loss to the entry price based on the risk-reward ratio.
## Advantage Analysis
1. Use the Bollinger Bands indicator to determine when it is appropriate to go long and short, which can increase the probability of profit.
2. The UT Bot Alerts indicator can produce more accurate signals.
3. Using risk-return ratio to stop loss and stop profit can effectively control risks.
## Risk Analysis
1. Bollinger Bands are prone to produce false signals in volatile markets.
2. ATR has hysteresis, and the stop loss distance may be too large at the beginning of the trend. 
3. Improperly setting the risk-reward ratio can also lead to being too aggressive or too conservative.
## Optimization direction
1. You can try to use other indicators instead of the UT Bot Alerts indicator.
2. The period and multiple of ATR can be optimized to make the stop loss distance more appropriate.
3. You can test different risk-reward ratios and find the best parameters.
## Summary
This strategy integrates the advantages of multiple indicators and has strong practicality. Through parameter optimization, it can become a stable and reliable breakthrough system. However, attention must also be paid to preventing risks caused by indicator failure and inappropriate parameters.
