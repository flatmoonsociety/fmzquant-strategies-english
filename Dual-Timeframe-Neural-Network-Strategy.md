...
## Strategy Principle
This strategy uses a neural network prediction model to determine price trends in two time frames and trade when the dual timeline signals are in the same direction.
The specific logic is:
1. Obtain the price changes of two timelines, such as the daily line and the 1-hour line
2. Input the price changes into the neural network for training and obtain the prediction output of each axis.
3. When the prediction outputs of the two axes are in the same direction, that is, exceeding the threshold, a trading signal is generated.
4. When the daily forecast is long, and when the 1-hour forecast is long, carry out long transactions.
5. When the daily forecast is short, and when the 1-hour forecast is short, perform short trading.
6. When the predictions of the two axes are inconsistent, close the position
This strategy makes full use of dual timeline information, judges the trend direction in multiple dimensions, and then conducts transactions, which can effectively reduce false signals.
## Strategic Advantages
-Dual timeline prediction to improve judgment accuracy
- Neural networks model complex data
- Trade in the same direction to avoid being trapped
## Strategy Risk
- Requires large amounts of data for network training
- Network structure design requires repeated testing
- Dual-axis combined signals appear less frequently
## Summarize
This strategy uses neural networks to judge price trends from dual timelines and conduct transactions while ensuring the accuracy of the judgment. However, it needs to optimize network parameters and set a reasonable transaction frequency. Overall, it can provide a more stable trading direction guidance.
||
