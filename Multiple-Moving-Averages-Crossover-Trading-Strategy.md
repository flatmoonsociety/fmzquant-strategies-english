...
![IMG](assets/images/be82050e01656289b5f8f5ecc449be1e58bfafc5a278cd7877c27fa34f4cc66f.png)

[trans]

### Overview
This strategy is a trading strategy based on the intersection of three moving averages (MA1, MA2, MA3). Flexible trading strategy combinations can be obtained by setting the type, period, price data source and resolution of the three moving averages, and whether to allow trading of crossovers between them.
### Principle
This strategy mainly uses the crossover and dip signals generated between three moving averages as trading signals. When the shorter period moving average crosses the longer period moving average from bottom to top, a long position opening signal is generated; and when the shorter period moving average crosses the longer period moving average from top to bottom, a closing signal is generated.
Users can freely choose the type of three moving averages (SMA, EMA, etc.), period, price data source (closing price, highest price, etc.), and K-line resolution (minute line, daily line, etc.). You can also choose whether to open trading for each moving average crossover to decide whether to take trading action on certain crossovers.
This strategy is currently long-only and uses market orders to open and close positions. The capital invested in each transaction is 100% of the total equity of the account.
### Advantages
1. You can freely select the type, period and other parameters of the moving average for optimization and combination to reduce the risk of curve fitting.
2. Multiple moving average crossovers can form multiple trading opportunities and increase trading frequency.
3. Using long, medium and short period moving averages at the same time can balance the trend and reversal
4. Supports different K-line resolutions and can conduct multi-time frame analysis
5. Comes with a prediction function that can test the parameter fitting effect
### Risk
1. A large number of parameter combinations may lead to over-optimization
2. Higher transaction frequency may increase transaction fees and slippage costs
3. Market orders cannot limit entry points.
4. Multiple moving averages may have conflicting signals
5. There may be differences between backtest and actual performance
### Optimization suggestions
1. Use walks forward analysis to obtain the effective parameter range
2. Add transaction fees and slippage costs to backtesting
3. Try limit orders instead of market orders
4. Add filter conditions to avoid conflicting signals
5. Validate strategies in backtests that simulate real environments

### Summarize
This strategy combines the smoothing properties of moving averages with the pattern recognition capabilities of crossover signals. Users have the flexibility to choose parameters to balance trend following and reversal identification. At the same time, attention needs to be paid to controlling the risk of over-optimization and verifying the robustness of the strategy in a complex market that simulates the real environment. Overall, this strategy provides an effective example of trading with multiple moving averages.
||
