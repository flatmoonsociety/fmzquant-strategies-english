...
## Strategy Principle
This strategy incorporates a neural network model, RSI indicator and Super Trend indicator for trading.
The specific logic is:
1. Construct a neural network model and input multi-dimensional data including trading volume change rate, Bollinger Bands, RSI and other
2. Network prediction to obtain future price change rate
3. Calculate the RSI indicator value and combine RSI with the predicted price change rate
4. Generate dynamic stop loss line based on RSI indicator value
5. When the price falls below the upward stop loss line, go short; when the price breaks through the downward stop loss line, go long
6. Filter based on the trend judgment of the super trend indicator
This strategy makes full use of the neural network's ability to simulate complex data, and supplements it with indicators such as RSI and super trends for signal verification, improving the accuracy of judgment while controlling trading risks.
## Strategic Advantages
- Neural network models multi-dimensional data to determine trends
- RSI stop loss protects profits and super trend assists judgment
- Multi-indicator combination verification to improve signal quality
## Strategy Risk
- Requires large amounts of data for neural network training
- RSI and super trend parameters need to be optimized and adjusted
- The effect depends on model judgment and there is uncertainty
## Summarize
This strategy uses machine learning technology supplemented by traditional indicator judgments to control risks while pursuing high efficiency. However, its parameter adjustment and model interpretability still need to be improved.
||
