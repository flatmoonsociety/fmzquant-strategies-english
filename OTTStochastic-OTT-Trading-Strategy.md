...
![IMG](assets/images/099a5c520a5a61cc171f690a6a80fcd8fbb07c169e3f89b7b1193a4d645c56cc.png)

[trans]


### Overview
This strategy combines the Stochastic and OTT indicators to generate trading signals. When the fast OTT line and the slow OTT line cross, a trade entry is triggered. In order to filter out false signals, the strategy adds a verification mechanism for the stochastic indicator.
### Strategy Principles
1. Calculate fast OTT lines and slow OTT lines. Calculated based on fast average and slow average respectively, combined with a certain percentage of stop loss.
2. Calculate the stochastic indicator. Calculated based on the highest price, lowest price and closing price of K line.
3. When the fast OTT line and the slow OTT line cross, determine the long and short directions. Also check if the stochastic indicator validates the signal.
4. Determine entry based on the cross situation and direction.
### Advantage Analysis
1. The OTT indicator itself has a good reversal effect and has a strong ability to capture turning points.
2. The stochastic indicator filters out false signals to avoid being caught in shocks.
3. The average line type of OTT line calculation can be customized to flexibly respond to the market.
4. Take-profit and stop-loss points can be set, and risks are controllable.
### Risk Analysis
1. Improper parameter settings may lead to excessive trading frequency or signal deviation.
2. In trending markets, the OTT indicator may produce false signals.
3. The macro-cyclical trend needs to be comprehensively considered.
### Optimization direction
1. Optimize the parameter combination and find the best parameter pair.
2. Use trend indicators to determine the effective period of the strategy.
3. Add fund management module.
### Summary
This strategy integrates the advantages of the OTT indicator to judge short-term reversal and the stochastic indicator to filter signals, which can effectively control risks and is suitable for reversal or shock markets. However, you need to be careful about entering a trend market or choosing a cycle by mistake. Further improvements can be made from aspects such as parameter optimization and fund management.
||
