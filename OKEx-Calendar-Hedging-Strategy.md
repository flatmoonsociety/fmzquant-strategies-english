...
OKEx Calendar Hedging Strategy
> Author
...
How to perform intertemporal hedging on Okex?
There are often price differences in the prices of current week, next week and quarter contracts on okex. If the price difference reaches or exceeds a certain threshold, you can perform intertemporal hedging, and then reversely close the position when the price difference disappears, and then use the reasonable return of the price difference to obtain profits. For example, there is a price difference between BTC's current week contract and the next week's contract and the current week's contract is lower than the next week's contract price. When the price difference reaches the set threshold, investors can go long on the current week's contract and short on the next week's contract (the same quantity) for hedging, and wait until the price difference between the current week's contract and the next week's contract returns to the normal value to perform corresponding reverse closing to obtain profits.
Risks of intertemporal hedging:
Because the delivery times of the two contracts are different, when the near-term contract is forced to deliver, losses may occur if the price difference fails to return.
Functions and features of strategy implementation:
Supports Okex intertemporal hedging
Supports Okex’s current week, next week and quarterly contracts
Supports all Okex contract trading varieties (BTC, BCH, EOS, BSV, ETH, etc.)
Special note: This strategy requires the Botvstools template library to run!
Please go here to download the template library:
https://www.pcclean.io/45gd
(After downloading the zip file and decompressing it, there will be two js files, one is the strategy and the other is the template library, please pay attention to the distinction)
