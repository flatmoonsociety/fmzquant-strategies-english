...
![IMG](assets/images/df6339fae0e4d04c65bf8915e8f4d51f0fadc0b34d78d9382ca9e12c7c20b959.png)
[trans]

## Overview
The Turtle Trading Decision System is a trend following trading strategy based on breakout theory. It generates trading signals through the moving average of the highest price and lowest price of the trading variety to realize the identification of potential trends. A trading signal is generated when the price breaks through the highest or lowest price in a specified period. This strategy also combines the trailing stop loss, position management and fund management modules, making it a relatively complete decision-making system.
## Strategy Principle
The core strategy signal of the Turtle trading decision-making system is generated by comparing the price relationship with the highest price in the N1 period and the lowest price in the N2 period. When the price goes above the highest price in the N1 period, a buy signal is generated; when the price goes below the lowest price in the N2 period, a sell signal is generated. Shutdown mode is used to control new signal generation.
After opening a position, the relationship between the price and the stop loss price will be compared in real time to generate a trailing stop loss signal. At the same time, the relationship between the price and the position increase line will also be compared to generate a position increase signal. The stop loss price and position increase line are both related to ATR.
The position units are calculated each time a position is opened, and a certain proportion of the initial capital is taken to avoid the impact of a single loss on the total capital. A single loss is limited to a certain range.
## Advantage Analysis
The Turtle trading decision-making system has the following advantages:
1. Capture potential trends: By comparing the relationship between price and the highest and lowest prices of the cycle to determine the direction of potential trends, potential price trends can be captured early.
2. Risk management: Use capital management and stop loss to control single and overall loss risks.
3. Adding positions management: Appropriate adding positions can obtain extra profits in the trend.
4. Completeness: It combines fund management, stop loss management and position addition management to make the decision-making system more complete.
5. Simple and clear: The signal generation rules are simple and direct, easy to understand and verify.
## Risk Analysis
There are also some risks in the Turtle trading decision-making system:
1. Risk of false breakthrough: The price may falsely break through the highest price or lowest price, causing a false signal. The parameters can be appropriately adjusted to filter out some false breakthroughs.
2. Trend reversal risk: There is a risk of increased losses due to price reversal after adding a position. The number of positions added should be appropriately limited and losses should be stopped in a timely manner.
3. Parameter optimization risks: Parameter settings in different markets will be quite different. Parameters should be optimized by market to reduce risks.
## Optimization direction
The Turtle trading decision-making system can also be optimized from the following aspects:
1. Add a filter: detect the strength of price breakthroughs and filter out some false breakthroughs.
2. Optimize the stop loss strategy: how to track the stop loss reasonably and find a balance between protecting profits and reducing unnecessary stop losses.
3. Market-specific parameter optimization: optimize parameter combinations according to the characteristics of different varieties.
4. Add machine learning: Use machine learning algorithms to help determine the trend direction.
## Summarize
The turtle trading decision-making system determines the potential trend direction by comparing the relationship between the price and the highest and lowest price within the specified period, and combines the risk management module to build the entire decision-making system. It has strong trend tracking capabilities, but also has certain false breakthrough risks and parameter optimization problems. This strategy can be used as a basic model for quantitative trading, and can be expanded and optimized on this basis to develop a decision-making system suitable for you.
||
...
|----|----|----|
|v_input_1|timestamp(2013-01-01T00:00:00+08:00)|From Date/Start Date|
|v_input_2|timestamp(2024-08-01T00:00:00+08:00)|To Date/End Date|
|v_input_string_1|Mode 1|Enter Mode/Enter System|
|v_input_int_1|20|Entry Length/System 1 entry length|
|v_input_int_2|10|Exit Length/System 2 exit length|
|v_input_int_3|55|Mode2 Entry Length/System 2 entry length|
|v_input_int_4|20|Mode2 Exit Length/System 2 exit length|
|v_input_int_5|14|ATR Period/Period for calculating ATR|
|v_input_float_1|0.02|Risk Per Trade/The risk of each transaction, 0.02 is 2%|
|v_input_float_2|2|Initial Stop ATR Multiple/ATR multiple used for stop loss|
|v_input_float_3|0.5|Pyramid ATR Multiple/ATR multiple used to add positions|
|v_input_int_6|4|Max Units/Maximum number of position units|
|v_input_3|true|Highlighter On/Off ?/Whether to highlight|
