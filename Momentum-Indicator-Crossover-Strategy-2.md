...
## Strategy Principle
This strategy uses 3 parameters to calculate the RSI indicator and its simple moving average of two different periods. First, calculate the conventional RSI indicator with a period of length. Then calculate the length2 period simple moving average SMA_RSI of RSI. Finally, calculate the difference delta between RSI and SMA_RSI, and then calculate the simple moving average SMA_RSI2 of length3 period for delta. When SMA_RSI2 crosses the threshold set by the user, a long transaction is performed; when SMA_RSI2 falls below the threshold, the position is closed.
In this way, a trading strategy signal based on the RSI indicator moving average crossover is formed. Since SMA_RSI2 is the moving average of the difference delta, it can reflect the momentum and changing trend of the RSI indicator and grasp the essence of the RSI indicator itself.
## Advantage Analysis
This strategy combines the advantages of the RSI indicator and its moving average, which can follow the price trend and avoid being misled by noise. The idea of ​​​​difference delta smoothing is adopted to make the trading signal clearer. Generally speaking, this strategy has a smaller retracement and stable profits.
The specific advantages are as follows:
1. Use difference delta to smooth price fluctuations and reduce false signals
2. The form of moving average crossover is simple, direct and easy to master.
3. There are many adjustable parameters and can be adjusted according to the market
4. Stable profits and small retracement
## Risks and improvements
This strategy also has some risks, mainly reflected in:
1. In sharp market conditions, stop loss may be larger
2. It is easy to make unstable profits in a volatile trend
Improvements can be made from the following aspects:
1. Optimize parameters and improve stability
2. Add a stop-loss mechanism to control single losses
3. Combine with other indicators to improve signal quality
## Summarize
This strategy is relatively simple and versatile overall. It increases the practicality of the RSI indicator itself through difference calculation, uses moving average crossovers for judgment, and has strong retracement control capabilities. It is a very practical momentum indicator strategy.
||
