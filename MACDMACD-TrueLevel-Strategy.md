...
## Overview
The MACD True Level Strategy uses the MACD indicator and the True Level Band indicator to determine buy and sell signals. Based on MACD, it adds the use of true level bands, which can more accurately locate the beginning and end of the trend.
## Principle
The strategy first calculates the MACD indicator and then calculates the true level bands for 14 different periods. True level bands are calculated based on the linear regression line of price and the standard deviation of periods of different lengths. Then select the highest upper rail and the lowest lower rail to determine whether there is a breakthrough as a buying and selling signal.
Specifically, the entry signal of the strategy is the golden cross of the MACD indicator or the true-level lower track selected by the price breakthrough, and the exit signal is the true-level upper track selected by the MACD indicator dead cross or price breakthrough.
The advantage of using true level bands is that it combines the direction and fluctuation range of the trend, allowing for a more accurate judgment of the beginning and end of the trend. Compared to using MACD alone, adding true level band filtering can reduce false signals.
## Advantage Analysis
1. Use MACD to determine the trend direction and track the trend effectively.
2. Add true level indicators to filter out false breakthroughs and confirm trends. The true level band takes into account both the price trend and the fluctuation range, making the judgment more accurate.
3. The true level band contains multiple periods and can comprehensively judge trend changes.
4. Adjustable true level parameters to adapt to different market environments.
5. Supports long and short operations, allowing you to capture trend opportunities in all directions.
## Risk Analysis
1. As a single indicator, MACD may produce many false signals and requires true level band filtering.
2. Improper setting of true band parameters may lead to leakage. Parameters need to be adjusted according to the market.
3. Long and short operations require sufficient financial support, otherwise the position may be liquidated.
4. Breakthrough trading itself carries the risk of arbitrage, and losses need to be stopped in time.
5. Any indicator strategy has the risk of not being applicable to highly volatile products such as agricultural products and digital currencies.
## Optimization direction
1. Test more cycle parameters and find parameters that are more suitable for the market.
2. Adding a stop-loss strategy can reduce the risk of loss.
3. Combining other indicators to filter entry, such as trading volume, KDJ, etc., can reduce false breakthroughs.
4. Optimizing entry point selection, such as breaking through high points, low points, etc., can increase profitability.
5. Add machine learning algorithms, use model training to determine trend probability, and reduce manual parameter adjustments.
## Summarize
The MACD true level strategy integrates trend judgment and range judgment. On the basis of MACD, the true level band indicator is added to more accurately locate the beginning and end of the trend. Compared with a single MACD strategy, it can effectively filter out false signals and capture trend opportunities. At the same time, there are certain risks, and parameters and stop losses need to be optimized. Generally speaking, this strategy is suitable for trend trading and can stably capture market trends in the medium to long term.
||
