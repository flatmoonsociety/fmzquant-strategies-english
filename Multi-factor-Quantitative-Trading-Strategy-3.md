...
1. Momentum indicators: Parabolic SAR, Leledc strength indicator, Kaufman adaptive moving average, etc.
2. Trend indicators: RahulMohindar Oscillator, Trend Magic, etc.
3. Ichimoku cloud map: including Tenkan line, Kijun line, etc.
4. Volume Flow Indicator: Volume Flow Indicator
5. Wave indicator: Wave Trend Oscillator
6. TD sequence
These indicators judge current market trends and strength from different perspectives. Parabolic SAR determines the trend reversal point, Leledc strength indicator determines momentum, and Ichimoku cloud chart determines support pressure. When most indicators give signals in the same direction, a final buy or sell judgment is formed.
This strategy also sets filter conditions to only conduct transactions within the specified date range every month and day, thereby reducing the number of invalid transactions.
## Advantage Analysis
- Comprehensive judgment of multiple factors to improve accuracy and have strong risk resistance ability
- Use different types of indicators for cross-validation to avoid the risk of a single indicator failing.
- Set filter conditions to avoid invalid transactions during inappropriate time periods
- Written in Pine Script, it can be used directly on the TradingView platform, which is convenient and fast
- Indicator parameters are adjustable and can be optimized for different markets
- Visually display indicator signals and intuitively judge the market structure
## Risk Analysis
- Multi-factor combinations require adjustment of weights and parameters, and there is a certain degree of optimization difficulty
- A single indicator may fail under certain market conditions
- Improperly set filter conditions may miss opportunities
- Care needs to be taken to avoid over-optimization
- Traders need to pay attention to the risk of indicator failure and adjust strategies in a timely manner
Countermeasures:
- Optimize and adjust indicator parameters to make them more effective for the current market
- Adjust the weight to increase the effect of effective indicators and reduce the effect of invalid indicators
- Adjust filtering conditions in a timely manner to take into account opportunities and avoid risks
## Optimization ideas
- Add machine learning algorithm to automatically adjust indicator weights
- Add sentiment indicators, capital flow indicators and more factors
- Test trading varieties and time periods, and set optimal parameters
- Test the effects of different holding times
- Combine more filters such as seasonality, economic data, etc.
- Add stop loss strategy
## Summarize
This strategy combines multiple indicators to form a final judgment and has the advantage of strong risk resistance. At the same time, we also need to pay attention to the risk of single indicator failure and continue to optimize and adjust parameters. In the future, we can further optimize the indicator weight setting, add more factors, test the optimal holding period, etc.
||
