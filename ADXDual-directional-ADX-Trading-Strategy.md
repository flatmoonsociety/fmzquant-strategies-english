...
### Strategy Principles
1. Calculate the true range (True Range)
2. Calculate the long directional movement (Directional Movement Plus) and short directional movement (Directional Movement Minus)
3. Calculate the smoothed true range (Smoothed True Range)
4. Calculate Smoothed Directional Movement Plus and Smoothed Directional Movement Minus
5. Calculate DIPlus, DIMinus and ADX indicators
6. Calculate the difference between DIPlus and ADX, DIMinus and ADX
7. Set the difference threshold for long and short transactions
8. When the difference is greater than the threshold, a trading signal is generated
9. Generate buy and sell orders
The core of this strategy is to use trend index indicators such as ADX to determine the direction and intensity of the trend, and combine it with the difference determination rule to set thresholds for automatic trading.
### Advantage Analysis
1. Use ADX to determine the trend direction and accurately capture the market trend.
2. Apply the difference judgment rule to effectively filter out false signals
3. Two-way trading can fully capture long and short opportunities
4. Fully automatic trading, no manual intervention required
...
2. The risk of two-way transactions increases, and losses may expand.
3. Improper setting of parameters may lead to over-trading
4. Backtest data cannot represent the real market, and real market risks still exist.
Solution:
1. Combine with other indicators to confirm trading signals
2. Optimize parameters and control transaction frequency
3. Strictly manage trading positions with Position Sizing
### Optimization direction
1. Optimize ADX parameters and improve its sensitivity
2. Add other indicators to filter signals
3. Apply machine learning algorithms to optimize parameters
4. Use advanced stop-loss strategies to control loss risk
5. Combine with model predictions to obtain more accurate trading signals
### Summarize
The two-way ADX trading strategy overall is a very practical quantitative strategy. It uses the ADX indicator to determine trends and capture trading opportunities in both directions. At the same time, difference judgment is applied to ensure signal validity. The logic of this strategy is clear and simple, easy to modify and optimize, and it is a two-way trend following trading strategy. Through reasonable parameter optimization, application of stop loss strategies and signal filtering, the stability and profitability of the strategy can be further enhanced.
||
