...
Y-Profit-MaximizerStrategyY-Profit-Maximizer-Strategy
> Author
...
![IMG](assets/images/467a089ec878bda768795cf2f3ad58c857db6ade203e9cf9e73509a9e72a960a.png)

[trans]


## Overview
The core idea of ​​this strategy is to use trailing stop loss to track value maximization, supplemented by filters and take-profit methods to optimize entry. The Y shape in the strategy name represents the crossover pattern of one buy and one sell signal line of the strategy.
## Strategy Principle
This strategy is mainly based on KivancOzbilgic's PMax explorer strategy, with some modifications.
1. Calculate PMax based on ATR and moving average. Generate a Buy signal when price moves above PMax.
2. Add T3 indicator and price as filters to ensure entry when the trend is upward.
3. Set the take-profit method: first set the double BAND strategy to determine the first take-profit; then use the dice strategy to determine the subsequent take-profit and stop-loss points.
4. The MOST indicator assists in judging trends to reduce unnecessary reverse operations.
## Advantage Analysis
1. The PMax strategy itself has the advantage of avoiding chasing high stop loss, and the moving stop loss mechanism is more conducive to reducing DD.
2. The double filter ensures that you only enter the market during an upward trend to avoid false breakthroughs.
3. Multiple take-profit point settings make profits more flexible.
4. The MOST indicator ensures that only long operations are performed and reverse transactions are avoided.
## Risk Analysis
1. PMax itself has a certain degree of hysteresis and it is easy to miss the first breakthrough.
2. Setting too many filters may also miss the golden entry point.
3. If the take-profit setting is too optimistic, the order will not be fully filled.
4. It is difficult to make a profit by doing long trading alone in products with high volatility.
## Optimization direction
1. You can test the MACD-like indicator to judge short-term deviations to determine the timing of entry.
2. Simplified filters can be tested and only one filter indicator is retained.
3. You can add an automatic adjustment take-profit mechanism to dynamically adjust subsequent take-profit points based on volatility and rate of return.
4. You can test the allowed short selling and adjust the position ratio according to the filter.
## Summarize
This strategy takes PMax as the core for entry judgment, and designs multi-layer filters and profit-taking methods for optimization, so that better returns can be obtained in trending varieties. In the later stage, by simplifying filters, optimizing take-profit settings, and appropriately adjusting position management, the strategy Parameter can be optimized and matured to adapt to more varieties, and achieve better performance in real trading.
|| 
