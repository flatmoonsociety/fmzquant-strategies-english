...
### Strategy Principles
This strategy uses the 6-day simple moving average and the 35-day simple moving average to define two moving averages. The buy signal line is defined as a 2-day exponential moving average, and the sell signal line calculates the slope and then shifts based on the closing prices of the past 8 days. In addition, the exponential moving average of 20-day trading volume is also defined as a trading volume indicator. In order to filter out some noise, the strategy also introduces weekly slope long and short judgment.
When the closing price of the stock is higher than the 35-day moving average, the trading volume is higher than the 20-day trading volume average, and it is a long market on a weekly basis, a buy signal is triggered from the golden cross below; conversely, a sell signal is triggered from the death cross above.
In terms of risk management, the strategy introduces a dynamic position adjustment mechanism. Calculate the actual position based on account equity, maximum position ratio, ATR and risk factors. This helps control the maximum drawdown of the strategy.
### Advantage Analysis
This strategy combines momentum factors and trend filtering to effectively identify medium and long-term directions. At the same time, the filtering of noise is also relatively in place, which is helpful to avoid false signals in volatile market conditions. In addition, the introduction of the risk management mechanism also allows the maximum drawdown to be properly controlled, thus ensuring the robustness of the strategy.
Judging from the backtest results, the overall return rate of the strategy is as high as 128.86%, with a very significant Alpha. At the same time, the winning rate of the strategy also reached 60.66%, which reflects the stability of the strategy effect.
### Risk Analysis
Although the strategy itself has optimized the risk management mechanism, there are still certain risks that need attention. Specifically, the main risks include:
1. Drawback risk. From the maximum single loss of 222,021.46 yuan, it can be seen that the strategic retracement is relatively large. This is related to the imperfect position management mechanism.
2. Signal stability risk. Strategy signals may be affected by special factors of individual stocks, resulting in false signals. This will have a certain impact on strategic returns.
3. Risks of changes in market environment. If the macro market environment changes significantly, strategy parameters may need to be adjusted to continue to be effective.
### Optimization direction
Based on the above risk analysis, this strategy still has the necessity and possibility of optimization.
1. From the perspective of the maximum loss, the position management mechanism can be further optimized and a stop-loss module introduced to control the magnitude of a single loss.
2. You can consider adding more filtering indicators to identify some special stock phenomena to reduce the probability of false signals. For example, the introduction of volume-price divergence indicators, etc.
3. The strategy parameters should be continuously backtested and verified, and parameters should be adjusted in a timely manner according to changes in the market environment. At the same time, over-optimization should also be prevented.

### Summarize
The momentum trend optimization combination strategy is a medium and long-term quantitative trading strategy that combines momentum factors and trend filtering to specifically optimize T+1 transactions. Judging from the backtest indicators, the overall effect of the strategy is significant and has a very amazing Alpha. However, we should also pay attention to possible risks and adjust parameters in a timely manner according to the market environment. This strategy can bring additional Alpha to quantitative traders and deserves further research and verification.
||
