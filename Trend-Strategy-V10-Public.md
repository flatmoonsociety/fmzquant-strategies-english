...
|----|----|----|
|TradeCurrency|ETH_USD|(?Basic settings) trading pair (the currency standard ends with USD, the U standard ends with USDT)|
|Interval|5000|Program running cycle (ms)|
|UseQuarter|false|Quarterly contract, default USDT perpetual contract|
|OnlyTrendJudgment|false|Only make trend judgments and do not place orders|
|EnableMessageSend|false|Push message|
|RunInKLinePeriod|true|(? Trend judgment) Execute the core strategy according to the K-line cycle|
|KLinePeriod|60|K line period (min)|
|EmaLength|90|EMA length|
|EmaCoefficient|0.004|EMA effective coefficient|
|UseStddev|true|Use standard deviation|
|UseRecordsMiddleValue|true|Use the K-line median to calculate the standard deviation, otherwise use the K-line closing price|
|StddevLength|33|Standard deviation length|
|StddevDeviations|true|Standard deviation deviations|
|MarginLevel|5|(?Order setting) Leverage multiple|
|OrderSize|10|Order quantity/number of sheets|
|OrderByMargin|true|Determine the order quantity based on the order margin|
|OrderMarginPercent|50|Order margin percentage (calculated based on initial capital)%|
...
|UseStopLoss|false|(?stop loss) use stop loss|
|StopLossPercent|1.4|Stop Loss Percent %|
|UseTakeProfit|false|Use Take Profit|
|TakeProfitPercent|10|Take Profit Percent %|
|UseTrackingTakeProfit|false|Use callback to take profit|
|UsePositionRetracement|false|Use position retracement percentage to take profit, otherwise use price retracement percentage to take profit|
|TakeProfitTriggerPercent|5|Calibration take profit price trigger percentage %|
