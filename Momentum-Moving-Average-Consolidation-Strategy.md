...
![IMG](assets/images/205c28e4c77a465b0433aa93be6d14946388e5e7cdf66013316ba2a0ef5cfc2e.png)
[trans]

### Overview
This strategy mainly uses the moving average HMA and EMA to form an average consolidation to determine the buying opportunity. When the HMA crosses the EMA, it is deemed that the consolidation is over and a new upward trend is formed, so buy when the HMA crosses the EMA.
This strategy also incorporates the RSI indicator to detect overbought and oversold conditions. Buying is allowed when the RSI is below 70, and partial take profit is considered when the RSI is above 80.
### Strategy Principles
This strategy uses the 200-period EMA and HMA to construct a moving average system. Among them, the HMA indicator is a more sensitive moving average indicator designed based on the improvement of EMA. When the HMA crosses the EMA, it indicates that the consolidation phase is over and the stock price begins to rise. At this time, if the RSI indicator shows that it is not overbought, a buy signal will be generated.
In the case of a position that has been opened, if the stock price falls back and the HMA crosses the EMA again, indicating the beginning of a new consolidation, all positions will be closed. At the same time, if RSI crosses 80, a partial profit stop of 20% will be implemented to prevent losses.
The trading logic of this strategy is relatively simple, mainly the long and short crossover of HMA and EMA, combined with the high and low judgment of RSI, to form a relatively stable trading strategy.
### Advantage Analysis
The biggest advantage of this strategy is that by using the consolidation trading patterns of EMA and HMA, most False Breaks can be filtered out, thereby increasing profitability. At the same time, the assistance of the RSI indicator can also effectively control risks. The combination of the two makes this strategy very suitable for consolidating the volatile market.
In addition, this strategy only uses three indicators and has simple logic, which makes its parameter optimization and backtesting more convenient and is conducive to the verification and improvement of the strategy.
### Risk Analysis
Although this strategy has certain advantages, there are still some risks to be aware of. For example, the holding time may be relatively long and sufficient financial support is required. If you encounter a period of sideways trading, you cannot stop the loss quickly and exit, and it is easy for losses to expand.
In addition, this strategy mainly relies on moving average indicators. If there is an abnormal breakthrough in price, stop loss measures may not be able to take effect in time, which will bring greater risks. In addition, parameter settings will also affect strategy performance, requiring a large amount of testing to find the optimal parameters.
### Optimization direction
Considering the above risks, this strategy can be optimized from the following aspects:
1. Combined with volatility indicators, dynamically adjust positions according to market fluctuations.
2. Add trend indicator judgment to avoid unnecessary reversal transactions.
3. Optimize the parameters of the moving average to make it closer to the current market characteristics.
4. Use time stop loss to avoid the problem of excessive single loss to the greatest extent.
### Summarize
Generally speaking, this strategy is a relatively classic and simple consolidation and shock strategy. It is mainly used in short-term and mid-term trading of stock indexes and popular stocks, and can obtain a relatively stable Alpha value. With the optimization of parameters and the strengthening of risk control measures, the performance of this strategy still has a lot of room for improvement.
||
