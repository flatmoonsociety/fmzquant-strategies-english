...
![IMG](assets/images/1f60fe8e927ad9f6f80595d5182e0a79a3646d15c7916229b57125c6f733e379.png)

[trans]

#### Overview
The multi-level dynamic trend following system is an improved strategy based on the Turtle Trading Rules. This strategy uses trend signals in multiple time periods, combined with dynamic stop loss and pyramid position increase, to achieve grasp of medium and long-term trends. The system captures trends at different speeds by setting two trend following periods (L1 and L2), and uses the adaptive ATR indicator to dynamically adjust the entry, increase and stop loss positions. This multi-level design allows the strategy to maintain stability in different market environments while maximizing profit potential through pyramiding.
#### Strategy Principle
1. Trend identification: Use two moving average periods (L1 and L2) to identify trends at different speeds. L1 is used to capture faster trends, and L2 is used to capture slower but more reliable trends.
2. Entry signal: A long signal is generated when the price breaks through the high point of L1 or L2. If the last L1 transaction was profitable, the next L1 signal is skipped until the L2 signal appears.
3. Dynamic stop loss: Use a multiple of ATR (default 3 times) as the initial stop loss distance. As the holding time increases, the stop loss position will gradually move upward.
4. Pyramid position increase: During the continuation of the trend, positions are added every time the price rises by 0.5 ATR, up to 5 times.
5. Risk control: The risk of each transaction does not exceed 2% of the account's net value, which is achieved through dynamic calculation of positions.
6. Exit mechanism: Close the position when the price falls below the 10-day low (L1) or the 20-day low (L2), or close the position when the moving stop loss line is triggered.
#### Strategic Advantages
1. Multi-level trend capture: Through the two cycles of L1 and L2, it can capture both fast trends and long-term trends, improving the adaptability and stability of the strategy.
2. Dynamic risk management: Using ATR as a volatility indicator enables dynamic adjustment of entry, stop loss and position additions to better adapt to market changes.
3. Pyramid position increase: Gradually increase positions when the trend continues, which not only controls risks but also maximizes profit potential.
4. Flexible parameter settings: Multiple adjustable parameters enable the strategy to adapt to different markets and trading styles.
5. Automated execution: Strategies can be run completely automatically, reducing human intervention and emotional impact.
#### Strategy Risk
1. Trend reversal risk: Excellent performance in strong trending markets, but frequent transactions in volatile markets may lead to losses.
2. Slippage and transaction costs: Frequent position additions and trailing stops may bring higher transaction costs.
3. Risk of over-optimization: There are many parameters, which can easily cause over-fitting of historical data.
4. Fund management risk: If the initial capital is small, multiple positions may not be effectively executed.
5. Market liquidity risk: In markets with poor liquidity, it may be difficult to execute transactions at the ideal price.
#### Strategy optimization direction
1. Introduce market environment filtering: You can add trend strength indicators (such as ADX) to judge the market environment and reduce trading frequency in volatile markets.
2. Optimize the position adding strategy: You can consider dynamically adjusting the adding interval and number of positions according to the trend strength instead of the fixed 0.5ATR and 5 times.
3. Introduce a take-profit mechanism: In the long-term trend, you can set up a partial take-profit to lock in profits, such as closing half of the position when the profit reaches 3 times ATR.
4. Multi-variety correlation analysis: When applying in combination, inter-variety correlation analysis can be added to optimize the overall risk-benefit ratio.
5. Add volatility filtering: During periods of extremely high volatility, trading can be suspended or risk parameters adjusted to deal with abnormal markets.
6. Optimize the exit mechanism: You can consider using more flexible exit indicators, such as parabolic SAR or Chandelier Exit.
#### Summarize
The multi-level dynamic trend following system is a comprehensive strategy that combines the classic turtle trading rules with modern quantitative techniques. Through methods such as multi-level trend identification, dynamic risk management and pyramid addition, this strategy improves the ability to grasp trends and profit potential while maintaining robustness. Although it faces challenges in volatile markets, through reasonable parameter optimization and risk control, this strategy is expected to maintain stable performance in different market environments. In the future, further improvements can be made by introducing market environment judgment, optimizing the position addition and exit mechanism, etc., to improve the robustness and profitability of the strategy.
|| 
