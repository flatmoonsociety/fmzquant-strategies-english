...
![IMG](assets/images/b6e5bf7cedd8cd2887da7332a3263bf32ec7250926285e9b076ebea618b0e42d.png)

[trans]

## Overview
The triple pattern swing trading strategy is a short-term trading strategy based on a combination of multiple technical indicators. This strategy combines the Super Trend Indicator, SSL Mixed Moving Average and Improved QQE Indicator to form stable trading signals. It is suitable for trading products with higher volatility such as digital currencies and stocks, especially with better performance after the breakthrough period.
## Principle
### Entry signal
Long position entry:
- Super trend from idle to long
- The closing price crossed the upper edge of the SSL hybrid baseline
- The improved version of QQE is blue (long)
Short position entry:
- Super trend changes from long to short
- The closing price crossed the lower edge of the SSL hybrid baseline
- The improved version of QQE is red (short)
### Exit signal
Long position exit: super trend changes from long to short
Short position exit: super trend changes from short to long
### Stop loss
You can choose percentage stop loss, ATR stop loss or recent highest and lowest price stop loss.
### Take profit
You can set the profit-taking return ratio and automatically calculate the profit-taking price
### Fund Management
Optional whether to use fund management logic to control position size
### Drawing
- Draw super trend lines and SSL mixed moving average channels
- You can choose whether to draw EMA moving average
- Draw long and short position opening, stop loss, and take profit lines
- Draw long and short position opening labels
## Advantages
1. Multiple indicator combinations to form stable trading signals
Combined with super trend, SSL mixed moving average and QQE improved version indicators, different indicators verify each other, which can filter out false breakthroughs and form high-quality trading signals.
2. Suitable for shock trading of volatile products
The strategy adopts short-term trading methods and focuses on capturing short- and medium-term price fluctuations. The super trend can effectively track the price trend, and the SSL mixed moving average can clearly identify the support and pressure levels. The two can be used together to gain profits in volatile markets.
3. Multiple stop-loss and stop-profit methods are available
Stop loss can be selected as percentage, ATR value or recent extreme value. Take profit can be set to return ratio. Money management can control positions. Users can freely combine according to variety characteristics and risk preferences.
4. Clear drawings
The strategy drawing is clear and can visually display the stop loss and take profit levels. Opening line markers make it easy to identify trading signals.
## Risk and Optimization
1. Small losses may occur
Due to the use of short-term trading, ordinary small shock losses cannot be completely avoided. The stop loss range can be appropriately relaxed and the capital management logic can be optimized.
2. Risk of false breakthrough
When there is a false breakout in price, a false signal may be formed. You can test EMAs of different periods to filter out false breakthroughs or optimize trend identification indicator parameters.
3. Risk of monitoring indicator failure
If the underlying indicator fails, multiple false signals will appear. It is necessary to regularly verify the effectiveness of indicators and make timely adjustments when problems are discovered.
4. Optimize the backtest cycle
The current backtesting cycle is a fixed time period and cannot correspond to the market cycles of different varieties. It is recommended to optimize to the corresponding main trading time period of the contract.
5. Optimize variety adaptability
The strategy parameters can be fine-tuned according to the data characteristics of different varieties to improve the winning rate of long and short positions. It is recommended to use a stepwise optimization method to compare the impact of different parameters on the strategy.
## Summarize
This strategy uses a combination of multiple indicators to form trading signals, which can effectively filter out false breakthroughs and is suitable for digital currencies and individual stocks with high volatility. At the same time, a variety of stop-loss and stop-profit methods are provided, which are flexible to use. Generally speaking, this strategy forms a stable trading signal and can obtain better returns in short- and medium-term volatile market conditions. Through further optimization, parameters can be optimized for different trading varieties and the strategy profit factor can be improved. This strategy is an efficient trading system that deserves further study.
||
