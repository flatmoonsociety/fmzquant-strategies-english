...
## Strategy Principle
This strategy trades based on the Modified Momentum Index (DMI) indicator. DMI determines the trend direction by comparing the difference between the positive movement indicator (+DI) and the negative movement indicator (-DI).
The specific transaction logic is:
1. Calculate +DI and -DI within a certain period
2. Map the values ​​of +DI and -DI to the interval from 100 to -100
3. Calculate the difference between +DI and -DI and draw the modified DMI curve
4. A buy signal is generated when the modified DMI crosses the 0 axis
5. A sell signal is generated when the modified DMI crosses the 0 axis
6. Can smooth the DMI curve to filter out false signals
This strategy directly compares the relative strength of +DI and -DI to determine the trend direction and avoids the limitations of single indicator judgment.
## Strategic Advantages
- Corrected DMI to intuitively reflect the strength comparison between +DI and -DI
- Combined with moving average to filter out false signals
- Clear long and short signal points
## Strategy Risk
- The DMI indicator itself lags behind and may miss the time point
- Need to optimize parameter cycle
- It’s easy to get trapped in a volatile market
## Summarize
This strategy provides a new perspective for capturing trends by modifying the DMI to determine changes in trend direction. However, the hysteresis problem still needs to be paid attention to, and other indicators can be used for verification.
