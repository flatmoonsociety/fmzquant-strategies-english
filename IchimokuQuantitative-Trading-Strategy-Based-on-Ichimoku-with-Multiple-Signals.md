...
![IMG](assets/images/2fb638606b47e2b5b9a92b495ed4ff2e92719a684e7364c57e8cf90cd4e5cd44.png)
[trans]

### Overview
This strategy comprehensively uses the Ichimoku Kinko Hyo indicator and a variety of other technical indicators to achieve the combination of multiple trading signals to take advantage of the Ichimoku system. At the same time, entry is made through multi-signal confirmation, which can effectively filter out false signals and control risks while pursuing a high winning rate.
### Strategy Principles
This strategy is mainly divided into the following parts:
1. Calculation of Ichimoku Kinko Hyo indicators, including calculation formulas for turning line (Tenkan-sen), baseline line (Kijun-sen), leading line (Senkou Span A), lag line (Senkou Span B), cloud stack (Kumo), etc.
2. Various filter settings, including Kumo cloud stack filter, Kijun baseline filter, MACD filter, RSI filter, Bill WilliamsARGUMENTS fractals filter, SuperTrend filter, Parabolic SAR filter and ADX filter, etc. These filters are used to confirm trend direction and protect trades from volatile markets.
3. Setting of multiple trading signals, including a total of 23 kinds of Ichimoku native trading signals, including the signal of the previous closing price breaking through the baseline, the signal of the relationship between Chikou amplitude and price or cloud stack, the signal of the relationship between the turning line and the baseline or cloud stack. In addition, a variety of other technical indicator signals are also added, such as MACD, RSI, Fractals, etc. These trading signals are used to find potential trading opportunities.
4. Two-stage filter settings are used to filter entry signals. Selecting one filter as the first-stage and second-stage filter respectively can effectively avoid false signals.
5. Two-stage filter settings are used to filter outgoing signals. Similar to admission filter.
6. Multiple signal sets serve as final entry and exit signals. The final trading decision is formed based on the specific trading signals selected by the user, combined with primary and secondary entry filters and exit filters.
7. Take profit and stop loss settings. You can choose whether to enable it and the specific take profit and stop loss points.
8. Backtest cycle setting. You can set the start and end time of backtesting.
### Strategic Advantages
This strategy has the following advantages:
1. Comprehensive use of the advantages of Ichimoku's multiple indicators and multiple trading signals, taking into account trend tracking and signal filtering.
2. Use two-level filter settings to avoid being trapped when entering the market and effectively control risks.
3. Provides a variety of trading signals to choose from, which can be optimized for different market environments.
4. Provides a variety of filters to choose from, which can be optimized according to the characteristics of individual stocks.
5. Stop-profit and stop-loss points can be set to help lock in profits and control risks.
6. Different backtest cycles can be set for verification to facilitate optimization of strategies.
### Strategy Risk
This strategy also has some risks:
1. The Ichimoku system is slow to judge buying and selling signals and may miss short-term trading opportunities. The optimization cycle can be appropriately shortened.
2. Multiple filters may be too cautious, leading to entry uncertainty. You can test and adjust filtering parameters.
3. The setting of a single stop loss point is not flexible enough and cannot cope with complex market conditions. Consider dynamic stop loss.
4. The backtest cycle setting is not accurate enough and cannot fully simulate the real environment. Need to adjust the verification multiple times.
### Strategy optimization direction
This strategy can be optimized from the following aspects:
1. Adjust the Ichimoku system parameters, such as shortening the turning line period to adapt to short-term trading.
2. Test different trading signal combinations and identify the most suitable signal selection for individual stocks.
3. Optimize filter parameters to balance filtering effect and admission certainty.
4. Try the dynamic stop loss method to make the stop loss closer to market changes.
5. Set a longer backtest period, or use tick backtest data to make the simulation more accurate.
6. Add a position management module to optimize capital utilization efficiency by adding positions.
7. Add automatic parameter optimization function to achieve more intelligent strategy adjustment.
### Summarize
This strategy uses a variety of indicators and trading signals provided by the Ichimoku system, and uses other technical indicators for signal filtering and confirmation to achieve a set of quantitative trading strategies that integrate trend tracking and breakthrough signals. The strategy gives full play to the advantages of the Ichimoku system, and at the same time designs parameterized modules for adjustment and optimization, which can better adapt to market changes. Through continuous testing and optimization, this strategy is expected to achieve high and stable profitability.
||
