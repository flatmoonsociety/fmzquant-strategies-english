...
RSI and Bollinger Bands Quantitative-Trading-Strategy RSI-and-Bollinger-Bands-Quantitative-Trading-Strategy
> Author
...
![IMG](assets/images/7c0e00d8c9b2f1e37486bdfc1dff2669fe5f0d043c7b703baaace4b69ee82df2.png)
[trans]

## Overview
This strategy identifies trading opportunities by combining the Relative Strength Index (RSI) and Bollinger Bands, and is a mean reversion strategy in quantitative trading. Buy when the RSI is below the set threshold, close the position when the price crosses the middle track of the Bollinger Bands channel, and there is no opportunity for shorting.
## Strategy Principle
1. Use the RSI indicator to determine whether the market is oversold. When the RSI is below 30, it is considered an oversold signal.
2. Use the Bollinger Bands channel to determine whether the price has started to rebound upward. When the price rebounds from the lower Bollinger Band and crosses the middle Bollinger Band, the long direction ends.
3. Combining the RSI oversold signal and the Bollinger Bands out-of-circle signal, the buying point can be set. Buy when the two signals are triggered at the same time, and wait for the price to cross the middle track of the Bollinger Band to close the position and cash out.
## Advantage Analysis
1. This strategy combines the mean reversal indicator RSI and the channel indicator Bollinger Bands to more accurately locate the buying opportunity.
2. The RSI indicator can filter out many false breakthroughs and reduce unnecessary transactions.
3. As a stop-loss indicator, Bollinger Bands can control the risk of a single transaction.
## Risk Analysis
1. The RSI indicator may send out wrong signals, leading to missed buying opportunities.
2. Improper setting of Bollinger Bands channel parameters may lead to too loose or strict stop loss.
3. Improper selection of trading products, such as higher liquidity risks when trading small market capitalization stocks.
## Optimization direction
1. You can test different parameter combinations, such as RSI period, Bollinger Band channel period and multiples, to find the optimal parameters.
2. You can combine it with other indicators such as KD, MACD, etc. to set more stringent buying conditions to filter signals.
3. You can set the stop loss range according to different trading varieties, such as setting a volatility stop loss.
## Summarize
This strategy first uses the RSI to buy at the low level, and then uses the Bollinger Bands channel to stop the loss at the high level. It is a mean reversion trading strategy. Compared with simply using indicators such as RSI or Bollinger Bands, this strategy can more accurately locate buying and selling points, thereby achieving better strategic effects. The next step can be further improved through parameter optimization, signal filtering, stop loss strategies, etc.
||
