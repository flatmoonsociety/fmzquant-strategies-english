...
## Overview
This strategy determines the market trend direction and achieves trend tracking by calculating the Fractal Chaos Oscillator (FCO) indicator. FCO determines price trends by comparing changes in local maximum and minimum values. The value range is between -1 and 1. The higher the value, the more obvious the trend. Go long when FCO reaches a higher value and go short when it reaches a lower value.
## Strategy Principle
Determine local maximum and minimum values ​​by looking for specific K-line patterns. Compare the changes in extreme values ​​of two adjacent groups and calculate the FCO index. For example, when the latest set of maximum and minimum values ​​is different from the previous set, FCO is 1, indicating that the price upward trend has become stronger. Determine the trend direction based on the FCO value. Go long when the value is higher, and go short when the value is lower.
## Advantage Analysis
- The FCO indicator is simple and effective in judging the trend direction.
- No need for complex parameter settings, easy to use
- Can achieve profits in the short term, suitable for intraday trading
- You can choose to go long or short as needed
## Risk Analysis
- Type recognition is not completely accurate and the turning point may be missed
- Unable to accurately judge the trend reversal point, and there is a risk of loss
- Frequent intraday transactions and heavy handling fee burden
Parameters can be optimized appropriately or combined with other indicators to determine trend reversal.
## Optimization direction
- Test different parting cycle parameters
- Optimize FCO’s long and short thresholds
- Combined with indicators such as moving averages to determine trend reversal
- Test parameter robustness in different varieties
## Summarize
The FCO strategy uses simple indicators to determine the trend direction and is suitable for short-term trading. The effect can be improved through parameter optimization and other methods. It is an easy-to-implement trend following strategy idea.
||
