...
This strategy uses the MACD indicator to form a trend judgment in a higher period (such as the daily line), and perform specific transaction execution in a lower period (such as 5 minutes). This cross-cycle operation is designed to improve the reliability of the MACD indicator trading strategy.
Strategy principle:
1. Calculate the MACD indicator in high cycles to determine the direction of the general trend.
2. Find the entry point in the low cycle, go long when MACD crosses the signal line above, and go short when it crosses below the signal line.
3. Combined with the overbought and oversold signals of the MFI indicator, further confirm the trading time.
4. Set stop-loss and take-profit lines for risk management.
5. Optimize parameters and improve the stability of indicators and strategies.
Advantages of this strategy:
1. Make judgments across cycles to avoid being disturbed by short-term market noise.
2. MFI indicator verification can filter out false signals and improve accuracy.
3. The stop-loss and stop-profit mechanism helps control the risk of a single transaction.
Risks of this strategy:
1. There is a lag problem in cross-cycle operations, and the best time may be missed.
2. Both MACD and MFI may have many false signals and need to be verified carefully.
3. Strict fund management is required to hedge transaction risks.
In short, this strategy uses cross-cycle MACD to determine the general trend, and uses the MFI indicator for filtration to trade in low cycles, which can improve stability. However, the problem of cross-cyclical lag still exists, and investors still need to operate with caution.
||
