...
RSI-VWAP Volume and Price Strategy RSI-VWAP-Price-Volume-Strategy
> Author
...
## Overview
RSI-VWAP volume and price strategy is a trend following strategy. It combines two indicators, the relative strength index (RSI) and the volume weighted average price (VWAP), to achieve multiple positions and stop losses in the trend. This strategy is suitable for medium and long-term trend trading.
## Principle
When the RSI indicator line falls back from the overbought zone and enters the oversold zone, it is considered that the trend is reversed and you go long; when the RSI indicator line rises from the oversold zone and enters the overbought zone, it is considered that the trend is reversed and you go short.
The stop-loss line for long positions is the latest opening price (1-stop-loss ratio), and the take-profit line is the average price of the position (1+take-profit ratio); it is similar for short positions.
After each position is opened, if the signal is triggered again, the position can be added. The number of positions can be added up to 5 times, and the amount added each time is increased to achieve trend tracking.
## Advantages
1. Combining the RSI indicator and the VWAP indicator can better determine the trend reversal point.
2. By adding positions multiple times, you can make full use of the trending market. As the number of positions increases, the position gradually expands to achieve trend tracking.
3. Setting a stop loss line can effectively control risks. Stop loss and exit after a loss occurs in a position to avoid further expansion of losses.
4. Set a trailing take-profit to lock in profits and avoid profit taking.
## Risk
1. The RSI indicator has a repaint phenomenon, and the actual signal trigger point may deviate.
2. VWAP may also repaint. The actual optimal entry point can only be determined ex post facto.
3. Improper setting of stop loss points may cause unnecessary losses.
4. Improper setting of the profit stop point may prevent profits from being realized.
5. If the trend is judged incorrectly, continuing to be long (short) may lead to larger losses.
## optimization
1. Optimize RSI parameters and find the best length period.
2. Optimize the overbought and oversold areas to make it more accurate to judge trend reversal.
3. Test different strategies to add positions and find the best way to add positions.
4. Optimize stop loss and take profit and find the optimal parameters.
5. Try to judge the trend in combination with other indicators to increase the probability of determining a trend reversal.
## Summarize
The RSI-VWAP volume and price strategy uses the RSI indicator combined with the VWAP indicator to determine the trend reversal point, and sets multiple positions to track the trend. It will stop profit when the profit reaches the preset standard and stop the loss when a loss occurs. It comprehensively considers risk control and profit protection. Through parameter optimization, better strategy effects can be obtained. This strategy is suitable for traders with certain experience to conduct medium and long-term trend trading.
||
