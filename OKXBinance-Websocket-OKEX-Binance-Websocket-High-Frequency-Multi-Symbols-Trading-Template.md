...
OKXBinance-Websocket High-Frequency Trading Template-Multiple Varieties OKEX-Binance-Websocket-High-Frequency-Multi-Symbols-Trading-Template
> Author
...
* Public stream subscription, trades and deep synthesis
* Private stream subscription, maintain local orders, positions and balance information
* Maintain order cancellation timeout status, support amendOrders of OKX to modify orders
* Supports multiple varieties, allowing high-frequency strategies to operate multiple markets at the same time with the lowest latency
* Add function EventLoop (requires support from the latest host). This function can wait for any websocket data to return when there are multiple websocket connections, avoiding polling.
* Supports OKX and Binance futures and Binance's spot and spot margin trading. OKX's spot has not been supported yet. If you are interested, you can change it yourself.
* OKX's private stream subscription requires that the Key and password have been written as parameters in the template
* Automatically handle some basic network functions such as disconnection and reconnection
* Support event callback for private flow order transaction cancellation report
* Unify the order logic of the two exchanges. For details, please see the template source code or examples.
* Supports subscribing to the websocket interface of multiple exchanges at the same time
* Order modification and deletion can be returned as tasks in batches, and transaction tasks will be automatically processed
When placing high-frequency orders, the orders returned by the exchange will not be timely (private channel), but the market may have changed at this time. We need to maintain an order status locally, and the strategy has been implemented.
The template will automatically identify whether it is OKX or Binance. Below is a simple example. It is strongly recommended to read the source code so that you can customize your own functions in a deeper way.
...
```
[trans]
The running effect is as follows, you can also subscribe to multiple varieties in the subscription function
||
