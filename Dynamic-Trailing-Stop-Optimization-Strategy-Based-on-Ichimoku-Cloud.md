...
![IMG](assets/images/4393400d0bc89f9a8764e5574a5e8b787a7cb5e6196866f9d3f686e8c6d65416.png)
[trans]

## Overview
The core idea of ​​this strategy is to combine the fishing net strategy and the dynamic moving stop loss mechanism to achieve a more efficient stop loss exit. The fishing net strategy uses K-line indicators to determine market trends and positions, and dynamically determines the timing of buying and selling. The dynamic stop-loss mechanism can set stop-loss points based on market fluctuations and flexibility to effectively control risks.
## Principle
This strategy is mainly based on the following modules:
1. Fishing net indicator module   
Calculate the fishing net indicator through Fisher strain and Stoch indicator to determine market trends and buying and selling points.   
2. Dynamic stop loss module
Dynamically calculate stop loss points based on ATR and RSI indicators to achieve dynamic tracking stop loss.   
3. Trailing stop loss tracking module
Set a fixed stop loss displacement point, and exit the position when the price reaches the stop loss point.
## Advantage Analysis
The biggest advantage of this strategy is its excellent risk control capabilities. The dynamic stop loss mechanism can set an appropriate stop loss range according to the degree of market volatility, effectively avoiding losses caused by excessive slippage, and can track trends better than fixed stop losses. In addition, the fishing net indicator is accurate and reliable in judging buying and selling timing, and can filter out some noise transactions.
## Risk Analysis
The main risk with this strategy is that improperly set stop loss points may result in an overly aggressive exit. In addition, if you use too aggressive parameter settings, it may also cause whipsaw transactions to be too frequent. To reduce these risks, parameters should be selected reasonably and avoid setting too large a movement range.
## Optimization direction
The optimization space of this strategy mainly focuses on the following aspects:
1. Optimize fishing net indicator parameters and find better parameter combinations to judge trends;
2. Optimize dynamic stop loss parameters to find a more balanced stop loss range;
3. Add a volatility-based position management module to adjust positions according to the degree of market volatility.
Through parameter search and rule optimization, this strategy can achieve higher Risk Adjusted Return.
## Summarize
This strategy comprehensively uses the fishing net indicator and dynamic stop loss technology, which can not only judge the market trend more accurately, which is conducive to timing decisions, but also dynamically adjust the stop loss range to effectively control risks. Through continuous module expansion and parameter optimization, this strategy can become an effective strategy framework that can be applied in the long term.
||
