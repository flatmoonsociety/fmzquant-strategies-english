...
Dual-Moving-Average-Reversal-Strategy based on dual moving average
> Author
...
Because of its principle of using simple moving averages to judge changes in market trends, I named this strategy "long-term reversal strategy based on double moving averages."
## Strategy Principle
The core logic of this strategy is to use two simple moving averages of different periods on the 14th and 28th to determine the market trend. The specific rules are as follows:
1. Define the fast line as the 14-day simple moving average and the slow line as the 28-day simple moving average.
2. When the fast line breaks through the slow line from bottom to top, it is a long signal and you should enter the market long.
3. When the fast line falls from above and breaks through the slow line, it is a short signal, so enter the market short.
4. After going long/short, when the fast line falls below the slow line again, it is a signal to close the position.
This strategy also combines stop loss, take profit, and trailing take profit for risk management. For the long and short situations, the stop-loss price for long positions, the take-profit price for long positions, the take-profit price for short positions, and the trailing take-profit price for long positions are respectively defined. These parameters are set in percentage form, making the strategy more flexible.
## Advantage Analysis
- The strategy of using double moving averages to determine the main market trends, the principle is simple and clear, easy to understand and verify.
- The period of the fast and slow moving average is set to 14 and 28 days, which represent the short-term and medium-term trend conversion respectively, and can better detect reversal opportunities.  
- Combine take-profit, stop-loss and trailing take-profit to control risks, lock in profits and avoid losses from expanding.
- Can be long and short at the same time to meet the needs of different market environments.
## Risks and Improvements
- Double moving average crossovers have a certain lag, and the best entry opportunity may be missed.
- False signals are likely to occur when long and short moving averages cross, so avoid setting an average period that is too short.
- Setting the stop loss distance too small may increase the probability of the stop loss being hit. The stop loss distance should be set appropriately according to different varieties.
- More indicators can be introduced for combination to improve the robustness of the strategy. For example, adding Bollinger Bands to determine the trend, or introducing MACD to check the entry timing.
## Optimization direction
- Test different moving average parameter combinations to find the moving average period that better matches the characteristics of the variety.
- Test different stop loss distance settings to find the optimal stop loss position.
- Test and add other indicators for optimization to find the optimal parameter combination to reduce false signals.
- Optimize position management rules to make profits more substantial.
## Summarize
Overall, this strategy is a very classic strategy for judging trend reversal based on double moving averages. It has the advantages of simple operating principle and easier to master; at the same time, there are also some directions that can be continuously optimized in the future. Overall, this strategy is relatively mature in principle and operation, and is a good introductory strategy for quantitative trading.
||
