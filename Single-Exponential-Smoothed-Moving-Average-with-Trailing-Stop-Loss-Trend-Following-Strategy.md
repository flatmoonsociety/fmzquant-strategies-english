...
![IMG](assets/images/877784cfa8dc791bbd5e24e40de0d6a2094f2e1c3b4c7dd86c5c4722493f0eb6.png)
[trans]

## Overview
This strategy combines the use of the Single Exponential Smoothed Moving Average (SESMA) and the Donkey Ladder surrounding mechanism with trailing stops to form a very stable and efficient trend following strategy. SESMA serves as the main line and is used to identify the price trend direction. The trailing stop loss mechanism can effectively reduce strategy risks while protecting strategy profits.
## Strategy Principle
The strategy consists of two core indicators:
1. Single Exponential Smoothed Moving Average (SESMA): SESMA draws on the idea of ​​EMA and improves the parameters to make the curve smoother and reduce the delay. Determine the price trend through the relationship between SESMA's direction and price.
2. Trailing stop loss mechanism: Combined with the highest price, lowest price and ATR indicator, the stop loss lines for long and short positions are calculated in real time. This is a dynamically adjusted stop loss mechanism that can adjust the stop loss range based on market volatility and trends. The relationship between the stop loss line and the price is used to determine the timing of closing the position.
The entry basis for this strategy is that the price breaks through SESMA. The exit signal is triggered by the stop loss line. You can set whether to display the mark.
## Strategic Advantages
1. The SESMA calculation method has been improved, which can effectively reduce delays and improve the ability to capture the trend.
2. The trailing stop loss mechanism can adjust the stop loss range according to real-time fluctuations to avoid the stop loss being too loose or too close. 
3. Comes with a visual aid to determine the timing of Entry and Exit.
4. Customizable parameters, suitable for different varieties and parameter optimization.
## Risks and optimization directions
1. When the trend reverses, stop loss may be triggered, leading to premature exit. The stop loss range can be appropriately relaxed.
2. SESMA parameters can be optimized to find the optimal length.
3. ATR parameters can also test different cycle lengths.
4. Test whether the mark is displayed.
## Summarize
This strategy integrates trend judgment and risk control indicators to form a more robust trend following strategy. Compared with the simple moving average strategy, this strategy can capture trends more flexibly while reducing retracement. Through parameter optimization, the strategy can achieve better results in different markets.
||
