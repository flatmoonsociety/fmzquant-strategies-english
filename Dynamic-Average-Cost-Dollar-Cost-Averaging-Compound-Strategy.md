...
![IMG](assets/images/65f2f6bbf585e33bb5bc32737f096be8b70c4ce5d8d3197d011471342ef5a85f.png)
[trans]

#### Overview
The dynamic average cost fixed investment compound interest strategy dynamically adjusts the number of positions opened each time, opening a small amount of positions at the beginning of the trend, and gradually increasing the position as the depth of consolidation increases. The strategy uses an exponential function to calculate the stop loss price at each level, and re-opens new positions in batches when triggered, thus keeping the position cost line declining at an exponential level. As the depth increases, the position cost can be gradually compressed downwards. After the price reverses, profit will be taken out in batches to obtain greater profits.
#### Strategy Principle
This strategy uses a simple RSI oversold point signal combined with an moving average timing method to select the opening time. The first opening signal is generated when the RSI is below the oversold line and the closing price is below the moving average. After the first order is opened, the lower limit of the price drop below the range is calculated based on the exponential function, and a DCA signal is generated. After each DCA, the position is adjusted to make each position equal. Due to the dynamic changes in position volume and position cost, this has a leverage amplification effect.
As the number of DCA increases, the cost of holding positions continues to decline, and each take-profit only requires a small rebound to achieve profits.
After consecutive multiple orders are opened, a stop loss line will be drawn above the average price. Once the price breaks upward again and exceeds the average price of the position and the stop loss line, the stop loss will be exited.
The biggest advantage of the strategy is that as the cost of holding positions continues to decline, even if the market is consolidating, the cost can be gradually reduced cumulatively. When the trend reverses, since the holding cost has been significantly lower than the market price, greater profits can be achieved.
#### Risks and Pitfalls
The biggest risk of this strategy is the limited initial position. If there is a continued downward trend, there is a risk of stop loss. Therefore, you need to set a stop loss range that you can bear.
In addition, there are also two extremes in the setting of stop loss range. Setting the stop too large won't allow the unit to catch enough depth of bounce. If the stop loss width is set too small, the probability of encountering a price limit reversal during the mid-term adjustment will be relatively high. Therefore, it is very important to choose an appropriate stop loss range based on different markets and your own risk preferences.
After the DCA cycle is longer and more layers are formed, if the price rises sharply, the position cost will be too high and the loss will be unable to be stopped. This also requires setting the DCA level appropriately based on your total position volume and the maximum position cost you can bear.
#### Optimization suggestions
1. Optimize timing signals. Different parameters and different indicator combinations can be tested in order to select signals with a higher winning rate.
2. Optimize the stop loss mechanism. You can test using Λ-shaped stop loss or arc-shaped stop loss instead of simple trailing stop loss, which may achieve better stop loss effect. You can also add a position time sharing strategy to adjust the stop loss range.
3. Optimize the profit-taking method. You can test different types of moving take-profits to find better take-profit exit opportunities, thereby increasing the overall rate of return.
4. Add anti-rebound mechanism. After the stop loss, the DCA signal may be triggered again to reopen the position. At this time, you can consider adding a certain range of anti-rebound range to avoid aggressive re-opening of positions immediately after stopping the loss.
#### Summarize
This strategy uses the RSI indicator to determine the buying opportunity and the dynamic stop-loss DCA strategy calculated based on the exponential function to dynamically adjust the position quantity and position cost, thereby gaining a price advantage in the swing market. The optimization plan mainly focuses on entry and exit signals, stop loss and take profit methods. Generally speaking, this strategy uses the core concept of index DCA to continuously reduce the cost of holding positions, gain more operating space during consolidation, and obtain higher returns in trending markets. However, you still need to choose appropriate parameters according to your own fund management plan to control the overall position risk.
||
