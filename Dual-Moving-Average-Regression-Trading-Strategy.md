...
![IMG](assets/images/f573a4c3ab06e00ec8cdff5c4543b184734af6abffad9db8efa9efddd69928ed.png)

[trans]
#### Overview
This strategy uses two linear regression lines of different lengths as trading signals, and combines ATR as stop loss and partial take profit. When the linear regression line of a shorter period crosses the linear regression line of a longer period from bottom to top, a long signal is generated; conversely, when the linear regression line of a shorter period crosses the linear regression line of a longer period from top to bottom, a short signal is generated. This strategy uses ATR as a trailing stop after opening a position, and also adopts a batch stop-profit method to maximize profits.
#### Strategy Principle
1. Calculate the linear regression lines of two different periods (default is 20 and 40) as trading signals
2. When the shorter period linear regression line crosses the longer period linear regression line from bottom to top, if there is no current position, open a long position
3. When the shorter period linear regression line crosses the longer period linear regression line from top to bottom, if there is no current position, open a short position
4. Once a position is opened, use ATR to calculate the trailing stop price. When the price hits the stop price, close all positions.
5. At the same time, this strategy adopts the method of taking profits in batches, calculating 16 different percentages of take-profit levels (from 5% to 80%) based on the opening price, and each stop-profit level closes the corresponding number of positions according to the input percentage.
6. Until all positions have been closed
#### Advantage Analysis
1. Use linear regression as trend judgment and trading signal to better capture the trend
2. The combination of two linear regressions with different periods forms a more reliable signal confirmation.
3. Using ATR as a stop loss can better control risks and keep pace with price fluctuations.
4. The method of taking profits in batches can obtain more profits when the trend continues, while also taking into account risk control.
5. The code is highly modularized, has many input parameters, and the strategy is highly customizable.
#### Risk Analysis
1. Linear regression signals may have erroneous signals, resulting in losses
2. Taking profits in batches may lead to longer holding times and face the risk of retracement
3. Improper parameter settings may lead to poor strategy performance
4. Under extreme market conditions, the strategy may face a large retracement
#### Optimization direction
1. You can consider introducing more indicators or filtering conditions to improve signal accuracy, such as trend confirmation indicators, volatility indicators, etc.
2. Optimize the positions and strategies of take-profit and stop-loss. You can consider dynamic take-profit and stop-loss.
3. Optimize parameters and find the best parameter combination
4. Add position management and adjust position size according to market fluctuations
#### Summary
This double moving average regression strategy combines trend tracking and stop-profit and stop-loss strategies, which can effectively capture the trend market while controlling retracements. However, this strategy may not perform well in volatile market conditions and faces the problem of parameter optimization. In general, this strategy is a typical representative of a trend following strategy and can be optimized and improved as a basic strategy.
|| 
