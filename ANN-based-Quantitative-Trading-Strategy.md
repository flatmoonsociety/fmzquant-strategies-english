...
ANN-based-Quantitative-Trading-Strategy based on artificial neural network
> Author
...
![IMG](assets/images/48bdc4f84baa6f1c697db274e463418321d0ec0c0447bf775ebc2bc781c6a95c.png)
[trans]

## Overview
This strategy uses artificial neural networks to predict future price changes and generates trading signals based on the prediction results. It is a trend following strategy. The advantage of the strategy is that it can identify complex non-linear trends and is suitable for medium and long-term transactions. However, there is also the risk that the backtesting effect is good but the real offer effect is not good.
## Strategy Principle
This strategy uses artificial neural networks (ANN) to predict the rise and fall of the next day.
The input layer has only one input node, which is the percentage increase or decrease in the past day.
The hidden layer contains 2 layers, the first layer has 5 nodes, and the second layer has 33 nodes, both using hyperbolic tangent (tanh) as the activation function.
The output layer has only one output node, which is directly used as the final prediction result after passing through the linear activation function.
If the prediction result is greater than the value of the threshold parameter, a buy signal is generated, and if it is less than the value of the negative threshold parameter, a sell signal is generated.
## Strategic Advantages
- Use ANN model for prediction, which can fit complex nonlinear trends
- Enter data using only one day, no need for a lot of historical data
- Can identify trends over longer time scales
- Use multiple hidden layers to achieve strong fitting capabilities
- The activation function and parameters have been optimized and the prediction effect is better
## Strategy Risk
- The ANN algorithm has the risk of over-fitting, and the actual effect may not be as good as backtesting
- Requires longer historical data for training, not suitable for recently listed stocks
- Parameters and structures need to be optimized repeatedly, and the effect is unstable
- Only predict one-day rise and fall, unable to judge long-term trend
- When the market enters a period of shock, the effect may become worse
## Optimization direction
- Add input variables, such as trading volume and other information
- Try different ANN structures and activation functions
- Optimize the parameters of ANN and improve the fitting accuracy
- Increase the sample size of the training set to avoid overfitting
- Forecast multiple time scales and determine long- and short-term trends
- combine with other models, ensemble learning
- Use indicators such as volatility to strengthen risk control
## Summarize
This strategy uses the ANN model to predict price changes, which can identify complex nonlinear trends and is suitable for medium and long-term transactions. However, the black box characteristics of the ANN model also bring great challenges to the real market. We need to optimize from input features, model structure, parameter optimization, ensemble learning, etc., while supplementing it with traditional technical analysis indicators to enhance operational effects and reduce the risk of real trading. Artificial intelligence strategies still need to be perfectly combined with traditional strategies to achieve maximum effect.
||
