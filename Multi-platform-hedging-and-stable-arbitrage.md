...
Multi-platform hedging and stable arbitrage
> Author
...
The program automatically converts the exchange rate and handling fee. When placing an order, refer to the buy-one-sell-one price. Note that this is not a move, but a hedging! No need to manually transfer money or coins, one deposits money, one deposits coins, one buys, one sells, advanced hedging mode.
This hedging model does not matter whether the price of the currency rises or falls, it only makes money from the price difference.
To open a position, you need to put money in one place and coins in the other. For example, the current price of BTC is 6,000, put 3,000 knives in one exchange, and put 0.45 coins in one, or double them all.
MaxDiff refers to the lowest platform price difference
SlidePrice refers to the sliding price when placing an order. This price is added to the buy order and subtracted to the sell order.
TickIntervalS refers to the detection period
...
|MaxDiff|3|Minimum difference (yuan)|
...
|StopPriceH|9000|Limit value (yuan)|
|optFeeTimeout|30|Fees update period (minutes)|
|AmountOnce|0.2|Amount of single transaction|
|UseMarketOrder|false|Use market order to stop loss|
|StopWhenLoss|false|Stop when losing money|
