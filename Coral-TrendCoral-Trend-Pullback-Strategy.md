...
Ripple strategy Coral-Trend-Pullback-Strategy based on Coral-Trend indicator in the backtest interval
> Author
...
![IMG](assets/images/393cc6bc5c522266163a3c688c522966cd18b2621926cc746519d5ad5e2ce2a4.png)
[trans]

## Overview
This strategy uses LazyBear's Coral Trend indicator to determine the price trend direction, and identifies potential entry points by identifying the reversal in the direction of the Coral Trend indicator. In order to filter out false breakthroughs, this strategy uses the ADX indicator or a combination of Absolute Strength Histogram and HawkEye Volume indicators as confirmation indicators to achieve more reliable entry.
The Exit mechanism uses the highest price/lowest price of the recent N K lines multiplied by the configurable risk-reward ratio to set the stop loss and take profit levels.
## Strategy Principle
After judging the direction of the general trend based on the Coral Trend indicator, while the color of the indicator remains unchanged, the price appears a small pullback in the opposite direction. At this time, if the callback ends and the price returns to the main trend direction indicated by the Coral Trend, then it can be considered a better entry opportunity.
Admission requirements include:
1. The Coral Trend indicator direction is consistent with the trading direction (long = green, short = red)
2. Since the last price completely broke through the Coral Trend indicator (the high points of the last bar are all above the Coral Trend line), there has been at least one K line with all lows above the Coral Trend indicator (long) or all highs below the Coral Trend indicator (short).
3. A small correction (pull back) occurs in the opposite direction. During this small correction, the closing price remains on the opposite side of the Coral Trend.
4. After the small correction ends, the closing price returns to the main trend direction indicated by Coral Trend
The above are the main conditions. At the same time, the strategy uses the ADX indicator or the Absolute Strength Histogram and HawkEye Volume indicators as confirmation conditions for entry.
The ADX indicator requires its value > 20 and the last 1 K-line to rise. And the order of the green and red lines of DI is consistent with the trading direction.
The Absolute Strength Histogram requires its color to be consistent with the trading direction (long = blue, short = red). HawkEye Volume requires its color to be consistent with the trading direction (long = green, short = red).
The Exit mechanism uses the highest price or lowest price of the recent N K lines multiplied by the risk-reward ratio to set the stop loss and take profit levels. Both the N value and the risk-reward ratio can be configured through parameters.
## Advantage Analysis
The biggest advantage of this strategy is to use the Coral Trend indicator to determine the main trend direction, and then identify its reversal to find entry opportunities to avoid following the trend in non-trending markets. At the same time, using the Confirm indicator can filter out many false breakthroughs, thereby improving the success rate of entry.
In addition, this strategy provides a complete risk control mechanism, including stop loss setting and risk exposure percentage control, so that even individual transaction losses will not have a major impact on the overall funds.
## Risk Analysis
The biggest risk of this strategy is that using indicators to make entry judgments can easily lead to the illusion that profits can be made automatically by relying entirely on parameter configuration. In fact, both parameter optimization and rule configuration need to be combined with the laws of underlying price changes and intuitively judge the effect of linkage between indicators and prices, in order to set a configuration that is more suitable for your own trading style and variety.
In addition, the stop loss and take profit levels must also be set appropriately. A take profit multiple that is too large may result in the inability to take profit and exit, while a stop loss level that is too small may result in excessive risks. This needs to be set according to the volatility of different varieties and personal risk tolerance.
## Optimization direction
Directions that can be optimized with this strategy include:
1. Adjust the parameters of the Coral Trend indicator to make it more responsive to price changes of different varieties.
2. Try different Confirm indicators or indicator combinations, such as KDJ, MACD, etc., to make the entry signal more accurate
3. According to the degree of volatility of different varieties, adjust the calculation method of stop loss and take profit levels to achieve better risk control.
4. Add a fund management module, which can adjust the amount of a single order according to the number of positions, effectively controlling the overall loss.
5. Add a trading time control module so that the strategy can only run during specific periods to avoid losses during periods of severe fluctuations.
## Summarize
This strategy first uses Coral Trend to determine the mid- to long-term price trend, and then determines its reversal and cooperates with the Confirm signal to filter out false breakthroughs to build a more reliable trend following strategy. At the same time, the perfect risk control settings also enable this strategy to operate for a long time with stable funds. Through further parameter and module optimization, it is expected that this strategy can be adapted to more varieties and have better stability and profitability.
||
