...
[trans]
The name of this strategy is "5-day high and low breakthrough price channel strategy". This strategy builds a price channel by counting the highest and lowest prices in the last five trading days, and conducts trend trading when the price breaks through the channel.
The specific steps are as follows:
1. Calculate the highest price and lowest price in the last 5 trading days.
2. Take the highest two highest prices to construct the upper track, and take the two lowest lowest prices to construct the lower track.
3. When the price rises above a certain percentage of the upper track (such as 0.3%), a buy signal is generated.
4. When the price falls by a certain percentage of the lower track, a sell signal is generated.
5. After entering the market, use the second highest and lowest price as the stop loss level, or track a certain percentage of the stop profit (such as 0.5%) before closing the position.
The advantage of this strategy is to identify trend turning points by breaking out of key high/low prices. A channel breakout represents a concentration of price and volume driving the price. However, it is necessary to prevent too many invalid signals from being generated during consolidation.
Generally speaking, focusing on breakthroughs in key price areas is a more classic trend following strategy. However, traders still need to assist in confirming other indicators and optimizing parameters to maximize the effectiveness of this strategy.
