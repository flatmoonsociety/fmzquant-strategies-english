...
![IMG](assets/images/6d6aac64f00ac9c4bc827003f344528d0e259d117cc87212722559e0e45778cc.png)

[trans]
## Overview
The two-way ATR wave trading strategy is a trend following strategy that combines moving averages, ATR and multiple technical indicators to conduct trend following transactions after the trend direction is established.
## Strategy Principle
This strategy uses the Kijun line as the main moving average indicator to determine the direction of the price trend. The strategy also combines the ATR channel to limit the range of price activity. Do not go long when the price is close to the upper track, do not go short when the price is close to the lower track, and avoid chasing highs and selling lows.
A buy signal is generated when the Kijun line breaks upward, and a sell signal is generated when a downward breakthrough occurs. In order to filter out false signals, the strategy also introduces multiple technical indicators for confirmation, including the Aroon indicator, RSI indicator, MACD indicator and PSAR indicator. Buy and sell signals are generated only when all indicator confirmation conditions are met.
After entering the market, the strategy uses stop loss and take profit methods to manage positions. The stop loss point is 0.5 ATR and the take profit point is 0.5%. When the price breaks through the Kijun line again and reverses, choose to stop the loss immediately and exit.
## Strategic Advantages
- Use the Kijun line to determine the trend direction and avoid being trapped by the volatile market
- ATR channel limits the range of price activities, which is beneficial for risk control
- Multiple technical indicator confirmations can greatly filter out false signals
- Combined with stop-loss and stop-profit risk management, it is beneficial to lock in profits
## Strategy Risk
- Multiple indicator confirmations cause signal delays and may miss the beginning of the trend.
- If the stop loss point is too small, you may be stopped out frequently.
- Unreasonable Kijun line and ATR parameters may lead to frequent false signals
- Relying on parameter optimization and historical data fitting results, the real offer may not be effective
## Optimization direction
- Try more advanced trend judgment indicators, such as Ichimoku cloud chart, etc.
- Adjust stop loss and profit points to optimize profit and loss ratio
- Test the best parameter combinations for different markets
- Added automatic parameter adjustment function to adjust parameters according to real-time market
- Test the effects of different Confirmation indicator combinations
## Summarize
The two-way ATR wave trading strategy comprehensively uses moving averages, ATR channels and multiple auxiliary technical indicators to perform trend following operations after determining the trend direction. Compared with a single indicator strategy, the signal quality and profit probability can be greatly improved. At the same time, the stop-loss and stop-profit mechanism controls risks. Through parameter optimization and combination testing, this strategy is expected to achieve stable profits. However, we need to pay attention to the problem of over-reliance on historical data, and the effect of the actual offer still needs to be verified. Continuous optimization is the key to ensuring the effectiveness of the strategy.
