...
![IMG](assets/images/97f11591348305994d608d94c8d6071ce7113d1020deb33ba14175c2dc2c4b90.png)
[trans]

## Overview
The name of this strategy is "Double Reversal Percent Change Histogram Quantitative Strategy". This strategy comprehensively uses two different types of strategies for combined transactions to give full play to their respective advantages and achieve better trading results.
The first strategy uses the principle of a reversal strategy. It uses the Stoch indicator to determine whether a reversal signal occurs based on the comparison of the closing price with the previous day or several days. The second strategy uses the "percentage change bar chart" indicator to determine the daily rise and fall changes as a basis for establishing positions.
## Strategy Principle
The Double Reversal Percent Change Histogram Quantitative Strategy utilizes two major components:
The first part is the 123 reversal strategy. The judgment logic of this strategy is:
1. If the closing price is lower than the previous day's closing price, and the Stoch fast line is higher than the slow line and above the 50 level, it is considered to be overbought and a sell signal is generated;
2. If the closing price is higher than the closing price of the previous day, and the Stoch fast line is lower than the slow line and below the 50 level, it is considered to be in the oversold area, and a buy signal is generated;
3. Establish corresponding long or short positions based on the generated buy and sell signals.
The second part is the percentage change bar chart indicator. The judgment logic of this indicator is:
1. Calculate the change percentage of the current K line relative to N previous K lines (defined by parameter input_barsback);
2. If the change percentage is higher than the positive value area defined by the parameter BuyZone, a buy signal is generated; if it is lower than the negative value area defined by SellZone, a sell signal is generated;
3. Establish corresponding long or short positions based on the generated buy and sell signals.
Finally, if the signals generated by the two strategies are consistent, the position is actually established. If the signals are inconsistent, there will be no position changes.
## Advantage Analysis
The double reversal percentage change bar chart quantification strategy has the following advantages:
1. By absorbing the respective advantages of two different types of strategies, it is expected to obtain more stable returns. The 123 reversal strategy performs well in determining market reversal points; the percentage change histogram indicator quickly identifies breakthrough markets. The combination of the two can both identify reversals and capture trends.
2. The combination of two strategy signals can effectively filter out some false signals, reduce unnecessary stop losses, and reduce transaction risks.
3. The 123 reversal strategy parameter optimization space is large, and the parameter combination can be adjusted to optimize and adapt to different varieties and cycles.
4. The percentage change column chart strategy is intuitive, and it is easy to grasp and control trading risks through parameter adjustment.
## Risk Analysis
There are also some risks associated with the Double Reversal Percent Change Histogram Quantitative Strategy:
1. When the signals of the two strategies do not match, a position cannot be established and some trading opportunities will be missed. The parameter interval of the percentage change histogram can be appropriately relaxed to increase the matching probability.
2. The 123 reversal strategy is sensitive to parameters, and inappropriate parameter combinations may result in too many false signals. The parameters should be tested separately for different varieties to ensure that the parameters are stable.
3. If the buying and selling signals generated by the percentage change bar chart are in the wrong direction and match the 123 reversal signal, a large loss will occur. The range of the change percentage parameter should be appropriately narrowed to control risks.
4. After the strategy runs for a period of time, the adaptability of the parameters will decrease. It is necessary to monitor the strategy's yield curve and trading signals to determine the timing of parameter adjustments.
## Optimization direction
The double reversal percentage change histogram quantification strategy can also be optimized from the following directions:
1. Optimize the Length, KSmoothing, DLength and other parameters of the 123 reversal strategy to find a parameter combination that is more suitable for different varieties and cycles.
2. Adjust the parameter input_barsback of the percentage change bar chart to determine the impact of a longer or shorter payback period on the strategy.
3. By introducing a stop-loss strategy, you can effectively avoid large losses caused by false signals generated by percentage change histograms.
4. Try to use machine learning and other methods to train a percentage change model that can more accurately judge buying and selling opportunities to obtain a higher winning rate.
5. Add other auxiliary technical indicators to judge, enrich the trading signals of the strategy, and increase the trading frequency.
## Summarize
The double reversal percentage change column chart quantitative strategy makes full use of the advantages of two different types of strategies and uses them in combination to control risks and increase profit margins. This strategy is easy to understand and optimize, making it ideal for research and practice. Through further parameter adjustment and strategy optimization, it is expected to obtain more stable excess returns.
||
