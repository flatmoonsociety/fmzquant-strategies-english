...
![IMG](assets/images/f58b699653de06459a1b37232b835c64800073e2e8d8e32ba74017a3de3d7b71.png)

[trans]

## Overview
The double moving average crossover strategy is a quantitative trading strategy based on moving averages. It determines the price trend by calculating the intersection of fast and slow lines, and issues buy and sell signals at the intersection. This strategy uses the Hull moving average as the fast line and the Super Smoother filter as the slow line. This combination combines the smoothness of the moving average and the ability to judge trends, which can effectively identify price trends and generate more reliable trading signals.
## Strategy Principle
The calculation formula of the double moving average crossover point strategy is as follows:
Fast line (Hull moving average): WMA (2 * WMA (value, n/2)-WMA (value, n), SQRT (n))
Super Smoother: Value cubic filter
Among them, WMA represents the weighted moving average, SQRT represents the square root, and the filter contains a first-order lag term and two second-order lag terms.
The strategy determines the intersection relationship between the two lines by calculating the values of the fast line and the slow line, where:
The fast line crossing the slow line is a buy signal
When the fast line crosses the slow line, it is a sell signal.
## Advantage Analysis
The double moving average crossover point strategy combines the advantages of double moving average judgment and point trading, which can accurately grasp the turning point of the trend and enter and exit the market in a timely manner. Compared with a single moving average strategy, it has the following advantages:
1. Double moving average combination eliminates false signals. The fast line determines the direction and strength of the trend, while the slow line filters out shocks, making the signal more reliable.  
2. The Super Smoother filter has powerful data fittings capabilities and can effectively extract price trends.  
3. Hull moving average is highly sensitive to price changes and can capture turning points in time.
## Risk Analysis
The double moving average crossover point strategy also has certain risks:
1. In a volatile market, more whipsaw signals may appear. The moving average spacing can be appropriately widened to reduce false signals.
2. If the distance between the two moving averages is too wide, some opportunities will be missed. There is a trade-off between the quantity and quality of captured signals.   
3. This strategy is more suitable for trend clearer varieties and should not be used in high volatility products.
## Optimization direction
The double moving average crossover point strategy can be optimized from the following dimensions:
1. Adjust the moving average parameters to adapt to varieties with different periods and fluctuation ranges.    
2. Add additional indicators or filters to judge trend quality to reduce whipsaw.     
3. Optimize the proportion of opening a position based on trend indicators.
## Summary
The double moving average crossover point strategy inherits the advantages of the moving average strategy, and expands the use of double moving average judgment and point trading methods to form a more advanced and reliable quantitative trading plan. It has unique advantages in timing transactions and is worthy of real-time verification and application exploration. This article provides an in-depth analysis of the principles, strengths and weaknesses of this strategy, and provides optimization ideas for reference. I believe that as the model and parameters continue to improve, this strategy will become a powerful timing tool.
||
