...
Linear regression lines can reflect the long-term trend direction of prices. The short-term linear regression line has small parameters and is more sensitive to price changes and can capture short-term price reversal opportunities; the long-term linear regression line has large parameters and represents the long-term equilibrium trend of prices. When two moving averages cross, it represents a turning point in the short-term and long-term trends, so trading signals can be generated accordingly.
The advantage of this strategy is that it uses the classic technical analysis strategy of moving average crossover and adds linear regression analysis, which can simultaneously identify price turning points in both the long and short term time dimensions. However, the linear regression line is easily affected by abnormal data and has a certain hysteresis. In addition, moving average crossovers themselves will produce more false signals.
In order to filter out some false signals, this strategy adds time constraints and only executes strategic trading signals within the specified date range. This can reduce the number of invalid transactions to a certain extent. However, the setting of the time window is also subjective and needs to be optimized through backtesting.
